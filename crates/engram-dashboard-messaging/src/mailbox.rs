...
//!     하는 데 쓴다(service.rs `drain_queue` 0단계 — 겹치면 뒤 배치가 앞 배치의 잔여를 앞지른다).
// ADR-0103
// ADR-0104
// ADR-0105
// ADR-0111
...
impl Mailbox {
    ...
    /// 그 수신자의 현재 in-flight 총 건수(두 레인 합) = **지금 드레인이 락 밖으로 들고 나가 아직 정산되지
...
mod tests {
    ...
    fn an_out_of_lock_batch_stays_observable_until_it_settles() {
        // ★옛 `has_pending_ahead` 단언의 **이전분**★: 그 동사는 직발송 합류 판정이라 전부-큐와 함께
        //   사라졌지만(ADR-0125), 그것이 봉인하던 성질 — **drain↔정산 사이의 큐는 비어 있는데 그 배치는
        //   아직 수신자에게 닿지 않았다** — 는 그대로 남아야 한다. 이제 그 성질을 읽는 쪽은 상위의 드레인
        //   중복 진입 가드다(service.rs `drain_queue` 0단계): 이 값이 0 으로 보이면 두 드레인이 겹쳐 돌아
        //   뒤 배치가 앞 배치의 잔여를 앞지른다(= "배달 순서 = 적재 순서" 파괴).
        let mut mb = Mailbox::new();
        let t0 = Instant::now();
