...
//!   attach 실패)에서 배달이 **영구 대기**한다. 그래서 busy 는 **관측된 사실이 있을 때만** 참이다 — 자료구조도 그 의미를
//!   구조적으로 못 박는다: 표는 `HashSet<(PeerId, epoch)>` 로 **busy 인 것만** 담고, 부재 = idle ∨ 미관측
//!   (둘을 구분하지 않는다 — 둘 다 즉시 주입이 정답이라 구분할 이유가 없다).
//!
//! ★busy 관측 capability = `output.structured` 프록시★: 턴 이벤트(MessageDone)는
...
//!   프록시가 깨지는 날(= structured 이지만 턴 이벤트가 없는 백엔드 등장) 그때 진짜 capability 필드를
//!   추가한다 — ADR-0104 capability 원칙과 정합(관측 불가 = 즉시 주입 폴백).
//!
//! ★알려진 미확인(측정 항목 — spec §7)★: 중첩 Task 서브에이전트의 `result` 라인이 **부모 턴 종료**
//!   신호로 새는지 미검증이다. 새면 부모가 아직 턴 중인데 idle 로 오판해 조기 주입할 수 있다
//!   (유실은 없고 타이밍만 어긋남). 해결하지 않고 실 하네스 측정 항목으로 남긴 상태다.
//!
...
//!   메시징 최악 실패 모드). 그래서 tap 은 **구독 이후 발생분만** 받는다. 부트스트랩 없이 시작하는 대가는
//!   positive-knowledge-only 폴백이 이미 흡수한다 — 관측 전엔 idle = 즉시 주입.
//!
...
//!   busy 로 **관측된 시각**을 함께 들고, `BUSY_MAX_TURN` 을 넘긴 항목은 주기 sweep 이 청소하고 그 id 를
//!   **도어벨로 깨운다**(대기 메일이 그때 배달된다).
//!
...
///   이 시간을 넘긴 busy 표시는 주기 sweep 이 **비정상 종료된 턴의 잔해**로 보고 청소한다.
///
...
/// ★tap 부착 seam★ — 출력 스트림 구독을 트레잇 뒤로 밀어, 단위 테스트가 실 PTY·claude 없이
...
pub trait TapHost: Send + Sync {
    ...
    ///
    /// ★`probe` 는 무엇인가★: 호스트 어댑터가 자기 출력 계층에 붙어 분류한 뒤 이
    ///   수신구의 `on_progress`/`on_turn_done` 를 부른다(분류가 여기 없는 이유 = `TurnProbe` 주석).
    ///
...
impl TurnStates {
    ...
    fn mark_busy_at(&self, key: (PeerId, u32), at: Instant) {
        // 락 순서 attached → busy. attached 를 든 채 busy 를 잡는다 — 그 사이 attach 가 바뀌어
    ...
    /// 턴 종료 관측 — busy 해제 후 **락을 놓고** 통지한다(락 보유 중 외부 호출 금지).
    ///
    ...
    ///
    /// ★왜 통지를 여기서 하지 않나★: `notifier` 는 외부 호출이다 — busy 락을 든 채 부르면 락
    ...
    /// 새로 표시할 때 같은 id 의 **다른 epoch** 표시는 청소한다(한 id 에 살아있는 epoch 은 하나).
...
///   (`messaging_host::TurnTapSink`)가 하고, 커널은 **신호 어휘 두 개**만 소유한다.
///
