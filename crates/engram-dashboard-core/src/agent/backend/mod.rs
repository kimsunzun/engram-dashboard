...
impl InputEncoder {
    ...
    ///   `본문 + CR` 을 **한 번의 write** 로 넣으면 텍스트가 입력창에 남고 턴이 시작되지 않는다. 같은
    ///   바이트를 **두 번으로 나눠** 보내면(둘 사이 지연 0 이어도) 제출된다. 즉 제출을 만드는 건 바이트열이
    ///   아니라 **write 경계**라, 바이트열 하나를 돌려주는 `encode` 로는 표현할 수 없다 — 그래서 별도 질문이다.
