...
pub struct AgentProfile {
    ...
    /// **시도한 자리에서 관측된 사실**이고(ADR-0161 결정 1), 지금 상태(`AgentStatus`)와 별개 축이다.
    ///
    /// ★`#[serde(skip)]` 가 의미의 일부다(위 `had_session` 과 같은 이유, 다른 근거)★: 데몬을 내리면 그
    ///   세션들도 함께 죽으므로 옛 실패를 디스크로 붙들 이유가 없다 — 수명을 **데몬 수명**으로 못박는
    ///   것이 이 skip 이다(ADR-0160 3층). 파일 규격 변경·하위 호환 처리도 함께 면제된다.
    ...
    /// ★스냅샷이 되돌리지 못한다★: `upsert_preserving_hierarchy` 가 live 값을 보존한다(`had_session` 과
    ///   같은 인과 — spawn 호출부가 넘기는 옛 사본이 그 사이 기록된 실패나 지움을 덮으면 안 된다).
    // ADR-0161
    #[serde(skip)]
    ...
    pub fn upsert_preserving_hierarchy(&self, mut profile: AgentProfile) {
        self.mutate(|m| {
            if let Some(live) = m.get(&profile.id) {
                ...
                // 마지막 실패도 같다 — 그 사실을 쓰는 곳은 활성화 관측과 턴 종료 지움뿐이고, spawn 이
                //   넘긴 옛 사본이 그 둘을 되돌리면 이미 지워진 실패가 화면에 되살아난다.
                // ADR-0161
...
mod tests {
    ...
    /// 성공적으로 활성화하면(epoch +1) 뒤늦은 옛 관측이 **산 새 화신**에 죽은 화신의 실패를 찍는다.
    #[test]
    fn a_late_write_from_an_older_incarnation_is_rejected() {
        ...
        // 화신 교체: epoch 0 → 1.
        reg.update_with(id, |p| p.had_session = true);
        assert_eq!(reg.epoch_for_spawn(id), Some(1), "전제: 새 화신은 epoch 1");

        assert!(
            !reg.set_last_failure(id, Some(0), Some(AgentFailureKind::EarlyExitAfterResume)),
            "옛 화신(epoch 0)의 지각 관측은 거부돼야 한다"
        );
        assert_eq!(reg.get(id).unwrap().last_failure, None);

        assert!(
            reg.set_last_failure(id, Some(1), Some(AgentFailureKind::EarlyExitAfterResume)),
            "현 화신의 관측은 그대로 쓰인다"
        );
        // 지움도 같은 가드를 탄다 — 옛 화신의 성공 관측이 새 화신의 기록을 지우면 안 된다.
        assert!(!reg.set_last_failure(id, Some(0), None));
        assert_eq!(
            reg.get(id).unwrap().last_failure,
            Some(AgentFailureKind::EarlyExitAfterResume)
        );
        assert!(reg.set_last_failure(id, Some(1), None), "현 화신은 지운다");
