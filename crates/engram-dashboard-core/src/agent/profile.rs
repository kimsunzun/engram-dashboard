...
impl ProfileRegistry {
    ...
    ///
    /// ★epoch 도 같은 이유로 live 보존★: 재활성화 respawn 은 이 upsert **직전** `bump_epoch` 로 registry
    ///   epoch 를 올려두는데, 옛 스냅샷(epoch 그대로)을 삽입하면 그 bump 가 되돌려져 새 세션이 죽은 세션과
    ///   같은 epoch 를 재사용한다 → 프론트 재구독 누락(빈 슬롯). 신규 id(live 없음)면 스냅샷 epoch(0)를
    ///   그대로 써 첫 spawn 은 여전히 epoch=0.
    ...
    /// ★정규화는 호출자 책임★: trim·빈 문자열 거부·미변경 스킵을 여기서 하지 않는다.
    ...
    /// 호출해야 한다(ADR-0007). 호출점이 적다고 dead code 로 오인해 지우지 말 것 — 지우면 재활성화가
    /// epoch 를 재사용해 프론트 재구독이 누락되고(빈 슬롯) epoch-guard 도 구분자를 잃는다.
    // ADR-0084 ADR-0082
