...
/// 조회는 v1 목록에 **실린다**(ADR-0150 — 가정 B 채택). 빼면 발견이 절반에서 멈춘다.
#[test]
...
/// ★스키마가 「없어도 된다」고 광고한 필드만 실제로 생략 가능해야 한다★(ADR-0150 — 광고가 곧 사용법이다).
///
...
/// 파생 파일의 원소와 등록 패킷의 `help` 가 **같은 한 출처**에서 나온다(ADR-0150).
#[test]
