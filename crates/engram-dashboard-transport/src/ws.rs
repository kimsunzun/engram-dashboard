...
//!   없어 진단이 남지 않는다. ★패닉이 아니라 값인 것이 요점이다★ — 감독 태스크의 `JoinHandle` 을
//!   아무도 안 쥐므로 그 안에서 터진 패닉은 관측되지 않고 발행 상태가 `Live` 에 얼어붙는다(그 자리
//!   주석이 증상을 든다). 오류는 감독이 `LinkLost` 로 옮겨 끊김 사건 + 재연결로 만든다. 릴리스와
//!   디버그가 같게 동작한다.
...
impl<S: WsStream> LinkTx for WsTx<S> {
    fn send(&mut self, frame: Frame) -> BoxFuture<'_, Result<(), LinkError>> {
        Box::pin(async move {
            ...
            //   (`Supervisor::write`) 끊김 사건과 재연결이 남는다 — 진단이 살아남는다.
