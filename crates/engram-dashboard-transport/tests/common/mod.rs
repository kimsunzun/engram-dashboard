...
//! ★거는 쪽에는 예외가 없다★ — 새 기다림을 더하는 자리에서 가르는 것은 **누가 판정하나** 하나다:
//! 판정을 지는 쪽의 기다림에는 전부 상한이 붙고, 손님을 기다리는 것이 그 자체로 일인 태스크에만 안
//! 붙는다. ★그 자리를 「몇 번째 테스트」로 세지 말 것★ — 세는 문장은 테스트가 하나 늘 때마다 조용히
//! 어긋난다.
//!
