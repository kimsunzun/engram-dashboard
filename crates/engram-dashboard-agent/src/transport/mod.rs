...
/// 자식이 **첫 입력을 받기 전에** 스스로 찍은 출력 바이트를 그대로 넘겨받는 자리.
///
/// ★통로는 무엇을 찾는지 모른다(ADR-0004 · ADR-0216 결정 6)★ — 바이트를 넘기기만 한다. 찾는 대상도
///   찾는 방법도 그것을 꽂은 backend 안에 산다.
/// ★창의 경계가 이 타입의 이름이자 계약이다★: 통로는 자기 입력 쪽과 출력 쪽을 **둘 다** 보는 유일한
///   자리라, 「사용자가 첫 글자를 치기 전」이라는 창을 닫을 수 있는 것도 여기뿐이다. 그 앞의 화면에
///   있는 것은 자식 프로그램 자신의 장식뿐이고, 그 사실이 「처음 맞은 것을 쓴다」를 떠받친다. 창이
///   닫히면 통로가 이 값을 **버린다** — 그 뒤로는 한 번도 불리지 않는다.
/// ★수명·상태 = pump 스레드 단독 소유★([`OutputDecoder`] 와 같은 모양·같은 사유): 가변 상태를 들고
///   pump 가 `&mut` 로 배타 접근하므로 `Send` 만 요구하고 `Sync` 는 요구하지 않는다.
///
/// 호출 계약 — ★셋 다 pump 스레드에 걸리는 제약이라 어기면 그 에이전트가 통째로 상한다★:
/// - **`emit` 직전**에 매 `read` 마다 한 번. 넘어오는 것은 그 `read` 가 돌려준 슬라이스 그대로다.
/// - **락을 잡거나 I/O 를 기다리지 않는다.** pump 는 유일한 PTY 리더라 여기서 멈추면 자식에 역압이
///   걸린다. 오래 걸리는 일은 막히지 않는 전달로 넘기고 다른 스레드가 한다.
/// - **panic 하지 않는다.** pump 의 `catch_unwind` 가 panic 을 종점 전이(`Failed`)로 옮기므로,
///   여기서 터지면 멀쩡히 돌던 세션이 죽은 것으로 보고되고 출력이 끊긴다.
// ADR-0216
pub type PreInputByteObserver = Box<dyn FnMut(&[u8]) + Send>;
