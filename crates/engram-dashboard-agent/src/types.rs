...
///
/// 두 소비자가 같은 값을 봐야 한다: CLI 파서(무엇을 받나) · 프라이밍 판정자(본문이 **실행 가능한** 호출을
/// 가르쳤나). ★셋째였던 「사용자 안내 문구」는 ADR-0220 결정 5 로 사라졌다 — help 화면이 CLI 우편
/// 동사를 더는 가르치지 않는다.★ 판정자가 계열 토큰까지만 보면 `engram mail` 처럼 **동사 없는 조각**도
...
pub struct ControlEndpoint {
    ...
    ///   전원에게 깔리고, 우편 가부는 데몬의 자격증명 거절이 가른다. 데몬이 MCP-capable 일 때만 config 를
    ///   쓰고 그 write 실패 시 provision 을 Err 로 끊으므로 반쪽 MCP endpoint 는 존재하지 않는다.
    ...
    /// 이 스폰이 **CLI/HTTP 우편 입구(`engram mail …`)를 쓸 수 있는가**(ADR-0133 결정 2) — 데몬이
    /// 판정해 실어 주는 **사실**이고, 이 crate 는 그것을 파생하지도 재유도하지도 않는다.
    ///
    ...
    ///
    /// ★오늘 이 칸을 읽는 운영 코드는 하나도 없다 — 그래도 지운 것이 아니다★: 같은 판정이 자격증명에
    ///   박혀 데몬이 거절하고(`control/mcp_server.rs`), 이 칸은 그 자격증명 축의 **기록**으로 남는다.
    ///   한때 backend 가 이 값을 스폰 env 표식으로 번역해 CLI 사용법에서 우편을 감췄는데, 그 표식은
    ///   에이전트 자신의 env 라 강제를 하나도 못 하면서 외부가 화면을 고르는 입구만 열어 뒀다
    ///   (제거 = 사용자 결정 2026-09-23). ★그 번역을 되살리지 말 것★ — Rust 는 읽는 곳이 없어져도
    ///   경고하지 않으므로, 이 문장이 "왜 남았나" 의 유일한 기록이다.
    // ADR-0133
