...
async fn fixture_with_table(tag: &str, with_table: bool) -> Fixture {
    ...
    if with_table {
        // 라우트가 실제로 태우는 것이 이 표다(ADR-0134) — 운영 조립(`lib.rs`)과 같은 조립 함수를 쓴다.
...
async fn new_registers_a_sleeping_agent_and_refreshes_the_clients() {
    ...
    let (status, body) = f
        ...
        .await;
    assert_eq!(status, reqwest::StatusCode::OK);
    // 성공 본문은 평평하다 — 반환을 명령마다 선언하므로 한 겹 더 감싸지 않는다(ADR-0134).
...
async fn a_body_that_is_not_a_command_object_still_gets_a_reason() {
    ...
    for raw in [
        ...
        //   **루트로 떼기**로 조용히 바뀐다 — 어느 값을 고를 근거가 없으므로 고르지 않는다(ADR-0136).
...
/// ★반려 목록이 **선언에서 파생됐다**는 것까지 본다(ADR-0136)★: 문구가 틀린 칸만 짚고 끝나면 손으로 적은
