...
//!   본문은 **agent 가 소유**하고(선언이 사는 곳이 곧 주인) 검문·실행은 그 공통 입구가 하며(버스 배달과
...
//!     갈리고, 갈린 쪽이 조용히 통과시킨다.
//!   - **공백 값·부재/`null` 의 구분은 표 쪽(agent `commands`)이 본다.** 어느 칸이 그런지는 동사마다
//!     다르고, 그 판정은 동사 본문과 한 집에 있어야 둘이 갈리지 않는다.
//!
//! ★지목 토큰을 다듬지 않는다(trim 없음)★ — 이 계열은 **정확 일치**를 약속하므로 `"worker "` 는
//!   `worker` 가 아니다. 규칙도 그 실물도 agent 하나가 소유한다(`agent::commands::resolve_in`) — 이 입구는
...
///   이 입구는 스스로 통지를 만들지 않는다.
/// ★몇 번 부르는지는 동사가 정한다 — 「한 동사 한 통지」가 아니다★: 기준은 "명부 구성" 이 아니라
///   "프로필 목록이 나르는 것이 바뀌었나" 다. 활성화는 그 항목의 「마지막 실패」를 바꾸므로(ADR-0172)
///   깨우기도 1회 알리고, 만들기+띄우기는 등록 직후와 활성화 직후로 2회 알린다(중간 실패에도 화면이
///   최신이어야 한다). 옛 「깨우기 0회」 분담을 이 doc 근거로 되살리지 말 것.
// ADR-0172
// ADR-0132
// ADR-0130
...
///
/// ★인자를 타입으로 받지 않는 이유(load-bearing)★: 인자의 계약은 선언(agent)이 쥐고 있다. 여기서 다시
///   struct 로 받으면 **두 번째 인자 어휘**가 생겨, 선언에 칸이 하나 늘 때 이 파일이 함께 안 늘면 그 칸이
///   조용히 사라진다. 날 것으로 실어 보내면 모르는 칸의 판정도(ADR-0157) 값의 해석도 선언 한 곳에서 난다.
...
///   이기면 계층에 붙이려던 요청이 **루트로 떼기**로 조용히 바뀐다 — ADR-0157 이 막으려는 바로 그 형태다.
...
///   **약 3초 폴링**하고, 이름 변경·계층 이동은 프로필 락을 쥔 채 디스크에 저장한다(agent `make_table` doc).
...
mod tests {
    ...
    ///   여기서 아는 칸만 골라 담으면 모르는 칸이 이 파일에서 증발해 ADR-0157 의 거절이 성립하지 않는다.
    #[test]
