...
//!
//! ★배선 0★: 이 표로 배달하는 곳은 아직 아무 데도 없다. 에이전트 제어의 실제 입구는 여전히 이웃
//!   `agent.rs` 의 `/control/agent` 동사 match 이고, 그 자리를 이 표 조회로 바꾸는 것이 다음 조각이다.
//!   그래서 이 파일이 생겨도 `engram agent …` 의 동작은 한 톨도 변하지 않는다.
...
///   여전히 통지를 건너뛴 채 성공(ok)으로 응답한다 — 그 요청-도착 창은 기존 진입점(`agent.rs`)도 같은
///   슬롯을 그대로 읽으므로 이번에 새로 생긴 게 아니라 **원래 있던 창**이다.
// ADR-0134
