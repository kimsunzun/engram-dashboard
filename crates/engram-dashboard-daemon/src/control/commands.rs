...
//!
//! ★이 표를 부르는 표면은 **둘**이고 검문은 **하나**다★: 제어 라우트(`/control/agent` — 이웃 `agent.rs`
//!   가 `{verb}` 를 `agent.<verb>` 로 찾는다)와 명령 버스 배달의 1단계(`command_delivery`). 둘 다
//!   [`call_daemon_command`] 하나로 들어오므로 입구 검문(ADR-0157)을 건너뛰는 표면이 없다. 표가 슬롯에
//!   안 꽂혀 있으면 라우트는 503 이고 배달은 1단계 미스다.
...
/// 이 표를 부르는 **유일한 자리** — 두 표면(제어 라우트 `/control/agent` · 명령 버스 배달)이 여기로 든다.
///
/// 반환 `None` = **이 표의 이름이 아니다**. 그 갈래를 어떻게 대접할지는 부르는 쪽이 안다(제어 라우트는
/// 「모르는 동사」로 답하고, 배달은 3단계의 다음 단계로 넘어간다).
///
