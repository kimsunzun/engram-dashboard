...
//! 계약(trait)은 아래층인 네트워크 행이 소유하고 실물은 양쪽이 나눠 꽂는다 — `FrameSink` 는
//! 네트워크 행(`ws::ConnFrameSink`)이, `ConnectionHandler`/`ConnectionHandlerFactory` 는 위층이
//! 구현한다(ADR-0110 의 포트 소유 idiom과 같은 결).
...
pub trait ConnectionHandler: Send + Sync {
    ...
    ///   status 브로드캐스트가 같은 큐에 먼저 들어갔을 수 있다. FIFO 선두를 점유한다고 가정하지 말 것.
    ///
    ...
    ///   연결은 이 호출 **전에** 이미 fanout 레지스트리에 등록되므로, 그 순간부터 전-연결 status
    ///   브로드캐스트가 같은 큐에 동시에 슬롯을 넣는다. 즉 실제 여유는 용량보다 **작고 가변**이다.
    fn on_connect<'a>(
        &'a self,
        conn_id: ConnId,
