...
//!
//! 에이전트 행 테스트가 네트워크 행 실물(`ws::ConnFrameSink`·`ws::ConnRegistry`)을 이름으로도
//! 부르지 않게 하는 것이 존재 이유다 — crate 가 갈리면 그 이름들이 다른 lib 으로 가고, 지금 붙여
//! 두면 dev-dependency 로 되살아난다. 더블은 포트 계약(`frame_port`)에만 의존한다.
//!
//! ★네트워크 행 정책은 흉내내지 않는다★: 큐 포화 시 종료 신호(close_signal)나 팬아웃 대상 관리
//!   같은 것은 네트워크 살림이고 그 실물 계약은 `ws` 자기 테스트가 지킨다. 여기서 재현하면 테스트가
//!   실물이 아니라 더블을 검증하게 된다.
// ADR-0129
...
use crate::frame_port::{Frame, FrameError, FrameFanout, FrameSink};
...
/// (`ws::ConnRegistry`)의 계약이라 그쪽 테스트가 지킨다.
#[derive(Default)]
