//! 명령 주인 명부의 공유 핸들 — **전 연결이 같은 한 부를 본다**(ADR-0149/0150 · TRD §3-7).
//!
...
/// 클라이언트 자작 식별자로 대체한다(ADR-0144). 토큰 문자열에서 연결 id 를 되짚는 조회를 쓰면 그날
...
impl CommandRoster {
    ...
    /// 세 쪽이 갈라지지 않는다(ADR-0148). 그 저장값을 밖에서 물으려면 [`CommandRoster::attached_owner`].
    ///
    ...
    /// 읽는다. 그래야 파생 규칙이 바뀌는 날 명부 등재·제거·조회가 **같은 토큰**을 본다(ADR-0148 재론
    /// 트리거 ②).
    // ADR-0148
    ...
    /// 그 파생이 바뀌는 날 물은 값과 저장된 값이 갈라진다(ADR-0148).
    ///
    ...
    /// 호출자는 `OUTCOME_UNKNOWN` 으로 답한다(ADR-0148). ★소유자 부재로 접지 않는다 — 그건 거짓
    ...
    ///
    /// ★ADR-0148 이 못 박은 제거 순서(명부가 먼저, 닿는 길이 나중)를 그대로 쓴다★ — 오늘은 둘이 한
    ...
    /// 데몬 수명 내내 `Available` 로 남는다(ADR-0148). 표에 없는 연결이면 지울 것도 없다 — 등록 자체가
    ...
    /// [`CommandRoster::attach`] 가 저장한 토큰이다 — 여기서 다시 파생하지 않는다(ADR-0148).
...
impl Shared {
    ...
    /// 것이다(파생 지점은 [`CommandRoster::attach`] 하나. ADR-0148). 반려 갈래에서는 토큰이 없고, 그것이
    /// 곧 이 그물의 뜻이다.
    ///
