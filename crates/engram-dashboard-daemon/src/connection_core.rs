...
impl ConnectionCore {
    ...
    pub async fn dispatch(
        ...
        match cmd {
            ...
            AgentCommand::SpawnProfile {
                ...
                //     새로 뜬다. 어느 쪽이든 프로세스는 멀쩡히 서므로 활성화 판정은 성립으로 떨어지고,
                //     **결과는 「새 대화인데 이어받음으로 보고된다」**다 — claude 처럼 요란하게 실패하지
                //     않는다. 즉 codex 에서 이 플래그가 뜻하는 것은 「이어받아라」가 아니라 ★「세션이
                //     없어도 좋으니 띄워라」★ 이고, 손잡이가 없으면 조용히 새 대화가 된다.
                //   ★이것을 동작 변경으로 고치지 않았다★ — 「이어받기 요청을 낼 수 없는데 Resume 으로
                //     띄운다」를 실패로 접을지는 사용자 결정이다(claude 와 맞출지, codex 를 정직하게
                //     `Fresh` 로 보고할지, 플래그 자체를 없앨지). 여기 적어 두는 것은 **읽는 사람이 이
                //     자리에서 그 차이를 만나게** 하기 위함이다.
    ...
    fn handle_subscribe(
        ...
        //   해제하지 않는다(오귀속 방어). 셋 다 셸의 `daemon_client::replay_flight` 가 내리는 결정이고
        //   바꿀 것이 아니다.
