...
export interface Command {
   ...
   * 자리에 있어야 한다는 **정책 결정**이 있고, 그 결정의 정본이 딴 데 있다」다.
   *
   * ★오늘 이 칸을 쓰는 command 는 **하나도 없다**(2026-09-22)★ — 유일한 사용처였던 codex 생성 문 둘이
   * 게이트를 걷었다(사유 = `agentCommands` 머리 주석 · 정본 = `engram-dashboard-agent` 의
   * `commands::LLM_BACKEND_POLICY`). ★그래도 이 칸을 걷지 않는다★: 위 문단이 정의하는 조건이 다시
   * 성립하는 날 닫을 수단이 필요하고, 그 조건은 「LLM 이 못 지나는 UI」가 아니라 **정책 결정**이다.
   * 게이트 자체의 동작은 `registry.test.ts` 가 합성 command 로 계속 잰다 — 사용처가 0 이어도 죽지 않는다.
   *
