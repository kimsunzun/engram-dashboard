...
// Changes: copied verbatim (no VSCode wiring); remapped theme tokens text-description/to-description -> text-muted/to-muted
//   (our data-theme tokens); Button/cn imports resolve to our tree.
		...
		return (
			<div className="ml-1 pl-0 mb-0 -mt-[2px]">
				{showTitle ? (
					<Button
						className={cn(
							'inline-flex justify-baseline gap-0.5 text-left select-none px-0 py-0 my-0 h-auto min-h-0 w-full text-muted overflow-visible',
							{
						...
						<span
							className={cn('text-[13px] leading-[1.2]', {
								'animate-shimmer bg-linear-90 from-foreground to-muted bg-[length:200%_100%] bg-clip-text text-transparent':
									isStreaming,
						...
						{showChevron &&
							(isExpanded ? (
								<ChevronDownIcon className="!size-1 text-muted" />
							) : (
								<ChevronRightIcon className="!size-1 text-muted" />
							))}
					</Button>
				) : null}
				...
				{isExpanded && (
					<Button
						...
						<div className="relative flex-1">
							<div
								className={cn(
									'flex max-h-[150px] overflow-y-auto text-muted leading-normal truncated whitespace-pre-wrap break-words pl-0 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden [direction:ltr]',
								)}
