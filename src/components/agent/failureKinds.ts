// 「마지막 실패」 종류별 처분표 — **화면이 문구를 얻는 유일한 곳**(ADR-0172 결정 5 · ADR-0173 §영향).
//
...
//   ADR-0172 결정 6 으로 후속). 양쪽에 두면 컴파일러가 못 맞추는 사본이 둘 생긴다. 소비자가 코어로
//   옮겨가면 그때 표도 함께 옮긴다.
...
export type FailureKindSpec = {
   ...
   * 쓸 자리이고 둘 다 후속이다(ADR-0172 결정 6). 지금 지우면 그 기능이 표를 다시 발명한다.
   */
  retryable: boolean
  /** 무슨 일이 있었나 — 한 줄. */
  reason: string
