...
//
// ★한 행의 기호는 하나뿐이다(ADR-0170)★: 축이 늘어도(상태 · 마지막 실패) 기호를 나란히 두지 않고
//   갈아끼운다. 그 판정은 `isFailureBlocked` 한 곳에 있고 기호·색·hover 가 함께 그것을 따른다.

import type { AgentFailureKind } from '../../api/types'

/** 마지막 실패가 상태 기호를 갈아끼울 때 쓰는 금지 표식(ADR-0170 — 기호를 하나 더 늘리지 않는다). */
const BLOCKED_GLYPH = '⊘'

/**
 * 마지막 실패가 지금 이 행의 기호를 차지하나 — **기호·색·hover 가 함께 따르는 단일 판정**.
 *
 * ★도는 중이 이긴다(ADR-0170)★: 마지막 실패를 들고 있어도 떠 있으면 원래 상태 기호를 그린다. 실패 표시는
