---
name: research
description: 주계열이 수집·합성하고 메인이 grounding으로 검증한 뒤, 종합·논쟁·confident-wrong 위험이 있으면 cross-family(blind) 리뷰어가 산출물을 적대 리뷰해 출처 단 보고서를 만든다. 강도(light/medium/deep)가 라우팅한다 — 단발 사실은 light, 종합·논쟁은 medium+, 고위험·비가역은 deep. 트리거 /research "<주제>" [light|medium|deep].
---

# Research

주계열 팬아웃이 수집·합성하고 메인이 grounding으로 검증한 뒤, 종합·논쟁·confident-wrong 위험이 큰 산출물만 cross-family(blind) 리뷰어가 적대 리뷰해 출처 단 보고서를 만든다. 핵심은 라우팅 — 강도(light/medium/deep)로 싸게 단일 수집으로 끝낼지 vs 적대 리뷰로 검증할지를 가른다.
...
- **수집 = 단일 주계열 + calibration.** 수집자는 모르면 기권하고 honest 확신도를 단다. 이 규칙이 옛 이중 수집 교차의 사실검증 몫을 대부분 대체한다 — 현대 모델은 obscure 사실에 확신-환각 대신 기권으로 self-calibrate하므로. cross-family 병렬 수집은 deep 옵션으로만 둔다.
...
- **abstention ≠ contradiction.** 한쪽이 근거 있는 답, 다른 쪽이 단지 기권이면 grounding 통과 시 채택 — 정면으로 다른 답을 낼 때만 contested. (`§5`)
- **확신도 = grounding + 리뷰에서 파생.** 자기보고 %도, 수집자 합의도도 근거가 아니다. 최상위 '확실'은 독립 교차확증(제2 1차자료/deep 독립 수집 합의)이 있어야 주고, 리뷰가 공격 안 한 단일 출처는 '가능성 높음'까지. (`§5`)
- **mode-aware 에스컬레이션.** 남는 load-bearing contested/미지지는 대화 모드 = 사용자 질문 / 자율 모드 = 태그 유지 + 로그. 마이너는 메인 자율. (`§5`)
