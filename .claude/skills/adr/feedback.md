...
| 2026-07-05 | **1 결정 → 다중 옛 ADR 부분 개정 미지원** — ADR-0046이 0040 전체 폐기 + 0007/0037/0043 부분 개정을 동반했는데, `supersede --mode partial`은 호출마다 새 ADR을 만들어 "하나의 새 ADR이 여러 옛 ADR을 Amends"를 표현 못 함. 이번엔 스크립트로 0040 full 처리 후 0007/0037/0043 양방향 Amends 링크를 수기 부착(스크립트 생성 형식 그대로) + `lint` 청정으로 검증 — lint·index는 다중 Amends를 이미 지원하므로 `supersede --amend-only --old N --new M` 류 오퍼레이션 추가가 자연스러운 확장. | 미반영 |
