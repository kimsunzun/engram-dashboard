...
서기 스크립트는 **스킬 폴더 안** `scripts/adr.mjs`(자족 — 스킬과 함께 소비처로 이동)다. 하나의 스크립트가 여러 소비처를 섬기고, 프로젝트 실값(ADR 폴더·인덱스 파일·템플릿·상태 어휘·앵커 루트)은 CLI 플래그로 주입한다 — 기본값은 dashboard 동작 그대로다. 실명령·경로·플래그는 `references/bindings/<project>.md`가 정본(engram = `references/bindings/engram.md`, 작업장 자체 = `references/bindings/skill-factory.md`). 스캐폴드 템플릿 실체는 `references/formats/`(dashboard = `adr.template.md`, 경량 = `adr-light.template.md`). 골격은 스택을 모른다 — 경로·명령·템플릿을 하드코딩하지 않고, 다른 프로젝트는 바인딩 파일만 추가한다.
