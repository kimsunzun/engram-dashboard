...
전용 체크리스트·명령·연동(code 단계 코드 불변식, QA 실측 명령, 결정 기록 방식)은 `references/bindings/<project>.md`. 현재 engram = `references/bindings/engram.md`. 다른 프로젝트는 같은 골격(`references/flow.md`)에 바인딩 파일만 추가한다. 스킬 골격에 특정 스택·불변식을 하드코딩하지 않는다.
