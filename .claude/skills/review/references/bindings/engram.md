...
doc-aware 렌즈(TRD·code·doc 단계의 opus)에 줄 ADR·불변식 묶음을 미리 추린다. 위 "code 단계 게이트" 목록 + 닿은 코드의 `// ADR-` 앵커(`rg "ADR-"`)가 출발점이다. blind 렌즈(PRD Advocate·Codex blind breaker)에는 주지 않는다.
