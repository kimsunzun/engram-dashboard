...
> **정본 = 코드의 `// ADR-NNNN` 앵커 주석 + 각 ADR(`docs/decisions/`).** 이 파일은 리뷰 때 꺼내 쓰는 **점검용 목록**일 뿐이다 — 코드·ADR이 바뀌면 rot한다. 불변식을 통째 복붙해 정본으로 착각하지 말고, 의심되면 `rg "ADR-"`로 코드 앵커를 확인한다.

## code 단계 게이트 — 우리 불변식 (골격 §2 code 행 "체크리스트 출처"에 주입)

code 단계 Adversary(opus doc-aware breaker)는 다음 불변식 위반을 공격 표면으로 삼는다. doc-aware 렌즈에만 준다(blind 렌즈엔 주지 않는다 — 앵커링 차단).
...
## QA 실측 게이트 명령 (골격 §5 "QA 실측 게이트"에 주입)

리뷰 판정과 무관하게 항상 돈다. 강도별 실명령은 qa 스킬 바인딩(`../../qa/references/bindings/engram.md`)이 정본이다 — 여기 베끼지 않는다.

- build/test — `cargo test`(워크스페이스 루트) / 멤버별 `cargo test -p engram-dashboard-core` · `-p engram-dashboard-protocol` / `cargo build`(루트).
- GUI 실측 — `scripts/cdp.mjs`(WebView2, Windows 전용). 화면·동작이 걸린 변경은 여기까지. **코드(test/tsc) PASS ≠ 동작 보장.**

## doc-aware 컨텍스트 준비 (골격 §1·§3에 주입)

doc-aware 렌즈(TRD·code·doc 단계의 opus)에 줄 ADR·불변식 묶음을 미리 추린다. 위 "code 단계 게이트" 목록 + 닿은 코드의 `// ADR-` 앵커(`rg "ADR-"`)가 출발점이다. 로깅이 걸린 변경이면 `docs/reference/logging-conventions.md`도 doc-aware 묶음에 넣는다(레벨·계측 의무·보안 준수 판정용). blind 렌즈(PRD Advocate·Codex blind breaker)에는 주지 않는다.

## 결정 기록 (골격 §6 "결과 보고 + 후속"에 주입)
