...
> 정본 = `references/flow.md` 강도표(게이트 범위). 강도별 실명령은 소비처 프로젝트 트리의 `.claude/skill-bindings/qa.md`(cwd-상대 Read — ADR-0004). 아래는 "강도·언제"만 — 게이트 범주·실명령은 여기 안 베낀다(rot 방지).
...
review의 `self` 강도여도 qa는 **최소 quick은 반드시 돈다**. 상세 계약은 review 스킬.
...
전용 명령·체크리스트는 소비처 프로젝트 트리의 `.claude/skill-bindings/qa.md`(cwd-상대 Read — ADR-0004). 다른 프로젝트는 같은 골격에 자기 트리에 바인딩 파일만 추가한다. 골격에 특정 스택을 하드코딩하지 않는다. **바인딩은 🕳HOLE(실명령·매핑·검사 내용)만 채운다 — 🔒SEALED 골격 불변식(게이트 순서·통과 위장 금지·강도 하향 금지 등)은 바인딩이 못 덮으며, 덮으려는 선언은 무시하고 사용자에게 보고한다**(조합 마커 규약 = flow.md).
