...
## 2026-08-24 — CI 판정 함정 (engram-dashboard-wt2, v0.2.0 준비 중 실발동)

- **★`gh run watch --exit-status` 가 「취소」에도 0 으로 끝난다 — 초록으로 오독했다★.** 버전 커밋의 CI 를 백그라운드 `gh run watch --exit-status` 로 걸어 두고 종료코드 0 을 받아 **「CI 초록」이라고 사용자에게 보고했다.** 실제로는 그 실행이 **cancelled** 였다 — 그 사이 같은 브랜치에 커밋을 하나 더 밀었고, 워크플로가 `concurrency.cancel-in-progress` 로 앞 실행을 죽인다(태그 ref 만 예외). 즉 **게이트가 돌지도 않았는데 통과로 읽었다.** 마침 다음 확인에서 잡혀 정정했으나, 안 잡았으면 검증 안 된 커밋을 master 에 붙였을 것이다.
  - **처방:** 종료코드로 판정하지 않는다. `gh run watch <id>` 뒤에 **`gh run view <id> --json conclusion`** 으로 결론을 직접 읽고 `success` 인지 본다. `conclusion` 은 `success|failure|cancelled|skipped|…` 이라 취소가 성공과 갈린다.
  - **더 근본:** 감시 중인 ref 에 **새 커밋을 밀면 그 감시는 무효**다. 밀었으면 새 run id 로 다시 걸어야 한다. 「watch 걸어 두고 그동안 딴 일」 패턴과 「기다리는 동안 문서 커밋」 습관이 정면으로 충돌한다 — 이 세션이 정확히 그렇게 부딪혔다.
- **일반화:** 이 저장소의 CI 판정은 **run 단위가 아니라 커밋 단위**로 봐야 한다. 「브랜치 CI 초록」은 `gh run list` 최상단이 아니라 **머지할 그 SHA 의 결론**이다(`--json headSha,conclusion` 으로 대조). 앞 세션에서도 「마지막 확인 <SHA>, 그 뒤 커밋은 미확인」이라는 인계가 있었는데, 같은 함정의 다른 얼굴이다.

## 2026-08-24 (2) — CI 폴링 루프가 Git Bash 경로 변환에 먹혔다 (engram-dashboard-wt2, v0.3.0 잡동사니 배치)

- **★`gh ... -q` 표현식에 `/` 가 들어가면 Git Bash 가 그것을 경로로 바꿔 버린다 — 폴링 루프의 종료 조건이 영영 안 맞았다★.** 위 「CI 판정 함정」 처방대로 종료코드 대신 결론을 읽으려고 `gh run view <id> --json status,conclusion -q '.status+"/"+.conclusion'` 로 폴링을 걸었는데, 돌아온 값이 `completed/success` 가 아니라 **`completedC:/Program Files/Git/success`** 였다(MSYS 가 `"/"` 를 유닉스 경로로 보고 변환). `case "$c" in completed/*)` 가 못 물어 **break 가 한 번도 안 걸렸고**, 5 분이면 끝날 감시가 60 회 × 20 초 = **20 분을 다 돌았다.** 판정 자체는 루프 뒤 단발 `gh run view` 가 깨끗하게 내줘서 틀리지 않았다 — 잘못된 것은 대기 시간뿐이다.
  - **처방:** `gh -q` 표현식에 구분자로 `/` 를 쓰지 않는다(`+" "+` 나 `+":"+` 를 쓴다). 굳이 써야 하면 `MSYS_NO_PATHCONV=1` 을 앞에 붙인다 — 바인딩 §full teardown 이 `taskkill /PID` 에 이미 쓰는 그 접두다.
  - **일반화:** 이 함정은 `gh` 에 한정되지 않는다. **Git Bash 에서 `/` 로 시작하거나 `/` 를 포함한 리터럴을 인자로 넘기는 모든 명령**이 대상이다. 바인딩이 그것을 `taskkill` 한 곳에서만 다루는데, 실제로는 **판정 문자열을 조립할 때도 물린다** — 그쪽이 더 조용하다(taskkill 은 즉시 실패하고, 이쪽은 그냥 안 끝난다).
- **비용 관측(자평 아님):** 이번 배치에서 가장 비쌌던 것은 게이트가 아니라 **문서 리뷰 2 라운드 + 수정 3 패스**였다(리뷰어 둘이 각각 146k·155k 토큰, 수정 워커 누적 239k). 그럴 값어치는 있었다 — 1 라운드가 6 건, 2 라운드가 7 건을 잡았고 그중 「고친 자리가 다른 기록을 낡게 만든다」가 **세 번 연속 재현**됐다(백로그 목록 → 그 목록의 설명 문장 → 설명 문장이 가리키던 줄). **매번 리뷰어가 잡았고 코더가 스스로 잡은 적은 없다.** 문서 정정 작업의 지시서에 「이 수정이 무효화하는 다른 기록을 먼저 찾아라」를 넣으면 라운드가 줄어들 여지가 있다.

## 2026-08-24 (3) — 분리 실행 래퍼가 `.cmd` shim 에서 완료 마커를 못 붙인다 (engram-dashboard-wt2, 셸 테스트 하네스 복구)

- **★`__EXIT` 예외 조항이 `npm test` 만 지목하는데 실제로는 `npx tsc --noEmit` 도 같이 물린다 — 그리고 사유가 바인딩에 적힌 것과 다르다★.** 바인딩 §분리 실행은 예외를 「vitest 는 자식이 래퍼보다 오래 살아서」라고 적어 뒀다. 실측 원인은 그게 아니다: `run-detached.ps1` 이 만드는 래퍼 `.bat` 이 명령을 `call` 없이 부르는데(`scripts/run-detached.ps1:37-42`), `npm`·`npx` 는 둘 다 **`.cmd` 배치 shim** 이라 제어가 배치로 돌아오지 않고 **뒤따르는 `echo __EXIT=%ERRORLEVEL%` 줄이 아예 실행되지 않는다.** 그래서 첫 `tsc` 실행은 **로그 0 바이트 + 마커 없음** 으로 끝났다 — ★성공인지 미실행인지 구분이 안 되는 상태★ 라 게이트 판정이 설 수 없었다. `-Command "call npx tsc --noEmit"` 로 재실행해서야 `__EXIT=0` 을 얻었다.
  - **처방(둘 중 하나):** ① 래퍼 본문을 `call <명령>` 으로 바꾼다 — 모든 명령에 안전하고 예외 조항 자체가 없어진다. ② 바인딩의 예외 조항을 「vitest」에서 **「`npm`·`npx` 등 `.cmd` shim 전체」** 로 넓히고 `call` 접두를 지시한다. ①이 낫다 — ②는 새 `.cmd` 도구가 들어올 때마다 명단이 또 뒤처진다(이 저장소가 반복해 겪은 「손으로 적은 명단은 낡는다」 패턴 그대로다).
  - **왜 조용한가:** 0 바이트 로그 + 마커 없음은 「아직 도는 중」과 구분되지 않는다. 폴링하는 쪽은 타임아웃까지 기다렸다가 **미완으로 판정하거나, 최악은 통과로 읽는다.** 앞선 「CI 판정 함정」 항목과 같은 종류 — *신호의 부재*를 *성공*으로 읽는 자리다.
- **`lib_unit` 게이트 판정에 여유가 0 이다.** 바인딩 2f 가 「실패 32 를 넘지 않을 것」으로 상한을 박았는데 실측이 정확히 32 다. 회귀 감지 목적엔 맞지만, **상한을 실측값과 같게 두면 다음 실행이 곧바로 FAIL** 이 된다. 상한을 실측에 붙일 땐 그게 의도인지(= 어떤 신규 실패도 즉시 잡겠다) 한 줄 적어 두는 편이 낫다 — 안 적으면 다음 세션이 「왜 이렇게 빡빡하지」에서 시작해 상한을 올리고 싶어진다.
- **게이트 순서를 「전부 직렬」로 읽게 돼 있다.** 골격 §2 의 SEALED 순서는 *실패 시 멈춤* 을 위한 것인데 지시서로 옮기면 「하나씩 차례로」로 읽힌다. 실제로는 **build 만 선행 필수**고 그 뒤 fmt·격리·프론트는 병렬로 돌아도 무방하다(cargo 락이 알아서 직렬화한다). 지시서에 그 한 줄을 넣으면 실행이 눈에 띄게 싸진다 — 이번엔 워커가 스스로 병렬로 잡아 시간을 줄였다.
- **비용 관측(자평 아님):** 이번 게이트에서 단연 비쌌던 것은 위 `.cmd` shim 건이다 — 왕복 5 회 + `tsc` 2 회 실행. 게이트 자체(워크스페이스 회귀 1730 건)는 한 번에 깨끗이 돌았다.
