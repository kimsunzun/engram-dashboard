...
test -d crates/engram-dashboard-agent/src || echo "FAIL — 게이트 경로 부재(crate가 개명·이사했나)"   # 4-pre) ★경로 존재 확인 — 없는 경로면 아래 rg가 0줄을 뱉어 PASS로 읽힌다★
