...
- 잡는 경로(코드 파생): 인스턴스를 들고 있는 것은 `terminalRef` 하나이고 window에 노출되지 않는다(`TerminalSlot.tsx:20`·`:90`). ★**시작점은 `.xterm`이 아니라 그 부모다**★ — fiber 키(`__reactFiber$*`)가 붙는 것은 React가 렌더한 컨테이너 div(`TerminalSlot.tsx:341`)이고 `.xterm`은 `term.open()`이 그 안에 만든 것이라(`:85`) 키가 없다. 슬롯이 여럿이면 `[data-slot-id]`로 먼저 좁힌다(`src/components/layout/LayoutLeaf.tsx:151`).
