...
> **정본 = `<skill>/scripts/adr.mjs`(서기 작업 실체) + `docs/decisions/README.md`(템플릿·인덱스·상태 범례·supersede 규칙) + CLAUDE.md "★ 설계 결정 기록 (ADR) — 강제 ★" 절.** 이 파일은 그 **현재 바인딩 스냅샷**일 뿐이다 — 충돌하면 스크립트/README.md/CLAUDE.md를 따르고 이 파일을 고친다(rot 방지). 템플릿·인덱스 표를 통째 복붙해 제2의 출처로 만들지 않는다 — *가리키기만* 한다.
