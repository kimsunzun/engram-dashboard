# 핸드오프: **0-4(인증 이사) 완료 + auth feature 경계** — ADR-0129 결정 1 불변식 **완전 충족** · **슬라이스 2·3은 ADR-0130으로 보류(2026-08-05)** · **푸시 완료(미푸시 0)** · 워킹트리 클린

> **★2026-08-05 갱신 — 이 문서의 슬라이스 2 관련 서술은 전부 무효★** ADR-0130이 ADR-0129의 결정 2·3(응용 층 lib 추출 + 얇은 조립)을 **보류**했다(사용자 결정). 아래 「슬라이스 2 — 착수 전 반드시 볼 것」 절과 "다음 첫 액션 = 슬라이스 2" 서술을 **따르지 말 것.** 특히 **"모듈 순환 3개"는 실측으로 반증됐다** — production 빌드에 순환이 없고, 기록된 순환은 테스트 블록 안 픽스처가 형제 모듈로 손을 뻗어 생긴 것이다. 근거·재개 조건 = **ADR-0130**, 경위 = **step-log S18.24**. 이 문서의 나머지(검증 상태·do-not·미결 목록)는 유효하다.

> **"에이전트 어휘 0"이 달성됐다.** 슬라이스 1이 의도적으로 이월했던 마지막 예외(`ws.rs`의 `AgentCommand::Auth`/`PROTOCOL_VERSION`)가 사라졌다. 0-4와 feature 경계는 커밋·푸시·기록까지 닫혔으니 **다시 손대지 말 것.**
...
- ~~**★다음 첫 액션 = 슬라이스 2(에이전트 시스템 lib 추출)★**~~ — **취소(ADR-0130 보류, 2026-08-05).** 여기 적힌 대로 범위 조사를 먼저 돌렸고, 그 조사 결과가 곧 보류 결정의 근거가 됐다(순환 반증 · 소비자 부재).
