...
> **결론(요지):** 프라이밍 v3(출력 불가시성 + 원칙자 앵커 + 사전승인 귀속, 메타주석 0)로 **실측 전 케이스 라우팅 성공**. 봉투 내 지시문·선제 항변·실험 주석은 보안 정책과 충돌해 역효과(실측+반증 출처 일치).
...
| **v3: v2 본문 + 주석 전면 제거** | colon | 2/2 | **2/2** | **1/1** | 보안 플래그 0 |
| **v3** | bracket 무-id | — | **2/2** | **1/1** | 보안 플래그 0, colon과 차이 없음 |

(haiku v3 = 별도 sanity 2런 — 본문 v2와 동일하고 haiku는 전 반복에서 플래그 0이라 동등 예상; 결과는 step-log에 추기)
