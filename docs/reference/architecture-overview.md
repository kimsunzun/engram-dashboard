...
>
> **주제별 상세는 여기 없다** — 이 문서는 조감도이고, 한 주제의 상세 구조(그림·argv·capability 선언·정책 표)는 `structure/`(`ls docs/reference/structure/`)가 갖는다. 상세를 이 문서로 되올리지 않는다(ADR-0197).
>
...
**복원은 프로필에 저장된 backend sid 단독에 의존한다 — 발급 주체는 백엔드가 정한다**(추적 파일은 best-effort). claude는 spawn 시 `--session-id`로 우리가 발급해 넘기고 `--resume`으로 **무손실** 복원한다(ADR-0008). ★**무손실은 claude 축의 말이다 — 발급 주체 중립 불변식에 붙이지 말 것**★: codex 는 스레드를 스스로 발급하고, 그 id 로 `thread/resume` 을 낸다(app-server 모드 한정 — 그래서 `capabilities().session.resume` 도 모드마다 갈린다). 그 이어받기가 **무손실**인지는 재 본 적이 없다. ★발급 주체를 불변식에 되올리지 말 것★ — 새 매핑·새 필드도 만들지 않는다(우리 id = `AgentId`, 백엔드 id = `backend_session_id`, 매핑 = 프로필 레코드).

- ★**발급받는 백엔드(codex)의 수령은 이제 배선돼 있다 — 「없다」로 적힌 옛 문장을 되살리지 말 것**★ (ADR-0185). codex는 클라이언트 지정을 아예 받지 않으므로(`ThreadStartParams`에 id 계열 속성 0개) 스레드 생성 **응답**에서 받는다. 조립점이 `open_spawn`에 기록 동사 하나(`SessionIdSink`)를 넣어 내려보내고, app-server 통로의 라이터가 `thread/start` 응답을 받은 자리에서 그것을 부른다 — ★`Link::Ready`보다 **먼저**★. 그 동사는 이제 이 화신의 첫 제출 래치에 건네는 것(offer)이다 — 영속은 첫 제출 때 래치가 화신 표식을 들고 비교-교체(`commit_session_id`)로 한다(ADR-0226 · 옛 서술 「`observe_session_id`로 간다」는 낡았다). ★**그 id 는 이제 읽히기도 한다 — 「적힐 뿐 이어받기에 쓰이지 않는다」로 적힌 자리를 만나면 낡은 것이다**★: 조립점이 Resume 스폰에서 `profile.backend_session_id` 를 `open_spawn` 의 `resume_session_id` 로 함께 넘기고, codex 가 그 값이 있으면 `thread/start` 대신 `thread/resume { threadId }` 를 낸다. 그래서 `can_resume_stored_session` 도 `capabilities().session.resume` 도 `is_app_server(command)` 다 — **둘이 같은 술어로 함께 켜지고 모드마다 갈린다**. ★수령 동사는 이어받기 갈래에서도 그대로 불리고, 적는 값은 **상대가 답한 id** 다★ — 우리가 보낸 것을 되쓰지 않는다(다르면 그 다른 값이 이 화신이 실제로 말하는 스레드다). ★거절당한 이어받기는 새 스레드로 되돌아가지 않는다(ADR-0082)★ — codex 쪽 보강 사유는 `-32600` 이 「모르는 스레드」 전용이 아니라는 것이다(모르는 메서드·중복 `initialize`·설정 오류가 전부 그 코드 — 실측 0.154.0). ★**claude `/clear` watcher를 codex의 수령 경로로 오해하지 말 것**★ — `backend::session_id_source`는 **우리가 발급한 기준 sid**(`expected_sid: Uuid`)를 요구하는 파일 폴러이고(`AgentBackend::session_id_source`), codex의 id는 JSON-RPC 응답으로 온다. 그 watcher는 지금도 별도 경로이고, 화신 표식 자리엔 `None`을 넘긴다(그 축을 나르지 않는다).
- ★**그 가드는 이제 있다 — 「없다」로 적힌 옛 문장을 되살리지 말 것**★. 활성화 입구 둘(명령 버스 `wake_existing` · WS `SpawnProfile`)과 부팅 복원이 **같은 판정 하나**를 부른다 = `backend::can_resume_profile(profile)`(= 이어받기 축 ∧ 저장된 sid 존재). 세 곳이 규칙을 각자 적던 것을 한 함수로 모은 것이라, 네 번째 입구가 두 항 중 하나만 옮겨 적을 자리가 없다. 회귀망은 입구마다 하나씩 있다(버스 = `commands` 의 `waking_does_not_resume_…` · WS = `connection_core` 의 `ws_spawn_profile_does_not_resume_…` · 판정 자체 = `backend` 의 `a_stored_sid_alone_does_not_make_a_profile_resumable`).
  - ★**그래도 두 구멍이 남는다**★: ① WS 입구의 `resume=true` 명시 요청은 그 술어를 **타지 않는다**(`resume || can_resume_profile(…)`) — 그 갈래로 들어오면 이어받을 수 없는 백엔드도 Resume 으로 뜬다. ② **스폰 결과에 새 대화라는 표식이 없다** — `AgentSpawnOk`(`commands.rs` 의 그 타입 선언)도 `AgentEvent::Spawned`(`protocol/src/messages.rs`)도 resume 여부를 안 싣는다. ★「wire에 하나도 없다」는 낡았다★ — 2026-09-24(ADR-0226)부터 구독 응답 `SubscribeAck.continues_conversation` 이 「이 화신은 이어받으려고 떴다」(스폰 때 손잡이를 실었나 — 이어받기의 성공 여부가 아니다)를 싣고 챗 슬롯이 그것으로 첫 화면과 로딩을 가른다. 스폰 결과 쪽의 구분은 `docs/process/S21-codex-backend/trd-resume-after-first-turn.md` §7 Q3(Phase B) 몫이다. ★**`resumed` 를 grep 해 찾으려 하지 말 것**★ — 그 이름은 `commands.rs` 의 `#[cfg(test)]` 테스트 더블 안(`FakeHost::activate_profile`)에만 있다. 두 입구가 서로 다른 규칙을 쓰는 위험은 위 공용 판정이 닫았지만, 그것과 이 표식 갭은 **다른 축**이라 여전히 남는다.
- **「받은 sid 를 첫 턴 허용 전에 persist 한다」 — 두 반쪽의 상태가 갈렸다(한 문장으로 읽지 말 것):**
  - **순서 반쪽 = 강제된다.** 기록 호출이 `Link::Ready` 보다 **먼저** 나고, 턴을 여는 유일한 자리가 그 `Ready` 를 요구한다(`backend/codex/transport.rs` 의 `record_session_id` · `take_turn_locked`). 회귀망도 둘이다 — 순서 자체(`the_session_id_is_recorded_before_the_gate_opens`)와 게이트의 반대쪽 절반(`nothing_is_sent_before_the_link_is_ready_even_when_the_thread_id_is_known`). ★한때 여기 적혀 있던 「`turn/start` 라는 이름이 저장소 코드·문서에 하나도 없다」는 낡은 서술이다★ — `backend/codex/protocol.rs` 에 있다.
  - ★**persist 반쪽 = 첫 제출 영속으로 옮겨졌다(2026-09-24 · ADR-0226 — 아래 옛 서술은 A2 이전 기록이다)**★: 위 기록 호출은 이제 래치에 건네는 것이고, 영속은 첫 사용자 턴이 나가기 직전 래치의 commit 이 한다 — 첫 턴 전 영속이 codex 터미널 밖에서 구조로 선다(codex 터미널은 id 가 락 회수로만 와 제출과 회수 중 늦은 쪽에 영속된다 — ADR-0226 결정 11). 디스크 저장 실패를 삼키는 것은 그대로다 — 「디스크에 남았다」는 여전히 보장되지 않는다.
  - (옛 서술 — A2 이전) **persist 반쪽 = 여전히 미충족(정직 표기).** 게이트가 실제로 서는 조건은 「기록 **호출이 돌아왔다**」이고 「디스크에 남았다」가 아니다 — 그 포트는 `()` 를 돌려주고 저장 실패를 자기 안에서 삼킨다. 그래서 저장이 실패해도 턴은 열린다. ★이 반쪽을 순서 반쪽과 함께 「강제됨」으로 적지 말 것★.
- ★**이어받기 갈래는 실 codex 로 재 본 적이 없다 — 아래 실측은 `thread/start` 쪽이다**★. 재고 있는 것은 가짜 app-server 를 띄우는 격리 항목 둘(`backend/codex/mod.rs`): 이어받기 요청이 실제로 나가고 상대가 답한 id 가 기록되나 · 거절당하면 새 스레드로 폴백하지 않나. **미검으로 남은 것 셋** — 실 codex 가 ① 스레드 생성 때와 **다른** `cwd`·정책을 받아들이나 ② `excludeTurns: true` 를 실제로 존중하나 ③ 이어받은 대화가 화면에 어떻게 복원되나(그 갈래가 무손실인가).
- **실측(GUI):** 실 codex app-server 가 스레드 id 를 실제로 건넸고 그 값이 `backend_session_id` 에 앉아 `agents.json` 에 남고 kill 을 넘겨 살아남았다. 성공 로그 한 갈래만 떴고 `epoch` 이 프로필과 일치해 화신 가드가 **우연히 통과한 것이 아니라 인정한 것**임까지 확인됐다(해독 실패 0 · 미반영 0). 음성 대조군 = Codex **Terminal** 프로필은 `null` 그대로. ★**받은 id 는 UUIDv7 이다**★ — claude 쪽 v4 와 버전이 갈리므로 그 칸에 v4 를 가정하지 말 것(수령 경로 doc 이 정본). ★한때 여기 있던 「실 app-server 로 id 를 받아 본 적이 없다」는 낡은 서술이다★. **턴은 안 돌렸다 — 핸드셰이크까지다.**
...
flowchart TD
  ...
  A -->|"원문 그대로 (비-MCP 스폰 — 지시서 없음)"| CLI --> CI
  CI --> VAL --> DELIV
  DELIV -->|"① 배달일 때만"| B
...
- ★**단 지시서(프라이밍)를 받는 모드는 그 둘 중 하나뿐이다**★ — 비-MCP 갈래의 프라이밍 변형(`prompts/agent-priming-cli.md`)은 삭제됐고 `PrimingVariant` 축도 함께 걷혔다(커밋 `2ef6902`, 사용자 결정 2026-09-19). 지금 판정식은 `wants_priming = uses_mail && accepts_mcp_config`(`crates/engram-dashboard-daemon/src/control/mod.rs`)라 **비-MCP 스폰은 지시서를 한 글자도 받지 않는다** — 인가(`mail_allowed`)와 CLI grant 는 그대로 나가는데 교육만 없다. 근거는 ★없는 도구를 설명하는 문서를 주는 것이 거짓말이고 침묵은 아니다★이고, **오늘 그 화살표를 타는 운영 백엔드는 0개다**(claude = MCP 가능 · codex 도 MCP 가능(`accepts_mcp_config=true`), ADR-0209). ★옛 문장 「CliOnly 스폰의 프라이밍엔 `send_message`라는 낱말 자체가 없다」를 되살리지 말 것★ — 그 파일이 없어서 참이 된 것이지 문구를 그렇게 쓴 것이 아니다. 정합 불변식(가르치는 채널 == 쓸 수 있는 채널)은 **아무것도 안 가르쳐서** 성립한다.
...
- **소유권 분할:** transport=master/writer/child/shutdown/job · core=subscribers/replay/seq/status/finalized/drain_handle · session=id/cwd/epoch/continues_conversation/cols/rows + 첫 제출 래치(ADR-0226). ([agent 클래스 구조](#agent-클래스-구조-소유-관계))
