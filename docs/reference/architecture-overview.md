...
- ★**발급받는 백엔드(codex)의 수령은 이제 배선돼 있다 — 「없다」로 적힌 옛 문장을 되살리지 말 것**★ (ADR-0185). codex는 클라이언트 지정을 아예 받지 않으므로(`ThreadStartParams`에 id 계열 속성 0개) 스레드 생성 **응답**에서 받는다. 조립점이 `open_spawn`에 기록 동사 하나(`SessionIdSink`)를 넣어 내려보내고, app-server 통로의 라이터가 `thread/start` 응답을 받은 자리에서 그것을 부른다 — ★`Link::Ready`보다 **먼저**★. 그 동사가 이 spawn의 화신 표식을 들고 `observe_session_id`로 간다. ★**그래도 켜지지 않은 것이 있다 — `thread/resume`**★: 그래서 `can_resume_stored_session`도 `capabilities().session.resume`도 codex 쪽은 그대로 `false`이고, 받은 id는 **적힐 뿐 아직 이어받기에 쓰이지 않는다**. ★**claude `/clear` watcher를 codex의 수령 경로로 오해하지 말 것**★ — `backend::session_id_source`는 **우리가 발급한 기준 sid**(`expected_sid: Uuid`)를 요구하는 파일 폴러이고(`AgentBackend::session_id_source`), codex의 id는 JSON-RPC 응답으로 온다. 그 watcher는 지금도 별도 경로이고, 화신 표식 자리엔 `None`을 넘긴다(그 축을 나르지 않는다).
