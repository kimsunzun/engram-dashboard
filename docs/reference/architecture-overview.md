...
- ★**발급받는 백엔드(codex)는 아직 배선이 없다 — 「지금 이렇게 돈다」로 읽지 말 것**★ (ADR-0185). codex는 클라이언트 지정을 아예 받지 않으므로(`ThreadStartParams`에 id 계열 속성 0개) 스레드 생성 **응답**에서 받아야 하는데, 그 응답을 `observe_session_id`로 넘기는 줄이 없다 — 그 함수의 유일한 생산 호출자는 claude `/clear` watcher다(`daemon/src/lib.rs:288`). ★**그 watcher를 codex의 수령 경로로 오해하지 말 것**★ — `backend::session_id_source`는 **우리가 발급한 기준 sid**(`expected_sid: Uuid`)를 요구하는 파일 폴러이고(`AgentBackend::session_id_source`), codex의 id는 JSON-RPC 응답으로 온다. 즉 수령을 켜는 것은 **새 배선 하나**이고, 세션 축을 쪼개는 일은 그것을 열어 주지 않는다(그 쪼개기는 착지했고 수령은 그대로 없다 — 그 사실이 그 독립성의 증거다).
