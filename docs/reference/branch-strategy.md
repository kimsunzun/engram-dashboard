...
> ★이 문서의 브랜치 네이밍 스킴은 폐기됐다(2026-08-16, 사용자 결정) — 아래 「네이밍」·「Lifecycle」 절을 그대로 따르지 말 것★
> 죽은 것: `wip/aN` 네이밍(비어 있는 가장 낮은 번호를 쓰는 규칙) + "브랜치명은 *내용*이 아니라 *작업 흐름*을 나타낸다"는 전제.
> 현행 정본 = **CLAUDE.md 「구현 실행 규약」** — 브랜치 = **작업 덩어리 이름**(`<타입>/<슬러그>`, master에서 파서 **`--no-ff`**로 통합 후 삭제, master 직접 커밋 금지).
> 결정 기록 = `docs/process/step-log.md` 2026-08-16 항목(옛 결정은 약 1621줄).
> 아래 본문은 **폐기된 접근의 기록**으로만 남긴다 — 지우지도 고쳐 쓰지도 않는다.

**상태:** 폐기(2026-08-16, 사용자 결정) — 아래 네이밍·Lifecycle은 기록용이며 현행 아님. (원 상태: draft — `/review doc` 대기(canonical·master 머지 전 검토 필요). 멀티 에이전트(dashboard1·2…) 협업 컨벤션.)
**적용(폐기됨):** 작업 브랜치 네이밍·lifecycle. 현행 = CLAUDE.md 「구현 실행 규약」. (왜 이 형태였는지·버린 대안 상세는 추후 ADR.)
