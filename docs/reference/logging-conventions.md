...
**상태:** crates(agent/daemon/discovery) de-facto 관행을 명문화. 마스킹 **레이어**는 *목표*가 아니라 **명시적으로 거부된 방향**이다(아래 보안 — ADR-0138. 헬퍼를 호출자가 명시 호출하는 것은 그 거부에 걸리지 않는다). load-bearing 경로 작성·리뷰 시 이 문서가 단일 출처.
...
- **`mask_secrets` 헬퍼는 자동 적용이 아니라 호출자가 명시 호출한다**(`base/src/logging/mod.rs`). 부르는 쪽은 외부 프로세스(claude·codex) 출력과 UI 설정 값을 문장에 싣는 자리들이다(찾는 법 = `rg "mask_secrets\(" -g '*.rs' crates/ src-tauri/` — 정의 한 줄·주석·테스트 문자열이 함께 걸리므로 호출만 세려면 그것들을 뺀다). PTY 텍스트 등 민감 가능 출력을 새로 로깅하게 되면 **호출자가 명시 적용**해야 한다(자동 "경유" 아님). 파일 sink 는 이 헬퍼를 경유하지 않는다.
- **★자를 거면 마스킹이 절단보다 먼저다★** — 먼저 자르면 경계에 걸친 자격증명이 패턴의 길이 수량자 밑으로 잘려 **마스킹을 빠져나간 채** 실린다(`agent/src/backend/codex/{decoder,transport}.rs` 의 `sanitize` 가 그 순서로 고정). 절단은 길이 방어이고 마스킹 뒤에 걸어도 그대로 선다. ★**단 「마스킹 먼저」가 모든 입력에서 안전하다는 뜻은 아니다**★ — 마스킹이 문자열을 줄여 뒤쪽 바이트를 앞머리 창으로 끌어올리는 입력이 있어서, `src-tauri/src/ui_settings.rs` 는 자르지 않고 **값의 모양으로 거르는** 게이트를 대신 뒀다.
