...
전체 개발 흐름의 **정의는 `../../CLAUDE.md`** — 「개발 스텝」(PRD→TRD→모듈 경계→구현+TDD) + 「구현 실행 규약」(코더→리뷰어→QA)에 있다. **여기 다시 적지 않는다(SSoT).** 이 문서는 그 흐름 *위에 문서를 얹는 레이어*다.
...
|---|---|---|
| 리서치 | `/research` | ✅ |
| ADR(채번·양방향·인덱스·lint) | `/adr`(전역 스킬 — 스크립트 내장, 바인딩 = `.claude/skill-bindings/adr.md`) | ✅ |
