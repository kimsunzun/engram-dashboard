...
1. **`on.push` 와 `on.pull_request` 에 트리거 단 `paths` 를 제외 목록 모양으로 둔다.** 첫 항목 `'**'` 가 모든 경로를 넣고, 네 경로를 뺀 뒤(`docs/**` · `CLAUDE.md` · `README.md` · `.claude/**`), **맨 끝에서** `.claude/skill-bindings/qa.md` 하나를 되살린다. 바뀐 파일이 **전부** 빠진 경로 안이면 워크플로가 뜨지 않고, 하나라도 밖이면(되살린 파일 포함) 전과 똑같이 전부 돈다. 초안은 `paths-ignore` 모양이었고 적대 리뷰 뒤 이 모양으로 바꿨다(아래 거부한 대안 첫 항목).
   ...
   - **`paths-ignore` 가 아니라 `paths` 인 이유** — `paths-ignore` 로는 되살리기를 표현할 수 없다. `!` 제외는 `paths` 에서만 먹고, 한 이벤트에 `paths` 와 `paths-ignore` 를 함께 둘 수 없다(근거 ⑤). `'**'` 는 경로 필터가 없을 때의 기본 동작과 같아서(근거 ⑤) 이 모양은 여전히 제외 목록이다 — 모르는 경로는 전부 「돈다」로 떨어진다.
...
- **`paths-ignore` 단독**(초안의 모양 — 네 경로를 그대로 나열) — 빠진 경로 안의 한 파일을 되살릴 수 없다(근거 ⑤). 적대 리뷰가 `qa.md` 를 CI 테스트 입력으로 잡은 뒤(아래 근거) 바꿨다. 기각 근거 강도 = 문서(GitHub 규격).
...
- **GitHub 동작 — 공식 문서로 확인한 것**(2026-09-26 열람):
  ...
  - ⑤ **`paths` 의 제외와 순서** — `paths` 와 `paths-ignore` 는 한 이벤트에 함께 쓸 수 없다 · `!` 제외는 `paths` 에서만 먹는다 · 순서가 뜻이다 — 양성으로 맞은 뒤의 `!` 패턴은 빼고, `!` 로 빠진 뒤의 양성 패턴은 되살린다 · `'**'` 는 "the default behavior when you don't use a path filter" 다. (①과 같은 페이지 — `paths` 절과 필터 패턴 절)
...
- ★**하지 않은 조사 — 성숙 프로젝트 서베이**★ — 지운 할 일(`docs/todo/ci-scope.md`)은 `/research` 로 성숙 프로젝트가 경로 필터와 잡 단위 건너뛰기 중 어느 쪽을 왜 고르는지 조사하는 것을 절차로 적어 두었다. 그 조사는 하지 않았고, 사용자가 그것을 생략하라고 명시한 적은 없다. 그 할 일이 착수 전에 풀라고 적은 두 미지(태그 push 에 경로 필터가 걸리나 · 필수 체크가 「Pending」에 묶이는 함정)는 서베이 대신 GitHub 공식 문서로 답했다(①·④).
...
- ★**미확인 — 이름 바꾸기**★ — 코드 경로의 파일을 빠진 경로로 옮긴(rename) push 를 옛 이름으로도 평가하는지 문서가 말하지 않는다. 새 이름으로만 평가하면 그런 이동만 담은 push 에는 CI 가 뜨지 않고, 옮겨 간 파일에 기대던 빌드·테스트가 깨진 것은 다음 코드 push 에서야 드러난다.
- **재론 트리거**
