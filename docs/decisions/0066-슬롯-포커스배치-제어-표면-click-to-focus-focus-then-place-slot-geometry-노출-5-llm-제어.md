...
- 상태: 확정 · 부분 폐기 by ADR-0067 (결정 2 focus-then-place · 결정 5 크로스-윈도우 place 타깃 → 우클릭 컨텍스트 메뉴) · 부분 폐기 by ADR-0068 (결정 3 geometry {x,y,w,h} 좌표 노출 우선 → 논리 도면 방향·이웃·순서 핸들 우선, 좌표계·실측 픽셀 보류) (2026-07-10, 근거: 사용자 결정 2건(배치=focus-then-place 먼저+드래그 나중 · 포커스 강도 65%) + `/research medium` OSS 서베이(VS Code·JetBrains·tmux·Zellij·i3/sway·wezterm) + Codex 적대 리뷰)
