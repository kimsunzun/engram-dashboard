...
- 상태: 확정 (2026-08-09~11, 근거: 사용자 결정 + 적대 리뷰 3라운드 + 실행 실측) · 부분 폐기 by ADR-0232 (결정 2 매 push 범위에서 문서 경로만 바뀐 push와 PR 제외)
- 관련: ADR-0003(코어 격리) · ADR-0110(메시징 커널 격리) · ADR-0129(네트워크 행 격리 게이트) · ADR-0100(릴리즈 co-location) · ADR-0012(모듈 격리 하네스) · step-log S18.26 · ADR-0230(플랫폼 중립 — 이 windows 단일 러너가 그 원칙 아래의 알려진 갭으로 기록됐다) · Amended by ADR-0232 (결정 2 매 push 범위에서 문서 경로만 바뀐 push와 PR 제외)
