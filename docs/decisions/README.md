...
| [0131](0131-CI-도입-windows-단일-러너-검증-전용-릴리즈는-태그-트리거.md) | CI 도입 — windows 단일 러너 · 검증 전용 · 릴리즈는 태그 트리거 | 확정 (부분 폐기 by ADR-0232: 결정 2 매 push 범위에서 문서 경로만 바뀐 push와 PR 제외) |
...
| [0226](0226-세션-id-는-첫-대화-뒤에만-영속하고-이어받는-화신은-구독-응답으로-로딩을-띄운다.md) | 세션 id 는 첫 대화 뒤에만 영속하고 이어받는 화신은 구독 응답으로 로딩을 띄운다 | 확정 |
...
| [0232](0232-문서-경로만-바뀐-push와-pr은-ci를-띄우지-않는다-트리거-단-경로-필터-제외-목록.md) | 문서 경로만 바뀐 push와 PR은 CI를 띄우지 않는다 — 트리거 단 경로 필터 제외 목록 | 확정 |
