...
| [0022](0022-통합-command-registry-palette-키바인딩.md) | 통합 command registry — palette+키바인딩+LLM+메뉴/트레이 단일 출처(VS Code 모델, 추가 여파 0 지향) | 제안 (부분 폐기 by ADR-0140: 미해결 forks 3건 소진) |
...
| [0140](0140-통합-command-버스-선언은-생산자-옆-배달은-홉마다-같은-3단계-명부는-런타임-등록만.md) | 통합 command 버스 — 선언은 생산자 옆, 배달은 홉마다 같은 3단계, 명부는 런타임 등록만 | 확정 (부분 폐기 by ADR-0144: 주인 토큰 산출) |
| [0140](0140-슬롯-분할-방향-어휘-재정의-가로-위아래-세로-좌우.md) | 슬롯 분할 방향 어휘 재정의 (가로 = 위아래, 세로 = 좌우) | 확정 |
| [0141](0141-명령-발견-인자와-반환-모양을-등록-패킷에-불투명-문자열로-동봉하고-조회를-v1에-포함한다.md) | 명령 발견 — 인자와 반환 모양을 등록 패킷에 불투명 문자열로 동봉하고 조회를 v1에 포함한다 | 확정 (부분 폐기 by ADR-0144: 끊긴 이름의 자취 보존) |
...
| [0142](0142-명령-인자의-모르는-칸-입구는-거절-배선은-관용.md) | 명령 인자의 모르는 칸 — 입구는 거절, 배선은 관용 | 확정 |
| [0142](0142-우편-배달-병렬화-축과-같은-세션-배제-축.md) | 우편 배달 병렬화 축과 같은 세션 배제 축 | 확정 |
| [0143](0143-버전-불일치는-끊지-않고-알린다-능력-광고-시작-시-1회-고지.md) | 버전 불일치는 끊지 않고 알린다 — 능력 광고 + 시작 시 1회 고지 | 확정 |
