...
| [0022](0022-통합-command-registry-palette-키바인딩.md) | 통합 command registry — palette+키바인딩+LLM+메뉴/트레이 단일 출처(VS Code 모델, 추가 여파 0 지향) | 제안 (부분 폐기 by ADR-0149: 미해결 forks 3건 소진) |
...
| [0081](0081-llm-ui-제어-relay-앱데몬-명령-수신-ws-peer-opaque-relay-봉투-tauri-invoke-shim-적용사람-경로-재사용.md) | LLM UI 제어 relay: 앱=데몬 명령 수신 WS peer + opaque relay 봉투 + Tauri invoke-shim 적용(사람 경로 재사용) | 확정 (부분 폐기 by ADR-0149: 결정 1과 2 대체 / ADR-0148: 대상 주소지정 불요 전제) |
...
| [0130](0130-데몬-lib-추가-분리-보류-목적은-net-분리로-달성-응용-층은-소비자-없음.md) | 데몬 lib 추가 분리 보류 — 목적은 net 분리로 달성, 응용 층은 소비자 없음 | 확정 (부분 폐기 by ADR-0145: 분리 판정 기준) |
...
| [0144](0144-명부-주인은-클라이언트가-만든-식별자-끊기면-자취-없이-제거한다.md) | 명부 주인은 클라이언트가 만든 식별자 — 끊기면 자취 없이 제거한다 | 확정 |
...
| [0148](0148-명령-배달의-대상-지목은-데몬이-자기-표로-한다.md) | 명령 배달의 대상 지목은 데몬이 자기 표로 한다 | 확정 (부분 폐기 by ADR-0153: 확실성 분류 미결 항목) |
| [0149](0149-슬롯의-에이전트-부재를-세-상태로-가른다.md) | 슬롯의 에이전트 부재를 세 상태로 가른다 | 확정 |
| [0149](0149-통합-command-버스-선언은-생산자-옆-배달은-홉마다-같은-3단계-명부는-런타임-등록만.md) | 통합 command 버스 — 선언은 생산자 옆, 배달은 홉마다 같은 3단계, 명부는 런타임 등록만 | 확정 (부분 폐기 by ADR-0144: 주인 토큰 산출) |
| [0150](0150-명령-발견-인자와-반환-모양을-등록-패킷에-불투명-문자열로-동봉하고-조회를-v1에-포함한다.md) | 명령 발견 — 인자와 반환 모양을 등록 패킷에 불투명 문자열로 동봉하고 조회를 v1에 포함한다 | 확정 (부분 폐기 by ADR-0144: 끊긴 이름의 자취 보존) |
| [0151](0151-명령-인자의-모르는-칸-입구는-거절-배선은-관용.md) | 명령 인자의 모르는 칸 — 입구는 거절, 배선은 관용 | 확정 |
| [0152](0152-버전-불일치는-끊지-않고-알린다-능력-광고-시작-시-1회-고지.md) | 버전 불일치는 끊지 않고 알린다 — 능력 광고 + 시작 시 1회 고지 | 확정 |
| [0153](0153-재시도-지시는-안전과-쓸모로-고른다-확실성-축은-기준이-아니다.md) | 재시도 지시는 안전과 쓸모로 고른다 — 확실성 축은 기준이 아니다 | 확정 |
