...
| [0126](0126-mcp-실패-시-cli-우회-교육-폐지-입구는-스폰-시-capability로만-갈리고-런타임-폴백은-없다.md) | MCP 실패 시 CLI 우회 교육 폐지 — 입구는 스폰 시 capability로만 갈리고 런타임 폴백은 없다 | 확정 (부분 폐기 by ADR-0128: 결정 3 배선 존치 / ADR-0211: 프라이밍 B 의존 조항 / ADR-0213: 프라이밍 에스컬레이션 pin 제거) |
...
| [0211](0211-지시서는-포인터로-줄이고-계약은-도구-설명문과-engram-help-가-진다.md) | 지시서는 포인터로 줄이고 계약은 도구 설명문과 engram help 가 진다 | 확정 |
| [0212](0212-engram-help-본문을-외부-파일로-빼되-내장-사본을-폴백으로-남긴다.md) | engram help 본문을 외부 파일로 빼되 내장 사본을 폴백으로 남긴다 | 확정 |
| [0213](0213-mcp-도구-이름에-eg-접두-에이전트-내장-도구와의-이름-충돌-차단.md) | MCP 도구 이름에 eg_ 접두 — 에이전트 내장 도구와의 이름 충돌 차단 | 확정 |
