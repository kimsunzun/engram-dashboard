...
| [0208](0208-codex-세션-id-는-훅이-되돌려-보내고-데몬은-빈-칸에만-받는다.md) | codex 세션 id 는 훅이 되돌려 보내고 데몬은 빈 칸에만 받는다 | 확정 (부분 폐기 by ADR-0210: 훅 등록 경계와 훅 진단 합류) |
| [0209](0209-우편-인가는-uses-mail-로-축을-쪼갠다-mcp-가-정식이고-cli-는-미러다.md) | 우편 인가는 uses_mail 로 축을 쪼갠다 — MCP 가 정식이고 CLI 는 미러다 | 확정 |
| [0210](0210-codex-훅은-파일-설치-없이-실행-인자로-등록하고-신뢰는-tui-심사로-얻는다.md) | codex 훅은 파일 설치 없이 실행 인자로 등록하고 신뢰는 TUI 심사로 얻는다 | 확정 |
