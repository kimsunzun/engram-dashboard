...
| [0216](0216-codex-세션-id-는-tui-상태줄에서-읽고-훅은-걷어낸다.md) | codex 세션 id 는 TUI 상태줄에서 읽고 훅은 걷어낸다 | 확정 (부분 폐기 by ADR-0217: 상태줄 회수 채널과 스캐너 대체) |
| [0217](0217-codex-세션-id-는-mcp-호출-메타로-받고-상태줄-스크래핑을-걷어낸다.md) | codex 세션 id 는 MCP 호출 메타로 받고 상태줄 스크래핑을 걷어낸다 | 확정 (부분 폐기 by ADR-0218: 회수 채널을 락 파일 홀더 PID 로 교체) |
| [0218](0218-codex-세션-id-는-락-파일을-쥔-프로세스가-우리-자식인지로-판정한다.md) | codex 세션 id 는 락 파일을 쥔 프로세스가 우리 자식인지로 판정한다 | 확정 |
