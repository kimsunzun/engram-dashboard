...
- 상태: 확정 (2026-07-14, 근거: 정상 실행 중 에이전트 재활성화 → 터미널·JSON 둘 다 Failed+빈 슬롯 유저 실측 + a4aac1a 체인 코드/git 확정) · 부분 폐기 by ADR-0185 (살아남는 상위 결정 중 sid 발급 조항) · 부분 폐기 by ADR-0201 (이어받기 실패를 알아내는 수단) · 부분 폐기 by ADR-0226 (결정 첫 항 중 손잡이 없는 이어받기 요청 및 영향의 Fresh 동사 이름)
- 관련: Supersedes ADR-0077 · Amends ADR-0008 (resume 조기종료 → fresh-fallback 조항 폐지) · Amends ADR-0076 (fallback_fresh 관련 불변식·"fresh-fallback 유효" 문구 폐지) · `agent/manager.rs::activate_profile`/`resume_with_fresh_fallback`/`fallback_fresh` · `daemon/connection_core.rs::SpawnProfile` · CLAUDE.md §5(LLM-우선 제어) · step-log · Amended by ADR-0185 (살아남는 상위 결정 중 sid 발급 조항) · Amended by ADR-0201 (이어받기 실패를 알아내는 수단) · Amended by ADR-0226 (결정 첫 항 중 손잡이 없는 이어받기 요청 및 영향의 Fresh 동사 이름)
