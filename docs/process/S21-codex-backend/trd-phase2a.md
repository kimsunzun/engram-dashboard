...
> 상태: **6판(2026-09-11) — 여덟 ADR + 사용자 결정 셋을 반영한 판. ★구현 진입을 막던 마지막 항목이 ADR-0194 로 닫혔다★.** 승인된 설계 브리핑이 확정한 3단계 중 **둘째 단계의 앞 절반**만 다룬다. Phase 0·1 의 TRD 는 `trd.md` 이고 이 문서가 그것을 대체하지 않는다 — 그쪽은 시점 기록이고 이쪽은 그 다음 단계다(ADR-0185 「영향」의 처분 그대로).
>
> **읽는 법:** 이 문서는 codex 동작에 관한 모든 주장에 **출처 등급**을 붙인다 — `[실측]`(이 PC에서 실제로 돌려 본 것) · `[문서]`(codex 공식 문서·README·상류 소스·이슈를 읽었을 뿐 돌려 보지 않은 것) · `[스키마]`(codex 자신이 내보내는 JSON 스키마·생성 TS 를 읽은 것) · `[미확인]`(어느 쪽도 아닌 것). **등급 없는 codex 주장은 이 문서에 없어야 한다.** 이 저장소 코드에 관한 주장은 등급 대신 `file:line` 을 단다(판독 시점 2026-09-09, 4판 갱신분 2026-09-10, 6판 갱신분 2026-09-11).
>
...
>
> ★**초판·2판과 달리 이 판은 고른 것을 적는다**★ — 1·2 판은 「이 문서는 아무것도 고르지 않는다」로 서 있었고 §5 가 선택지 넷을, §10 이 미결 열여섯을 이고 있었다. 그 사이에 **ADR 일곱이 핵심을 닫았다**: **ADR-0187**(Phase 2 통로 = `codex app-server` — `exec`·SDK 는 실측으로 기각) · **ADR-0188**(권한·승인을 백엔드 중립 축으로, 값 설계는 후속) · **ADR-0189**(app-server 통로 = `backend/codex/` 안의 `AgentTransport` 구현체) · **ADR-0190**(json 모드 입력 큐) · **ADR-0191**(백엔드가 자기 통로를 만들어 넘긴다 — 가르는 switch 는 한 곳뿐) · **ADR-0192**(백엔드 동작의 기본값을 claude 와 맞춘다 — 권한값 하나 · 죽은 뒤 재연결 안 함) · **ADR-0193**(큐 해제 판정의 주인을 각 통로 구현체로 — ADR-0190 결정 4 의 개정) · ★**ADR-0194**★(터미널이냐 JSON이냐는 **백엔드 중립 축 하나**로 둔다 — 밖으로 드러나는 것은 에이전트 **종류**와 이 **모드 축** 둘뿐이다). ★**그리고 사용자 결정 셋이 5판에서 더 닫았다**★ — 입력 영수증은 하나로 퉁친다(§4-10 · §10-20) · 무기한 쓰기 정지에 강제 종료 감시를 만들지 않는다(§4-9 · §10-21) · 실행 모드 표현을 **구현 진입 앞**에 답하기로 한 것(§10-C). ★**그 셋째가 6판에서 실제로 답을 받았다 — ADR-0194 다**★. ★**닫힌 것은 「누가 닫았나」를 함께 적고, 안 닫힌 것은 열린 채로 §10·§11 에 남는다**★ — 이 문서가 스스로 고르는 것은 여전히 없다(CLAUDE.md 「개발 스텝」의 순서 불변).
>
...
| 항목 | 어디로 | 이유 |
|---|---|---|
| resume 배선 셋 중 **남은 하나 = 수령 배선**(+ `thread/resume`) | **Phase 2b** | ADR-0185 결정 2 의 ①②③ 중 ★**②(세션 축 쪼개기)와 ③(활성화 입구 가드)은 2a 에서 착지했다**★ — `assigns_session_id`(발급) / `can_resume_stored_session`(이어받기) 두 술어와 공용 판정 `backend::can_resume_profile` 이 그 실물이다. 남은 ①은 그대로 2b 다. **첫 게이트(§9)가 스레드 생성 응답의 실제 모양을 확정하기 전에 설계하면 그 실측이 뒤집을 문장을 쓰게 된다** — 그 ADR 자신이 「거기서 어긋나면 결정 2가 흔들린다」로 적는다 |
...
- ★**§4-10 의 입력 큐는 이 계층에 아무것도 더하지 않는다 — 읽지도 않는다**★. 큐 해제 판정의 주인은 **각 통로 구현체**이고 ★**백엔드별 예외가 없다**★(ADR-0193 · §4-10 규율 4). ★**5판이 여기 적은 「claude json 의 해제 조건만 이 계층을 읽는다」는 틀렸다**★ — 그 예외가 서 있던 사실 문장(「claude 는 에코로 **쓰는 순간** 턴 신호를 만든다」)이 거짓이고, 그 반증은 §4-10 규율 4 가 줄로 적는다. **지우는 호출 지점은 그대로 둘이다.**
...
2. ★**입력을 버리지 않는다**★ — 상한을 넘거나 **그 호출 시점에 이미 보낼 길이 없으면** 오류로 돌린다. ★단 **담겨서 `Ok` 가 나간 뒤에** 실패한 것은 그 호출자에게 못 돌려준다 — 아래 영수증 절의 한계 항이 그것이다★. `send_input` 이 이미 `Result` 다(`crates/engram-dashboard-agent/src/transport/mod.rs:45`). ★조용히 버리는 갈래는 기각됐다 — 그것이 T-28 이 기록한 사고 부류다★.
...
4. ★**해제 판정의 주인은 각 통로 구현체다 — ADR-0193 이 그렇게 정했고, ★백엔드별 예외가 없다★**★. ADR-0190 결정 4 는 원래 「턴이 도는가는 **코어가 이미 안다** — 큐는 읽는다」였는데, ★**ADR-0193 이 그 결정을 개정해 판정을 각 구현체 안으로 옮겼다**★. ★**이 문서가 그 결정을 다시 해석하지 않는다 — 사유의 정본은 그 ADR 이고 아래는 결과만 옮긴 것이다**★:
   - ★★**축은 「코어를 읽느냐」가 아니다 — 「보낼 수 있나」 판정과 「진행 중으로 전이」가 **한 락 안**에서 일어나나 하나다**★★. `AgentTransport::send_input(&self, …)` 은 불변 참조라 **동시 호출이 구조적으로 가능하고**(`crates/engram-dashboard-agent/src/transport/mod.rs:41-45` · §4-1 이 같은 사실 위에 선다), 그 둘이 갈려 있으면 두 호출이 **둘 다 idle 을 보고** 각자 턴을 연다 — 어느 표를 읽었는지와 무관하게 겹친다. **check-and-set 을 원자적으로 한다.** ★그 원자성은 **stdin 과 큐를 함께 쥔 객체 안에서만** 지을 수 있고 그 객체가 통로 구현체다 — 판정이 거기 사는 이유가 그것이고, 그래서 백엔드마다 갈릴 자리가 아니다★.
   - ★★**5판은 여기에 claude json 면제를 적었다 — 그 면제는 없어졌다**★★. 5판의 문장은 「claude 는 `input_echo_event` 를 선언해 **쓰는 순간** 턴 신호를 만들어 미관측 창이 없으므로 코어를 읽어도 된다」였다. ★**그 사실 문장이 거짓이고, ADR-0193 은 이미 그 자리가 고쳐진 채로 서 있다**★ — 줄로 적는다:
     - ★**에코는 `send_input` 이 돌아온 *뒤에* 발화한다**★ — `write_input_observed` 는 `self.transport.send_input(…)?` 를 먼저 부르고(`crates/engram-dashboard-agent/src/session.rs:164`) 그 **뒤에** `input_echo_event` → `core.emit` 을 탄다(`:175-176`). 그 자리 주석이 스스로 「send_input **성공 후**」라고 적는다(`:168` · `:174`).
     - ★**그래서 claude json 에도 미관측 창이 있다**★ — 쓰기가 나간 시점과 턴 사실이 생기는 시점이 벌어져 있고, 그 창에서 코어가 주는 답은 codex 와 똑같이 idle 이다(`crates/engram-dashboard-messaging/src/busy.rs:177-180` — `turn_fact` 가 `None` 이면 곧장 `false`).
     - ★**그리고 그 창을 없애도 안 닫힌다**★ — 위 첫 항의 동시 호출이 남기 때문이다. **즉 「에코가 있느냐」는 이 축의 판정 근거가 될 수 없다.**
   - ★**새 락을 만들지 않는다**★ — §4-1 이 이미 요구하는 구현체 안 뮤텍스가 이 전이를 함께 진다. ★그리고 **그 락을 쥔 채 blocking write 를 하지 않는다**★ — 흘려보내기는 §4-9 의 유계 큐에 **넣고 즉시 놓는** 것까지다(ADR-0006 락 순서: 락 보유 중 외부 호출 금지).
   - ★**codex 쪽 사실은 그대로 남고, 바뀐 것은 claude 가 그 반례가 아니라는 것뿐이다**★ — codex 는 `input_echo_event` 를 선언하지 않아 trait 기본값 `None` 을 타고(`crates/engram-dashboard-agent/src/backend/mod.rs:209-211`) 첫 알림 도착까지 미관측이다. 그 결론(**보낸 그 자리에서** 진행 중으로 넘어가는 자기 턴 기계 — §3)이 이제 **두 백엔드 공통의 모양**이다.
   - ★**`AgentManager::turns()` 를 큐가 부르지 않는다**★(`crates/engram-dashboard-agent/src/manager.rs:537`) — 5판은 claude json 해제 조건이 그 접근자를 읽는다고 적었다. ★**그 소비자가 늘지 않으므로 ADR-0127 쪽 회계도 건드릴 것이 없다**★ — 그 결정이 고정한 것은 턴 관측을 **지우는 호출 지점 둘**이고, 읽는 소비자는 메시징 busy 게이트 하나 그대로다.
   - ★**§7-4 ④가 이 줄로 닫히지 않는다**★ — 여기서 닫은 것은 **큐의 해제 조건**이고, 우편 게이트는 **여전히 코어를 읽는다.** 미관측 창은 큐에서는 없어지고 **우편에서는 그대로 남는다** — 그것을 닫는 것은 §7-4 의 ①②④ 선언이다.
...
- **여전히 원래 호출이 `Err` 로 돌아오는 것 셋 — ★셋 다 「맡겠다」고 답하기 *전에* 갈린다★**(ADR-0190 그대로 — 입력을 버리지 않는다): ① **큐 상한 초과** ② **그 호출 시점에 이미 보낼 길이 없다**(이 화신의 연결이 이미 끊겨 준비가 실패로 확정된 뒤) ③ **준비된 상태라 큐를 거치지 않고 그 호출 안에서 흘려보낸 write 가 실패.** ★`Ok` 로 위장할 자리가 없다★(§4-9 의 「짧은 `Ok` 금지」와 **같은 문장**).
  - ★★**그러니 「준비 실패는 원 호출에 `Err`」를 *담긴 뒤*까지 넓혀 읽지 말 것 — 5판이 그 자리를 두 갈래로 적었다**★★. 초기화 창(약 130ms — L1)에 도착해 **큐에 담긴** 입력은 그 순간 이미 `Ok` 로 돌아갔다. ②가 덮는 것은 **그 호출 시점에 이미 끝나 있던 실패**뿐이고, **담긴 뒤의 준비 실패**는 아래 한계 항으로 간다. ★두 문장을 함께 세워 두면 이 절이 두 가지를 말하게 된다★.
...
- **Phase 1 의 PTY codex 경로는 그대로 산다** — 이 결정은 통로를 **더하는** 것이고 기존 것을 걷지 않는다. 같은 백엔드가 통로 둘을 갖게 되고, 그 둘의 권한 정책이 갈리는 문제는 **ADR-0188** 이 진다. ★단 그 「둘」이 **기동 argv 부터 갈린다** — §8 이 그 행을 진다★. ★★**그 갈림의 입력은 이제 정해져 있다 — `AgentCommand::Codex` 의 모드 칸이다**★★(ADR-0194 · 바로 아래 항).
...
- ★★**그 switch 가 무엇을 보고 통로를 가르나 — ADR-0194 가 그 입력을 확정했다**★★. **`AgentCommand::Codex` 의 모드 칸 하나다**(터미널 / JSON). ★그 축은 새로 세우는 것이 아니라 **이미 있고 이미 공용인 축**이다★ — `crates/engram-dashboard-agent/src/profile.rs:29-37` 의 값이 이미 중립이고(`Terminal` / `StreamJson`), 그 주석이 스스로 「프로세스 기동 방식(= transport)과 프론트 렌더러를 **함께 가른다**(ADR-0044)」고 적으며, ★gemini 백엔드가 이미 그 타입을 빌려 쓴다★(`crates/engram-dashboard-agent/src/backend/gemini/mod.rs:135` · `:179`). 밖으로 나가는 이름도 이미 중립이다(`crates/engram-dashboard-agent/bindings/AgentNewArgs.ts:17` = `output_format?: AgentOutputFormat`). ★**위 「백엔드 이름으로 가르지 않는다」와 충돌하지 않는다**★ — 가르는 것은 백엔드 이름이 아니라 **그 세션이 고른 모드**이고, 「같은 JSON 이 claude 에서는 헤드리스 NDJSON, codex 에서는 app-server」라는 차이는 `backend/<name>/` 안에 남는다(ADR-0004 · ADR-0194).
...
- ★**단 `InputEncoder` 축이 아예 안 늘어난다는 뜻은 아니다**★ — 셋째 태그를 세울지, 아니면 이 통로에서는 인코더를 안 태울지가 §8 의 행으로 남는다. **컴파일러가 그것을 강제한다**: `InputEncoder::submit_sequence()` 의 match 가 exhaustive 다(`backend/mod.rs:443-446`). 태그를 더하면 그 arm 이 요구되고, 안 더하면 그 자리가 안 늘어난다. ★이 갈림은 통로 결정도 ADR-0194 도 닫지 않았다 — §10-7 ②★. ★**ADR-0194 가 바꾼 것은 하나뿐이다 — 가를 *자리*가 생겼다**★: 모드가 `AgentCommand::Codex` 의 칸이라 `input_encoder(c)` 가 모드별로 갈릴 수 있다(그전엔 `extra_args` 에 묻혀 못 갈랐다). **어느 쪽을 고르나는 그대로 열려 있고 메인 몫이다.** ★**`TransportShape` 쪽과 헷갈리지 말 것**★ — 그쪽은 **셋째 변형을 세우는 것으로 닫혔고**(ADR-0189 「영향」 · §5-1), 이쪽은 **세울지 말지가 열려 있다.** 두 enum 이 같은 자리에 있지 않다: shape 는 ADR-0191 뒤로 **선언 축 하나**라 변형을 더해도 요구되는 arm 이 없고(생성 match 가 은퇴한다), 인코더는 여전히 `Copy` 태그로 세션까지 흘러 `submit_sequence()` 의 exhaustive match 를 만난다(`session.rs` 가 그 태그로 `encode`·`input_echo_event` 를 부른다).
...
- ★★**그런데 그 표에 구멍이 하나 있고, ADR-0194 가 그것을 *실재하는* 구멍으로 만들었다**★★ — 모드가 **별 변형이 아니라 `AgentCommand::Codex` 의 칸**으로 실리기 때문이다. 표의 키가 `AgentCommand::Codex { .. }` **변형 하나**라(`backend/mod.rs:779`) **두 모드가 같은 줄로 접히고, 한 모드만 맞아도 초록이다.** ★**그래서 그 구멍을 같은 변경 안에서 메우는 것이 요구사항이다 — 안 메우면 그 테스트가 초록인 채 아무것도 안 잰다**★(§8 이 그 행을 진다). ★선례가 바로 옆에 있다★ — claude 도 모드가 칸인데 표가 **모드별 두 줄**로 갈려 있고(`:763-774`) 샘플이 둘을 다 싣는다(`:704-722` · 그 주석 `:703` 이 「claude 는 모드가 둘이라 둘 다 싣는다(같은 슬롯 — 중복은 허용)」로 적는다). ★★**단 그 둘째 샘플을 강제하는 것은 아무것도 없다**★★ — 커버리지 배열이 **변형 단위**라(`BACKEND_VARIANTS = 3` · `variant_slot` `:684`) 모드를 하나만 실어도 「슬롯 전부 채움」 단언이 통과하고, 같은 파일의 세 관문 설명(`:672-679`)이 세는 것도 **새 variant** 이지 새 모드가 아니다.
...
| 칸 | 2a 에 독자가 필요한가 | 근거 |
|---|---|---|
| `control.resize` | ★**아니오**★ | ★**초판의 실측 서술이 틀렸고, 2판이 그 자리에 **또 다른 틀린 측정**을 놓았다 — 둘 다 고친다**★. ① 초판의 「호출자 하나뿐」이 틀렸다. ② 2판의 「`rg resizePty src/` = 호출 넷 + 선언 둘」도 **그 명령이 돌려주는 것이 아니다** — 그 명령은 **40줄**을 낸다(테스트 목·주석·죽은 `src/lab/` 포함 — 재연결 재전파가 들어오며 24 에서 늘었다). ★열거는 맞고 측정 문장이 틀렸다★. **운영 호출 다섯** = `src/components/slot/TerminalSlot.tsx:118` · `:213` · `:287` · `:305` · `:357`(재연결 전이 — ADR-0195 가 유실시킨 몫의 재발행) · **선언** = `src/api/agentClient.ts:187` · **구현** = `src/api/protocolClient.ts:852`. ★**그래도 결론은 선다 — 다섯이 전부 같은 컴포넌트 안이기 때문이다**★(터미널 슬롯). `RichSlot.tsx` 에는 한 줄도 없다 → codex 를 챗 표면에 그리면 그 경로에 애초에 안 들어온다. **판정 근거는 「호출자 개수」가 아니라 「운영 호출자가 전부 터미널 슬롯 안」이다** |
...
| `session.resume` | **아니오 — 2b** | 2a 는 값을 만들지만 켜는 것은 2b 다. ★**막던 사유가 갈아탔다 — 옛 사유를 근거로 쓰지 말 것**★: 「켜면 발급도 함께 켜진다」던 커플링은 축이 쪼개지며 **없어졌다**(발급 축은 codex 에서 영구 false 다). 지금 막는 것은 **수령 배선과 `thread/resume` 이 없다**는 것 하나이고, 그 배선 커밋이 이 칸과 `can_resume_stored_session` 을 **함께** 켠다 |
...
| ④ | ★**쓰기 시점의 턴-시작 신호**★ | ★**없다**★ | 오늘 claude 에서 그 신호를 만드는 것은 **입력 에코**다 — `write_input_observed` 가 `encoder.input_echo_event(...)` 를 물어 `Some` 이면 `core.emit` 한다(`crates/engram-dashboard-agent/src/session.rs:175-176`). 그 메서드의 **trait 기본값이 `None`** 이라(`backend/mod.rs:209-211`) 선언하지 않은 백엔드는 에코를 하나도 안 낸다. ★**단 「쓰기 시점」은 엄밀히는 「`send_input` 이 돌아온 *뒤*」다**★ — 호출 순서가 `:164`(send_input) → `:175-176`(emit)이고 그 자리 주석이 스스로 「성공 **후**」라 적는다(`:168`·`:174`). 우편 게이트에는 그 차이가 거칠어서 안 보이지만 ★**큐 해제 판정에서는 결정적이라 §4-10 규율 4 가 그 위에서 claude 면제를 걷었다**★ |
...
★**같은 fail-open 이 §4-10 의 입력 큐도 때린다 — 그쪽은 이 절을 안 기다리고 스스로 닫았다**★. 큐의 해제 조건을 코어의 사실 표에서 읽으면 이 창에서 `turn/start` 두 개가 겹치므로, ★**큐의 해제 판정을 통로 구현체 자신의 턴 기계로 못 박았다 — 백엔드별 예외 없이**★(ADR-0193 · §4-10 규율 4). ★그러나 그것이 이 절을 닫지는 않는다★ — 우편 게이트(`busy.rs`)는 **여전히 코어를 읽고**, 그 독자는 큐가 아니라 메시징 커널이다. **큐에서는 닫히고 우편에서는 안 닫힌다** — 우편 쪽을 닫는 것은 아래 ①②④ 선언이다.
...
| 턴 | ✓ (기계만) | **통로 구현체**. ★**사실 계층(`turn.rs`)은 안 건드린다**★ — 코어 호출 지점을 늘리지 않는다(ADR-0127 결정 5). ★**큐는 그 계층을 읽지도 않는다 — 백엔드별 예외 없이 그렇다**★(ADR-0193 · §4-10 규율 4). 해제 판정은 **구현체 자신의 턴 기계**이고, 판정과 전이가 한 락 안이다 |
...
| ★**json 모드 입력 큐**★ — 「지금 보낼 수 있나」 축 + 순서 보존 흘리기 | 쓰기 창구 앞(json 모드 **공통**) | ★**codex 전용이 아니다 — claude json 도 탄다**★(ADR-0190 · §4-10). ★**해제 판정의 주인은 각 통로 구현체 — 백엔드별 예외가 없다**★(**ADR-0193** — ADR-0190 결정 4 의 개정). ★**코어의 사실 표를 읽지 않는다**★: `AgentManager::turns()`(`crates/engram-dashboard-agent/src/manager.rs:537`)를 큐가 부르는 자리는 **claude json 쪽에도 없다.** ★판정과 「진행 중으로 전이」는 **한 락 안**이다★ — `send_input(&self)` 이라 동시 호출이 구조적으로 가능해 그 둘이 갈리면 **읽은 표가 무엇이든** 턴이 겹친다(§4-10 규율 4) |
...
| ★**`AgentCommand::Codex` 의 모드 칸**★ — `#[serde(default)]`, 기본 = 터미널 | `crates/engram-dashboard-agent/src/profile.rs:66-69`(오늘 `extra_args` 하나) | ★**ADR-0194 가 확정했다 — 더 이상 매달린 행이 아니다**★. **칸이고, 별 변형이 아니고, `extra_args` 로 나르지 않는다.** ★**디스크 계약은 안 바뀐다**★ — `#[serde(default)]` 라 옛 프로필이 그대로 흡수되고(형제 `Claude.output_format` 이 그 선례이며 그 doc 이 `:62-65` 에서 「칸을 더할 땐 `#[serde(default)]` 를 달면 옛 프로필이 그대로 흡수된다」로 그 방법을 이미 적어 둔다), **저장 파일 버전도 안 올린다 · 마이그레이션도 없다.** ★대가 = 선언 표 트립와이어가 이 칸을 안 본다 — 같은 변경 안에서 메운다★(아래 `:779` 행 · §5-5) |
### 고치는 것

| 파일:줄 | 무엇 | 절 |
|---|---|---|
| `backend/codex/mod.rs` — `assigns_session_id()` / `can_resume_stored_session()` | ★**완료(2a)**★ — 옛 `needs_session()` 한 술어를 두 축으로 쪼개고 doc 을 `// ADR-0185` 앵커와 함께 고쳤다. 「호출자가 세션 id 를 정할 수 없다」는 사실은 **발급 축**에만 남고, 복원 성립은 별개 축이 답한다 | ADR-0185 「영향」 |
...
| 같은 파일 — `capabilities()` 의 `session.resume` | ★**2b**★. 2a 는 값을 안 뒤집는다. ★사유는 위 §7-2 표와 같다 — 옛 커플링이 아니라 **수령 배선 부재**다★ | §7-2 |
...
| `backend/codex/mod.rs` | ★**옛 판의 「넷을 하나도 선언하지 않는다」는 지금 거짓이다**★ — `transport_shape` · `input_encoder` · `turn_classifier` · `output_decoder` 를 **전부 선언한다**(실측 2026-09-14). 그 서술은 이 선언들이 들어오기 전에 쓴 것이고, ★바로 아래 행이 이미 `output_decoder` 선언을 전제로 서 있어 표 안에서 서로 어긋나 있었다★. 세션 축 둘(`assigns_session_id`·`can_resume_stored_session`)도 여기서 선언한다 — 2a 가 옛 `needs_session` 한 술어를 쪼갠 결과다 | §3 · §5-5 · §6 · §7-4 |
...
| ★**`backend/codex/mod.rs:90-137` — `build_spec`**★ | ★**새 행. 3판이 빠뜨렸다**★ — 오늘 이 함수는 **무조건 대화형 argv** 를 만든다(`--cd <cwd> -s workspace-write -a on-request` + 패스스루, 상수 `:43-51`). app-server 를 띄우려면 argv 가 ★**`app-server --stdio` 축으로 갈려야 한다**★ — 실측이 뜬 형태가 그것이고 정책은 argv 가 아니라 `thread/start` params 로 갔다(§2 L4 · §10-5). ★★**그 갈림의 입력이 6판에서 생겼다 — 바로 위 모드 칸이다**★★(ADR-0194): `build_spec` 이 그 값을 읽어 argv 를 가른다. ★5판이 이 행에 달아 둔 「§10-7 ① 에 매달림」은 **없어졌다**★. **프로세스 cwd 는 `CommandSpec.cwd` 가 그대로 지고, 워크스페이스 폴더는 그 params 의 `cwd` 로 간다.** ★`cmd.exe /c` 감싸기는 그대로 필요하다★(shim — L15). ★**그리고 그 감싸기가 남기는 `%VAR%` 확장 위험이 이 모드에서 *줄어든다* — §8 이 4판에서 그것을 안 적었다**★: 오늘 argv 로 실려 cmd 를 지나던 **워크스페이스 경로**(`--cd <cwd>`)가 app-server 모드에서는 **argv 를 떠나 `thread/start` params 로 가기 때문**이다(L4). ★**없어지는 것은 아니다**★ — 패스스루 `extra_args` 는 여전히 argv 를 타고 cmd 를 지난다. 그리고 그 확장이 우리 값에서 실제로 무엇을 하는지는 ★`[미확인]`★ 이다(재 본 적이 없다) | §5-1 · §10-5 · **ADR-0194** |
| ★**`crates/engram-dashboard-agent/src/profile.rs:29-37` — 실행 모드 타입 이름을 중립으로**★ | `ClaudeOutputFormat` → 중립 이름(★claude 쪽도 함께 — 사용자 결정 · **ADR-0194**★). ★**값도 wire 표기도 한 글자 안 바뀐다**★ — 변형은 `Terminal`/`StreamJson` 그대로, 칸 이름도 `output_format` 그대로다 ⇒ ★**디스크 계약 불변 · 마이그레이션 없음 · 저장 파일 버전 안 올림**★. ★**바뀌는 것은 식별자뿐인데 그 식별자가 넓다**★ — `crates/` 아래 **22 파일 85줄**(실측 2026-09-11 · `src-tauri/src/` 는 0줄). ★기계가 잡아 주는 자리가 하나 있다★ — 명령 선언 어휘 핀(`crates/engram-dashboard-agent/src/commands.rs:1569` `the_declared_vocabularies_are_pinned_to_the_core_ones`, 그 doc 이 `:62-65`)이 `AgentOutputFormat` 과 코어 타입을 **변형·wire 표기까지** 묶어 둔다. ★**단 wire 미러까지 이름을 따라가나는 안 정해졌다 — §10-22**★ | ★**ADR-0194**★ · §5-1 |
| ★**`crates/engram-dashboard-agent/src/manager.rs:1940-1976` — 통로 선택 단위 테스트 둘**★ | ★★**「그대로 초록이면 된다」가 아니라 *옮겨야* 한다**★★ — 이 둘(`select_transport_stdio_ndjson_shape_picks_structured_pipe` `:1940` · `select_transport_pty_shape_picks_pty` `:1961`)은 **`select_transport(TransportShape::…, …)` 를 이름으로 직접 부른다.** ADR-0191 이 그 shape-keyed 선택자를 은퇴시키므로 **그 호출 형태가 남아 있을 수 없다.** ★단언 자체는 버리지 않는다★ — 재는 것이 「터미널 통로 = `terminal_bytes` + `resize` · 파이프 통로 = `structured` + `!resize`」라 그 값은 그대로 유효하다. **옮길 곳 = 백엔드가 만들어 넘긴 통로의 `capabilities()` 를 재는 형태**(§5-1 · §5-5 의 세 문장). ★**5판 §9-3 이 이 자리를 「기존 하네스가 판정 한 건 안 바뀌고 초록인 것」으로 적어 둔 것은 불가능한 요구였다**★ | §9-3 · ADR-0191 |
| ★**`crates/engram-dashboard-agent/src/backend/mod.rs:366-370` · `crates/engram-dashboard-agent/src/manager.rs:66-77` — shape 선언 주석 둘**★ | ★**둘 다 ADR-0191 뒤로 거짓이 된다 — 같은 변경에서 고친다**★. `:366` 은 shape 를 「`manager::select_transport` 의 **입력**」으로 적고, `:368-370` 은 「그래서 **조립점이 이 값을 보고** `structured` output caps 를 주입한다」로 적는다. ★둘 다 아니다★ — shape 는 **선언 축 하나**이고 caps 는 **그 백엔드의 통로 생성 코드가 나란히 주입한다**(§5-5 의 세 문장). 조립점 쪽 doc(`manager.rs:66-77`)이 같은 인과를 한 번 더 적으므로 함께 든다. ★**방치하면 다음 세션이 「shape → caps」 인과를 주석에서 읽고 되살린다 — 4판이 실제로 그렇게 틀렸다**★(§5-1) | §5-5 · ADR-0191 |
...
| 같은 파일 **`:779`** | ★**선언 표 트립와이어**★ — `expected_codec_axis` 의 codex 행이 오늘 `(InputEncoder::Raw, false, TransportShape::Pty)` 다. app-server 모드는 **decoder 를 갖고 통로 모양도 갈린다** = 둘째·셋째 칸이 함께 뒤집힌다(shape 단언은 `:800-802`). ★**같은 커밋에서 갱신하지 않으면 `codec_axis_is_consciously_declared_for_every_backend` 가 빨개진다**★. ★★**그리고 모드가 칸이라(ADR-0194) 이 표가 그것을 *안 본다* — 같은 변경 안에서 고치지 않으면 이 테스트는 초록인 채 아무것도 안 잰다**★★. 고칠 것 둘이고 ★**둘째를 빠뜨리면 첫째가 무의미해진다**★: ① `expected_codec_axis` 의 codex arm 을 **모드별 두 줄**로 가른다(claude 선례 = 같은 파일 `:763-774`) ② ★**`mail_eligibility_samples()` 에 app-server 모드 샘플을 더한다**★(`:704-722` · claude 가 둘째 모드를 `:717-720` 에서 `push` 하는 것이 그 선례이고 주석 `:703` 이 그렇게 적는다) — ★**그 둘째를 강제하는 관문이 없다**★: 커버리지 배열이 **변형 단위**라(`BACKEND_VARIANTS = 3` · `variant_slot` `:684`) 모드를 하나만 실어도 「슬롯 전부 채움」이 통과하고, 같은 파일의 세 관문 설명(`:672-679`)이 세는 것도 **새 variant** 이지 새 모드가 아니다 | §5-5 · **ADR-0194** |
...
- **claude 실행 경로** — ADR-0185 「claude 경로는 한 줄도 바뀌지 않는다」. ★**단 그 문장의 범위는 spawn·복원 경로다 — 이번 판에서 둘이 그 밖에서 닿는다**★: ① §4-10 의 입력 큐가 **claude json 의 쓰기 타이밍**을 바꾼다(턴 중 입력이 즉시 안 나간다 — ADR-0190, 사용자 확인받음) ② §10-2 가 claude decoder 의 `MessageDone` 계약을 **테스트로 박는다**(그건 「바꾸는 것」이 아니라 「이미 참인 것을 적는 것」이다) ③ ★**ADR-0194 의 타입 이름 중립화가 claude 쪽 소스 줄을 바꾼다**★ — `backend/claude/mod.rs` 를 포함해 `crates/` 22 파일 85줄(실측 2026-09-11). ★**바뀌는 것은 식별자뿐이고 값·serde 표기·동작은 그대로다**★(§8 의 그 행). ★그 셋을 「한 줄도 안 바뀐다」로 덮지 말 것★.
...
| ★**큐 해제가 코어의 미관측을 idle 로 읽지 않나**★(§4-10 규율 4) — 순수 하네스. 코어의 사실 표를 **비운 채로**(= `turn_fact` 없음 = `busy.rs:177-180` 이 `false` 를 돌려주는 상태) 턴을 하나 열고 둘째 입력을 넣어 **그것이 안 나가는 것**. ★이 단언이 없으면 fail-open 회귀가 「가끔 두 개가 겹친다」로만 나타난다★. ★★**짝 = 동시성 쪽이고 이쪽이 본체다**★★ — `send_input` 을 **두 스레드에서 동시에** 부르고 **턴이 하나만 열리는 것**(판정과 전이가 한 락 안 — §4-10 규율 4). 그 락이 갈리면 같은 겹침이 「가끔」으로 돌아오고 앞 단언만으로는 안 잡힌다. ★★**둘 다 codex 전용이 아니다 — claude json 레인에도 같은 단언을 건다**★★: 에코가 `send_input` **뒤에** 발화하므로(`crates/engram-dashboard-agent/src/session.rs:164` → `:175-176`) 그쪽에도 같은 창과 같은 동시 호출이 있다 | |
...
| ★**claude·shell 경로의 동작 변화가 0 인가**★(§5-1 · ADR-0191) — 조립점 접기의 수락 조건. ★★**그 단언을 「기존 하네스가 그대로 초록」으로 적을 수 없다 — 5판의 이 줄이 틀렸다**★★: 그 둘은 `select_transport(TransportShape::…, …)` 를 **이름으로 직접 부르고**(`crates/engram-dashboard-agent/src/manager.rs:1940-1976`) 그 선택자가 은퇴하므로 **그 호출 형태가 남아 있을 수 없다.** ★그래서 요구는 「초록 유지」가 아니라 **이전**이다★ — 같은 두 판정(터미널 = `terminal_bytes` + `resize` · 파이프 = `structured` + `!resize`)을 **백엔드가 만들어 넘긴 통로의 `capabilities()`** 위에서 다시 세우고 **그 값이 한 건도 안 바뀌는 것**(§8 의 그 행). 짝 둘 = 선언 표 트립와이어가 claude·shell 행에서 그대로 초록인 것 · ★**shape 선언 주석 둘이 함께 갱신된 것**★(`backend/mod.rs:366-370` · `manager.rs:66-77` — 선언 판독) | |
| ★**codex 가 셋째 `transport_shape` 를 실제로 선언하나 — 그리고 그 표가 *모드별로* 재나**★(§5-5) — 선언 표 트립와이어(`backend/mod.rs:800-802`). ★이 단언은 **비-`#[ignore]` 레인**이라 CI 신호를 받는다★(§9-2). 짝 둘: ① `output_decoder` 가 `Some` 인 것(같은 표의 둘째 칸) ② ★★**`mail_eligibility_samples()` 에 app-server 모드 샘플이 실제로 실려 있는 것**★★(`:704-722`) — 모드가 `AgentCommand::Codex` 의 **칸**이라(ADR-0194) 샘플이 하나뿐이면 이 단언이 **터미널 모드만 재고 초록이 된다.** ★커버리지 배열은 **변형 단위**라 그 누락을 안 잡는다★(`BACKEND_VARIANTS = 3` · `variant_slot` `:684`) | |
...
★**1·2 판의 이 절은 「이 문서는 아래 중 어느 것도 고르지 않는다」로 시작했다. 그 문장은 더 이상 참이 아니다**★ — 그 사이에 **ADR 여덟**이 핵심을 닫았고, **사용자 결정**과 **메인 판정**이 몇 개를 더 닫았다. 그래서 이 절은 이제 두 가지를 한다: **닫힌 것은 「누가 닫았나」와 함께 적고**, **안 닫힌 것은 열린 채로 둔다.**
...
| ★**7 ①**★ | ★**통로 실행 모드를 무엇으로 표현하나 — 터미널이냐 JSON이냐**★ | ★**ADR-0194**★(사용자 결정) | ★**백엔드 중립 축 하나로 둔다 — 종류만 구분자다.**★ 밖으로 드러나는 것은 둘뿐이다: ① 에이전트 **종류**(Claude·Codex…) ② 이 **모드 축**. 그 밖은 백엔드마다 다른 이름으로 노출하지 않는다. ★★**그 축은 새로 세우는 것이 아니라 이미 있고 이미 공용이며 이름만 한 백엔드를 달고 있었다**★★ — `crates/engram-dashboard-agent/src/profile.rs:29-37` 의 `ClaudeOutputFormat { Terminal, StreamJson }` 은 **값이 이미 중립**이고 그 주석이 스스로 「프로세스 기동 방식(= transport)과 프론트 렌더러를 **함께 가른다**(ADR-0044)」고 적으며, ★gemini 백엔드가 이미 그 타입을 빌려 쓴다★(`crates/engram-dashboard-agent/src/backend/gemini/mod.rs:135` · `:179`). 밖으로 나가는 이름도 이미 중립이다(`crates/engram-dashboard-agent/bindings/AgentNewArgs.ts:17` = `output_format?: AgentOutputFormat`). **집행 셋:** ① **Rust 타입 이름을 중립으로 바꾼다**(★claude 쪽도 함께★) ② **codex 에 그 칸을 `#[serde(default)]` = 터미널로 더한다** ③ ★**디스크 계약은 안 바뀐다**★ — 칸 이름 `output_format`·값 `Terminal`/`StreamJson` 유지 ⇒ **마이그레이션 없음 · 저장 파일 버전 안 올림.** ★같은 「JSON」이 claude 에서는 헤드리스 NDJSON 스트림이고 codex 에서는 app-server 다 — 그 차이는 `backend/<name>/` 안에 남는다★(ADR-0004 · ADR-0191 의 통로 생성 경로가 **이 값을 읽는다** — §5-1). ★**그래서 §10-C 에서 이 항목이 빠졌다 — 더 이상 구현 진입을 막지 않는다**★. ★남은 대가 하나 = 선언 표 트립와이어가 **칸을 안 본다** — 같은 변경에서 메운다★(§5-5 · §8) |

### 10-B. 열린 것

★**아래는 여덟 ADR 과 사용자 결정 셋에 대고 다시 적은 것이다 — 3판에서 하나(11-a)가 닫혔고, 4판에서 하나(17)가 생겼으며, 5판에서 그 17 이 절반으로 줄었고, ★6판에서 7 ① 이 닫혀 10-A 로 갔고 22 가 새로 열렸다★.**★
...
| **7** | 새로 만드는 것의 **이름·자리** | **메인**(안 보이는 내부) | ★★**①이 닫혔다 — ADR-0194 로 10-A 에 올라갔다**★★(모듈이 어디 사나는 3판에 ADR-0189 가, 통로를 누가 만들어 넘기나는 4판에 ADR-0191 이 이미 닫았다). ★**남는 것은 ② 하나이고 메인 몫이다**★ — `InputEncoder` 에 셋째 태그를 세우나, 아니면 이 통로에서는 인코더를 안 태우나. 강제 지점 = `submit_sequence()` 의 exhaustive match(`backend/mod.rs:443-446`)이고, 세우면 `backend_for_encoder` 표에 줄이 하나 는다(`:296-301`). ★**ADR-0194 가 여기서 바꾼 것은 하나뿐 — 가를 *자리* 가 생겼다**★: 모드가 `AgentCommand::Codex` 의 칸이라 `input_encoder(c)` 가 모드별로 갈릴 수 있다(그전엔 `extra_args` 에 묻혀 못 갈랐다). **어느 쪽을 고르나는 그대로 열려 있다.** ★**그리고 이 항목은 구현 진입을 막지 않는다**★ — §5-3 이 「통로 구현체가 stdin 을 직접 소유하므로 봉투를 인코더가 만들 이유가 없다」를 이미 적어 두어 **기본 갈래가 `Raw` 쪽에 서 있다.** ★`TransportShape` 쪽은 이 항목에 안 든다 — **셋째 변형을 세우는 것으로** 닫혔다★(ADR-0189 「영향」 · §5-1) |
...
| **22** | ★**실행 모드 타입 이름의 중립화가 wire 미러까지 가나**★ | **메인**(안 보이는 내부) | ★**이름을 바꾸는 것은 확정이고(ADR-0194) 그 사정권이 어디까지인지가 안 적혀 있다**★. 그 ADR 이 「밖으로 나가는 이름은 이미 중립이다」의 근거로 든 것은 **명령 인자 표면**(`crates/engram-dashboard-agent/bindings/AgentNewArgs.ts:17` = `AgentOutputFormat`)인데, ★**프로필 wire 미러는 그 표면이 아니다**★ — `crates/engram-dashboard-protocol/src/domain.rs:137` 에 **같은 이름의 독립 타입** `ClaudeOutputFormat` 이 따로 있고(agent 가 protocol 무의존이라 미러를 둔다 — 같은 파일 `:128-131`), 그것이 `crates/engram-dashboard-protocol/bindings/ClaudeOutputFormat.ts` 를 굽고 `AgentCommand.ts`·`AgentSpawnCommand.ts` 가 그것을 import 하며 손으로 쓴 프론트 타입도 같은 이름이다(`src/api/types.ts:92`, 독자 = `src/api/agentClient.ts` · `src/api/protocolClient.ts` · `src/commands/agentCommands.ts`). **갈래 둘:** ① **미러까지 함께 바꾼다** — 생성물 **파일 이름**이 바뀌므로 CI 생성물 sync 게이트가 그 커밋을 함께 요구한다(CLAUDE.md 「CI」) ② **코어만 바꾸고 미러는 옛 이름으로 둔다** — ★대가 = `crates/engram-dashboard-agent/src/commands.rs:62-65` 의 「어휘는 `ClaudeOutputFormat` 과 **변형도 wire 표기도** 같아야 한다」가 **코어에 없는 이름을 가리키게 된다**★. ★★**어느 쪽이든 값·serde 표기를 안 건드리므로 wire 호환은 둘 다 유지된다**★★ — 갈리는 것은 식별자와 생성물 파일 이름뿐이다 |

### 10-C. 남은 질문의 순서

★★**6판에서 맨 앞이 비었다 — 7 ① 이 ADR-0194 로 닫혔다**★★. 5판이 그것을 첫머리로 끌어올린 사유(「7 ① 이 안 정해지면 `build_spec` 이 만들 argv 의 모양이 없다」)는 **답을 받아 없어졌다.** ★**그래서 지금 구현 진입을 막는 항목이 하나도 없다 — 그것이 이 절의 결론이다**★. 아래는 전부 구현과 **나란히** 답해도 되는 것이다.

1. ★**4 — 첫머리**★. 11-a 가 ADR-0191 로 닫히면서 codex 세션의 `output.structured = true` 신고가 확정됐으므로 **프론트는 이미 `rich` 로 간다**(`renderMode.ts:24`). 즉 4 는 「그리나」가 아니라 ★**빈 상태 라벨을 어떻게 하나**★만 남았다.
2. **4 → 6-b ③** — 챗 표면에 그리기로 하면 빈 상태 라벨의 출처가 필요해지고, 그것이 `Capabilities` 쪽이면 어휘가 는다.
3. **15** — 관측 대기라 늦어도 되고 ★**이제 아무것도 막지 않는다**★. 에코를 내기로 하면 그 shape 가 ADR-0189 가 이미 고른 자리에 실린다. ★4·5판이 이 행을 7 ① 과 한 사슬로 묶어 두었는데 **그 사슬이 없어졌다**★.
4. ★**7 ② · 22 는 줄에 안 선다 — 둘 다 메인 몫이다**★. 구현 중에 정하고 **사유를 코드 주석에 남긴다**(7 ② = `InputEncoder` 셋째 태그를 세우나 · 22 = 이름 중립화가 wire 미러까지 가나).

- ★**17 은 어디에도 안 매달리고 구현 전에 답이 있어야 하는 것도 아니다**★. 큐가 서는 순간 「표시 없음」이 기본값으로 돌고, 표시는 그 위에 얹는 변경이다. **단 그것도 선택이라는 것을 사용자가 알고 지나야 한다.**

★**그리고 「지금 물을 수 없는 것」이 여전히 있다**★ — **15** 는 절반이 관측 대기이고(codex 가 유저 메시지를 replay 로 되울리나), **6-b ①②** 는 실측이 아예 없다(추론 델타가 안 왔고 도구 실행 턴을 안 돌렸다). ★고르기 전에 재는 것이 §1 이 세운 순서 그대로다 — 그 재기를 §9-1 첫 게이트가 진다.★ ★**7 ① 이 그 부류가 아니었고, 그래서 6판에 답이 왔다**★ — 재서 나오는 것이 아니라 **고르면 되는 것**이라 관측을 기다릴 이유가 없었다(ADR-0194).
...
**6판에서 — ★ADR 하나가 마지막 차단 항목을 닫고, 3차 적대 리뷰가 잘못 선 문장 셋을 걷었다★:**

- ★★**7 ① 이 닫혔다 — ADR-0194 다. 10-B 에서 10-A 로 옮겼고 §10-C 의 맨 앞이 비었다**★★. ★**그 결정이 고른 것은 「새 축을 만든다」가 아니라 「있는 축의 이름을 바로잡는다」다**★ — 값(`Terminal`/`StreamJson`)도 밖으로 나가는 이름(`AgentOutputFormat`)도 이미 중립이었고 gemini 가 이미 그 타입을 빌려 쓰고 있었다. **그래서 §5-5 와 10-B 7 행이 줄었다** — 재던 갈래 셋(칸이냐 `extra_args` 냐 별 변형이냐)이 하나로 접혔다.
- ★**그 대신 §8 이 늘었다 — 결정이 접합점을 *만들기* 때문이다**★. 새 행 셋: **타입 이름 중립화**(넓다 — `crates/` 22 파일 85줄) · **통로 선택 단위 테스트 이전** · **shape 선언 주석 둘 갱신**. ★뒤 둘은 5판이 각각 「그대로 초록이면 된다」·아예 언급 없음으로 두었던 자리다★.
- ★★**claude json 의 큐 해제 면제가 없어졌다**★★ — 5판은 「claude 는 에코로 **쓰는 순간** 턴 신호를 만들어 미관측 창이 없으므로 코어를 읽어도 된다」로 적었는데 ★**그 사실 문장이 거짓이다**★: 에코는 `send_input` 이 **돌아온 뒤에** 발화하고(`crates/engram-dashboard-agent/src/session.rs:164` → `:175-176` · 주석이 스스로 「성공 **후**」라 적는다 `:168`·`:174`) `send_input(&self)` 은 동시 호출을 허용한다(`crates/engram-dashboard-agent/src/transport/mod.rs:41-45`). ★**축은 「코어를 읽느냐」가 아니라 「판정과 전이가 한 락 안이냐」이고, 그 락은 통로 구현체 안에서만 지을 수 있다 — 백엔드별 예외가 없다**★(ADR-0193). 그 자리를 §3 · §4-10 · §8 · §9-3 넷에서 함께 고쳤다.
- ★**§4-10 의 영수증 서술이 하나로 줄었다**★ — 5판은 「준비 실패는 원 호출에 `Err`」와 「초기화 창에 담긴 입력은 이미 `Ok` 로 돌아갔다」를 **함께** 적어 두 갈래로 읽혔다. ★**`Err` 셋은 전부 「맡겠다」고 답하기 *전에* 갈린다**★로 좁혔고, **담긴 뒤의 실패는 돌아갈 길이 없다**는 한계로만 남겼다(§10-20 그대로 · §11).
- ★**22 가 새로 열렸다**★ — 이름 중립화의 **wire 미러 사정권**. ADR-0194 가 근거로 든 「밖으로 나가는 이름은 이미 중립」은 **명령 인자 표면**의 말이고, 프로필 wire 미러(`protocol::ClaudeOutputFormat` + 그 생성 바인딩 + 손으로 쓴 프론트 타입)는 그 표면이 아니다. ★값·serde 표기는 어느 갈래에서도 안 바뀌므로 **wire 호환은 걸려 있지 않다**★ — 갈리는 것은 식별자와 생성물 파일 이름뿐이다.
...
| ★★**담긴 뒤에 실패한 write 는 그 호출자에게 돌아갈 길이 없다**★★ | ★**알려진 한계**★ — 큐를 앞에 세운 대가이고 **해결하지 않는다**(§10-20 · 사용자 결정) | 오늘은 write 가 **호출 안에서** 일어나 실패가 `Err` 로 그 호출에 돌아간다(`crates/engram-dashboard-agent/src/session.rs:150-156`). 큐가 서면 그 호출은 이미 `Ok` 로 돌아간 뒤다. ★★**그리고 이 한계는 write 실패만이 아니라 「담긴 뒤의 *준비* 실패」도 함께 덮는다**★★ — 초기화 창에 담긴 입력은 그 순간 `Ok` 로 돌아갔으므로, 그 뒤에 준비가 끝내 실패해도 **그 호출자에게 돌아갈 길이 없다.** §4-10 의 「`Err` 셋」이 덮는 것은 **호출 시점에 이미 끝나 있던 실패**뿐이다. ★**보이는 것은 있다**★ — 통로 구현체가 출력 스트림에 `Error{message}` 를 낼 수 있다(§4-10). ★**그러나 그것은 화면·LLM 이 뒤늦게 보는 것이고 호출자도, 이미 성공으로 적힌 배달 관측 레코드(ADR-0088)도 고쳐지지 않는다**★. ★정정 채널을 발명하지 않는다★ |
