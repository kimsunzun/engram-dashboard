...
대상: **codex-cli 0.151.0, 이 PC, 인증된 상태.** 아래 등급은 이 문서 전체에서 그대로 유효하다.
...
| # | 항목 | 갈림 | 매달린 것 |
|---|---|---|---|
| **A** | ★**`agents.json` 하위호환**★ — 가장 큰 위험 | A1 관용 파싱 / A2 스키마 버전 / A3 손실 수용 (조합 가능) | §4-6. **첫 codex 스폰에서 바로 발생한다** |
...
**이 문서가 근거로 삼은 것:** codex-cli 0.151.0 CLI 스파이크(이 PC, 인증됨) · 이 저장소의 코드·주석·ADR 정적 판독(줄 번호는 2026-09-07) · 브리핑 `briefing.html`. **실행하지 않은 것:** 빌드·테스트·앱 기동 — 이 작업은 문서 하나만 쓴다.
