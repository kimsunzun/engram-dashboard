...
Orca의 `sequence` / `delivered_at` / `read`(불리언) 분리 설계는 채택 가치가 높다:
...
- `read`: consumer가 실제 inbox를 소비했는지 별도 표시 — **정정(2026-07-19, 실소스 검증):** 초기 조사는 `read_at`(타임스탬프)로 기록했으나 실제 스키마는 `read INTEGER NOT NULL DEFAULT 0` 불리언 플래그다(`db.ts` @8c1d13dc — 커밋 고정 permalink 검증). 타임스탬프 아님.
