...
- **데몬화 다음(phase4 2단계+)**: 프론트 DaemonClient(WS 클라, AgentClient 구현) + clientFactory 두-모드 토글 + Origin 실측 + #6 반환매핑 — **GUI 실측(cdp) 필요라 사용자 복귀 후**. 이어서 VT-framebuffer/roaming/predictive(모바일)·별건(pty→agent rename 등).
