...
- **겹친 번호 = 옛 대기자를 `Superseded` 로 깨우고 새 것이 승계** — 오늘 동작 보존(`connection.rs:1445-1460`). 회귀망도 이미 있다(`src-tauri/tests/daemon_client_pending.rs`).
  - ★**구현이 이 줄의 뒷부분을 따르지 않았다(2026-09-06, 적대 리뷰 적출) — 승인 대기 중**★. 승계를 그대로 두면 **먼저 나간 요청의 늦은 답장이 나중 요청의 짝으로 배달된다**: 상관 키가 그 번호 하나뿐이라 두 답장을 가를 재료가 wire 에 없고, `route` 는 `pending.take(&tag)` 로만 짝을 찾기 때문이다. 지금 crate 는 **겹치는 순간 그 번호를 은퇴시켜 양쪽을 다 실패시킨다**(옛 대기자 = `Superseded` 그대로, 새 요청 = `TagRetired`). 즉 「옛 대기자를 `Superseded` 로 깨운다」는 지켰고 「새 것이 승계」는 뒤집었다.
  - **되돌리려면 오배달을 받아들여야 한다** — 둘 다 만족하는 형태는 없다. 어느 쪽을 고를지는 사용자 결정이고, 그때 이 줄과 `lib.rs` 헤더 「알려진 한계」를 함께 고친다. 회귀망 = `pending::tests::a_reply_for_a_collided_tag_reaches_nobody`.
