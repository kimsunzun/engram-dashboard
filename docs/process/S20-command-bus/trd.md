...
- ★3판 신설★ **왕복 중 주인 연결이 끊긴 경합이 지금 어떤 코드를 내는가** — 명부에는 살아 있는 등록이 있는데 조회와 발송 사이에 그 연결이 끊긴 경우다. `OWNER_UNAVAILABLE`은 어휘에 남아 있으나 3판부터 **도달 불가**이고(§4-⑦), 이 경합이 그 코드의 **자연스러운 미래 소유자**로 보인다 — 그러나 현 경로가 실제로 무엇을 내는지(§4-④의 `OUTCOME_UNKNOWN`인지, 다른 무엇인지) **실측하지 않았다**. ★이번에 그렇게 바꾸지 않는다★ — 추측을 결정으로 올리지 않는다.
