...
> 상태: **개정 3판(2026-08-17).** 이 판은 **ADR-0144**(명부 주인 = 클라이언트가 만든 식별자 · 연결이 끊기면 자취 없이 제거)를 반영해, 2판이 **tombstone 위에 세운 조항들**을 갈아낸다. ★판을 올린 이유★ — 아래 「부분 정정 — Step 1 실측」이 판을 **안** 올린 근거는 "설계가 뒤집힌 것이 아니라 회계와 서술이 틀렸다"였고, 이번은 **설계가 뒤집혔다**(자취 보존 → 제거 · 주인 키 산출 교체 · 두 오류 구분 포기). 갈린 절 = §0 · §1 · §2-4 ③ · §3-2 · §3-5 · §3-7 · §4-②·④ · §5 · §6 Step 2·3 · §7 · §8 가정 A · §9.
> 이력 — **개정 2판(2026-08-13).** **ADR-0140**(S20 구조 자체)와 **ADR-0141**(발견 경로 3건 — 등록 패킷에 모양 동봉 · 값 읽기 v1 포함 · tombstone 만료 없음)를 반영해 그 셋에 걸려 있던 미확정·미결 항목을 닫았다. ★그중 「tombstone 만료 없음」은 3판이 폐기했다★ — 만료를 어떻게 둘지가 아니라 **자취 자체가 없어진다**(ADR-0144가 ADR-0141 결정 3만 개정 — 나머지 두 조항은 살아 있다).
...
> 앵커: **ADR-0144(명부 주인 = 클라이언트 자작 식별자 · 자취 없는 제거 — 3판의 정본) · ADR-0140(S20 구조 — 주인 토큰 산출만 0144가 개정) · ADR-0141(발견 경로 3건 — 결정 3만 0144가 개정)** · ADR-0146(되돌려 보내는 문자열엔 상한 없음 — 0144의 입구 상한과 **방향이 반대인 다른 축**) · ADR-0142(명령 인자의 모르는 칸) · ADR-0022(방향) · ADR-0055(프론트 레지스트리 골격) · ADR-0064(메뉴 = command id 참조) · ADR-0081(릴레이 형태 — 이 문서는 **확장**이지 번복이 아니다) · ADR-0132 결정 7(후속 순서) · ADR-0129(net 격리) · ADR-0035/0057(레이아웃 권위) · ADR-0003(코어 격리) · ADR-0012(테스트 격리) · CLAUDE.md 「LLM-우선 제어」.
...
★개정(3판)★ **명부의 주인 키 = 클라이언트가 스스로 만들어 첫 인사에 실어 보내는 식별자이고, 연결이 끊기면 그 주인의 등록은 자취 없이 제거된다.** 위 블록 ④의 「명부에 있나」가 묻는 대상이 이것이다. 구판(1·2판): 주인 키를 **연결 id에서 파생**했고 끊긴 이름은 **자취(tombstone)로 남겼다**. 폐기 근거 = 재접속마다 새 연결 id → 새 주인이 되어 덮기가 아니라 **쌓기**가 되고 회수 경로가 없다(ADR-0144). 귀결로 **「그런 명령 없음」과 「주인이 자리 비움」의 구분을 포기한다**(§4-②).
...
- ★2판 추가★ **발견 경로가 왕복 없이 닫힌다.** 등록이 이름과 모양을 함께 나르므로 **프로세스 밖 호출자(LLM·CLI)가 명부 조회 한 번으로 인자를 채울 수 있다** — 소유자에게 되묻는 왕복이 없다(§2-4 ③ · §3-7 개정 요약 ㉤ + 조항 1. ★2판 원문은 「조항 4」를 가리켰으나 3판에서 조항 4는 **제거 조항**이 됐다 — 포인터만 고친 것이고 이 문장의 뜻은 그대로다).
  - ★개정(3판)★ **단 「주인이 꺼져 있어도 모양이 조회된다」는 성립하지 않는다.** 구판(축약 보존): "주인이 꺼져 있어도 마지막으로 등록된 모양이 tombstone에 남아 있다." → 연결이 끊기면 이름과 모양이 **함께 제거된다**(ADR-0144 · §3-7 조항 4). 왕복 제거라는 이득은 **주인이 붙어 있는 동안**에 대해 그대로이고, 꺼진 주인의 명령은 이름조차 조회되지 않는다.
...
| 이름 하나에 주인 여럿 (가진 주인 전부 발송 + 결과 여럿) | ★3판 추가★ ADR-0144 대안 G — **제외가 아니라 유보(미룸)**. 「하나 연결하면 하나 끊는」 방향도 검토 중이라 다중 클라 동시 접속 자체가 미확정이다. 지금 자료구조는 **이름당 주인 하나 · 나중에 등록한 쪽이 이김**이고 그대로 둔다 |
...
- ~~**앱이 꺼지면 이름만 나온다.**~~ ★개정(3판) — **이 세 번째 거부 근거는 무너졌다**★. 구판(축약 보존): "주인이 없으면 `describe`가 답할 수 없어 「지금 못 부르는 명령을 어떻게 부르는지 알고 싶은」 상황에서 모양이 사라진다 · 동봉은 tombstone에 모양이 함께 남는다(만료 없음)." → 자취를 남기지 않기로 했으므로(ADR-0144 · §3-7 조항 4) **꺼진 주인의 명령은 동봉 쪽에서도 이름조차 남지 않는다.** 두 방식이 이 축에서 같아졌다. **채택 판정은 바뀌지 않는다** — 위 두 근거(발견 경로의 실패 모드 · 등록 1회 대 조회 매번)가 각각 독립으로 서고, 이 근거 하나가 빠져도 결론이 뒤집히지 않는다. 「꺼진 주인의 명령을 어떻게 부르는지」가 실제로 필요해지면 **자취를 되살리는 것이 아니라** 연결 목록을 응답에 싣는 쪽으로 푼다(ADR-0144 영향절 — 이번 범위 밖).
...
3. 둘 다 없으면 **오류**(§4-②). ★개정(3판)★ 구판(축약 보존): "`UNKNOWN_COMMAND` / `OWNER_UNAVAILABLE`을 **갈라 낸다**." → **그 구분을 포기했다**(ADR-0144). 명부에 살아 있는 등록이 없으면 `UNKNOWN_COMMAND` 하나다 — 끊긴 주인의 이름은 명부에서 제거되므로 데몬 눈에 「원래 없던 이름」과 같다.
...
**현판:** 아래. 바뀐 것은 ㉠ `OwnerClass` 삭제 ㉡ **델타**와 **연결 해제 무효화**를 계약에 명시 ㉢ **셸이 웹뷰 몫을 대신 등록**한다는 조항 ㉣ 데몬이 빌드 목록을 **조회하지 않는다**는 금지 조항 ★2판 추가★ ㉤ 등록 단위가 **이름에서 `{name, help}`로** 바뀐다(ADR-0141). ★3판 추가(ADR-0144)★ ㉥ 연결 해제 처리가 **자취 남기기에서 제거로** 바뀐다(조항 4) ㉦ 명부 주인 키가 **첫 인사에 실려 온 클라이언트 자작 식별자**가 된다 — 연결 id 파생을 대체한다(조항 5) ㉧ 그래서 **첫 인사 프레임에 칸 하나가 additive로 생기고 입구 검증이 붙는다**(조항 6).
...
//   ★3판(ADR-0144)★ 위 두 variant 의 `owner` 칸은 **데몬이 계속 무시한다** — 명부 주인 키는
...
//   ★3판(ADR-0144)★ `available` 이 지던 일이 없어진다 — 자취를 남기지 않으므로 **명부에 있는
...
//   ★3판 신설(ADR-0144)★ 비밀이 아니다 — 인증은 여전히 데몬 공유 토큰이 한다. 이 값은
//   인가가 아니라 **상관(correlation)** 용이고, 소비처는 이번엔 **명부뿐**이다(조항 7).
```

★**`available` 칸을 남긴다 — 값은 항상 `true` 다.**★ ★3판 확정 — **ADR-0144 문면에 없는 오케스트레이터 판단**★ 0144는 등록 메시지의 `owner` 칸만 「계속 무시」로 명시했고 이 칸은 언급하지 않았다. **같은 논리를 이 칸에 옮긴다.** 구판(축약 보존): "★`available` 칸을 떼는지는 이 판이 정하지 않는다★ — 값이 상수가 되었을 뿐이고, 칸을 떼는 것은 계약 변경이다."

- **근거: 이 조각은 wire 무변경으로 간다.** 칸을 떼면 바인딩 재생성 + 골든 테스트 수정 + 클라 동반 수정이 딸려오는데 **얻는 것이 없다** — 이 칸을 읽는 클라이언트가 아직 없다. ADR-0144가 `owner` 칸에 내린 판정(「계속 무시하고 사유 주석을 남긴다 — 떼는 것도 계약 변경인데 얻는 것이 없다」)과 같은 회계다.
...
- **떼는 것은 나중에 함께 판단한다** — 첫 인사 확장(ADR-0144 결정 1)이 wire를 여는 그 자리에서 본다. 그때는 바인딩·골든이 이미 갱신되므로 지금 이 칸을 떼면 딸려오던 비용이 사라진다.
...
1. **붙을 때 패킷 하나.** 주인은 인증 직후 자기 선언 **전량**을 `RegisterCommands` 한 방으로 보낸다(★2판★ 구판은 "자기 **이름** 전량" — 이제 `{이름 + 모양}` 전량이다). 이름마다 왕복하지 않는다. 재연결마다 재전송하고 중복 owner는 last-wins — ADR-0081 「RegisterRole 재연결 재전송 + last-wins」와 같은 규칙이다.
   - ★3판 — 「인증 직후」가 계약으로 굳었다(ADR-0144 결정 4)★ **등록을 첫 인사에 합치지 않는다.** 「붙는 순간 등록 끝」이라는 목적은 옳지만 한 프레임에 토큰과 목록이 같이 오면 **토큰을 검증하기 전에 목록을 파싱**하게 되어 공격 표면이 는다. 같은 효과를 **인증 통과 직후 별도 등록 메시지**로 대가 없이 얻는다(ADR-0144 거부 대안 E).
...
4. **연결이 끊기면 그 주인의 등록을 제거한다 — 자취를 남기지 않는다.** ★개정(3판, ADR-0144)★ ADR-0081 「연결 cleanup sweep」과 같은 트리거를 쓰는 것은 그대로다.
   - **구판(축약 보존):** "★지우지 않고 **비가용 표시(tombstone)로 남긴다**★ — §4-②가 요구하는 두 오류 구분이 여기 달려 있다. ★2판 확정 — 만료를 두지 않는다(ADR-0141)★: tombstone은 데몬 수명 동안 유지되고 TTL·LRU로 지워지지 않는다 · `help`도 함께 남으므로 주인이 꺼져 있어도 모양은 조회된다."
   ...
   - **제거 지점은 한 곳뿐이다** — 끊김 감지에서 detach로 가는 그 경로. ★**두 번째 제거 지점을 만들지 않는다**★(인과가 갈라진다 — ADR-0144 영향절).
   - **TTL은 여기서도 되살리지 않는다.** ADR-0141 대안 C가 이미 기각했고, 우리가 채택한 계기는 **시간이 아니라 연결 종료라는 사건**이다. 그래서 §4-②가 TTL을 거부하며 든 문제(같은 질문에 시점에 따라 다른 답이 나간다)가 생기지 않는다.
...
6. **★신설(3판) — 첫 인사에서 받는 값이므로 입구 검증을 둔다.★** 길이 상한과 문자셋을 건다. 근거: 이 값은 **인증을 통과하기 전에 들어온다.**
   - ★**ADR-0146과 혼동 금지 — 방향이 반대인 다른 축이다**★. 0146이 상한을 두지 **않기로** 한 것은 **거절·안내 응답으로 나가는** 호출자 문자열이고(읽는 주체가 LLM이라 꼬리를 자르면 다음 행동이 사라진다), 이것은 **인증 전에 들어오는** 값이다. **둘을 한 규칙으로 합치려 하면 한쪽이 깨진다.**
...
### ② 미지 명령 vs 주인 부재 — ★3판에서 이 구분을 포기했다(ADR-0144)★
...
- **TTL은 여전히 거부다 — 근거가 살아 있다.** 시간 만료를 두면 **같은 질문에 시점에 따라 다른 답이 나간다**(호출자 입장에서 재시도가 의미 있는지가 시계에 달린다). 3판이 채택한 계기는 시간이 아니라 **연결 종료라는 사건**이므로 이 문제가 생기지 않는다. 구판(축약 보존): "보존 기간 미확정(데몬 수명 전체 / TTL) → ★ADR-0141로 닫힘: 만료 없음★ · dedup store와 같은 축으로 보던 1판의 읽기가 틀렸다 — dedup는 *진행 중 왕복*이라 유한하지만 명부는 *어휘*라 유한하지 않다."
- ★**「바운드 대신」 논리가 실제로는 서지 않았다 — 이것이 3판의 계기다**★ 구판(축약 보존): "이름 집합은 빌드가 아는 어휘 크기로 제한되고 재등록이 last-wins라 무한 성장하지 않는다." **틀렸다** — 주인 키가 연결 id 파생이라 재접속이 last-wins로 덮이지 않고 **쌓였고**, 상한은 **이름 개수**를 세며 자취도 그 몫을 먹는다. 만료도 회수 경로도 없어 명부가 영구히 차고 **데몬 재시작만이 해소**였다(ADR-0144 맥락).
...
★**단 `OWNER_UNAVAILABLE`을 오류 어휘에서 떼지는 않는다 — 이 절의 판정이 그것을 내지 않을 뿐이다.**★ ★3판 확정 — **ADR-0144 문면에 없는 오케스트레이터 판단**★ 0144는 두 판정의 구분을 포기한다고만 정했고 코드의 존속은 언급하지 않았다. 어휘는 §4-⑦이 들고 있고 **거기서 떼지 않는다** — 그 절에 근거를 적었다. 이 절과 §4-④ 표에서 그 코드가 사라진 것은 **생산 경로가 사라졌다**는 뜻이고, 어휘에서 사라졌다는 뜻이 아니다.

★**「여기가 유일한 상한 자리」 같은 문장을 쓰지 않는다**★(ADR-0146 결정 4) — 이 절의 오류 문구에는 **호출자가 보낸 이름이 그대로 실린다.** 그 되돌림에는 길이 상한을 두지 않는다(ADR-0146 결정 1 — 읽는 주체가 LLM이라 꼬리를 자르면 다음 행동이 사라진다). **§3-7 조항 6의 입구 상한과 방향이 반대인 다른 축이다.**
...
★**`OWNER_UNAVAILABLE`을 이 집합에서 떼지 않는다 — 단 3판부터 도달 불가다.**★ ★3판 확정 — **ADR-0144 문면에 없는 오케스트레이터 판단**★ 0144는 두 판정의 구분 포기만 정했고 이 코드의 존속은 언급하지 않았다.
...
- ★**개정(2026-08-18) — 그 경합은 ADR-0148이 가져갔고 답은 `OUTCOME_UNKNOWN`이다.**★ 위 「자연스러운 미래 소유자」는 **더 이상 예정이 아니다**(§9의 해당 미확인도 그 결정으로 닫혔다). `OWNER_UNAVAILABLE`은 **도달 불가인 채로 남고** 이 절의 나머지(어휘에서 떼지 않는 근거)는 그대로다.
...
| `engram-dashboard-daemon` | `mail.*` 선언 + `make_table` · 주인 명부(토큰 → 이름 — ★2판★ 각 이름의 `help` 문자열을 **불투명하게** 보관한다. 파싱·검증·분기 금지, 자료형은 `String` 고정: §3-7 하드 제약) · 라우팅 표(ADR-0081 형태) · `connection_core.rs`의 **`dispatch`**(그 함수의 `&dyn OutboundSink` 인자)에 새 variant arm — ★줄 번호를 박지 않는다★(구판이 적어 둔 `:582`/`:586`은 낡았다. 함수명·인자 타입으로 찾을 것)<br>★개정(3판, ADR-0144)★ 구판(축약 보존): 주인 명부가 "**tombstone 포함**". → **자취 없음**(연결 종료 시 제거). 그리고 ㉠ **주인 키가 연결이 첫 인사에서 받아 보관한 클라이언트 자작 식별자**가 된다(연결 id 파생 대체 — 안 보낸 연결은 파생 유지, fail-open) ㉡ **연결별 상태에 그 식별자 칸이 하나 생긴다** ㉢ **끊김 처리는 detach 한 곳만 바꾼다**(자취 남기기 → 제거. 두 번째 제거 지점 금지) |
| `src-tauri` | `window`/`tab`/`slot` 선언 + `make_table` · **ADR-0081이 요구한 공유 적용 서비스**(오늘 없음) · `daemon_client` 인바운드 수신기(오늘 없음 — `connection.rs:805`에 `Request` 갈래 추가) · 웹뷰 몫 대리 등록<br>★3판 추가(ADR-0144)★ **자기 식별자를 만들어 보관하고 첫 인사에 실어 보낸다**(`src-tauri/src/daemon_client/connection.rs`) · 등록은 첫 인사에 합치지 않고 **인증 통과 직후** 별도 메시지로 보낸다(§3-7 조항 1) |
| `src/api/`(웹 클라이언트 transport) | ★3판 신설(ADR-0144)★ **자기 식별자를 만들어 보관하고 첫 인사에 실어 보낸다**(`src/api/wsTransport.ts`). ★**클라 양쪽을 함께 고친다**★ — 오늘 웹과 데스크톱 셸이 **같은 첫 인사 값**을 보내 데몬이 그 둘조차 구분하지 못한다. 한쪽만 고치면 어긋난다 |
...
| `engram-dashboard-net` | ★**개정(3판) — 변경 없음이 아니다**★ 구판: "★변경 없음★". → **첫 인사(핸드셰이크) 프레임이 net 소유**이므로(`crates/engram-dashboard-net/src/auth.rs` `AuthFrame::Auth`, 오늘 `{token, protocol_version}` 두 필드 · 디코드는 `ws.rs`) **거기 칸 하나가 additive로 붙고 그 프레임의 골든 테스트를 같은 커밋에서 갱신한다**(ADR-0144 — 빨개지는 것이 의도된 신호). 값을 데몬 쪽 연결 상태로 넘기는 통로도 한 칸 필요하다. **명령 어휘는 여전히 net에 안 들어온다** — 들어오는 것은 뜻을 모르는 문자열 한 칸이다. ★**net 경계·격리 게이트의 정본은 그 crate `src/lib.rs` 헤더다**★ — 이 칸이 그 헤더의 불변식에 걸리는지는 착수 시 그 헤더를 읽고 판정한다(여기서 대신 세지 않는다) |
...
5. ★신설(3판, ADR-0144)★ **셸이 자기 식별자를 만들어 보관하고 첫 인사에 실어 보낸다** — 등록은 **첫 인사에 합치지 않고 인증 통과 직후** 별도 메시지로 보낸다(§3-7 조항 1·5·6). **웹 클라이언트(`src/api/wsTransport.ts`)를 같은 배치에서 함께 고친다** — 한쪽만 고치면 어긋난다. 첫 인사 프레임의 골든 테스트를 같은 커밋에서 갱신한다(§5 net 행).
...
★개정(3판, ADR-0144)★ **그 「주인 토큰」이 이제 클라이언트가 만들어 보낸 문자열이다** — 데몬이 만들지 않고 받는다. **C1(데몬은 셸을 모른다) 형식 보존은 그대로다:** 이 값도 **상관(correlation)용 불투명 문자열**이고 데몬은 그것으로 명부 칸을 가릴 뿐이다. 인가는 여전히 공유 토큰이 하고, **컴파일에 들어가는 UI 타입은 여전히 0**이다. ★**소비처 울타리**★ — 이 식별자는 **명부에만** 쓴다(조회 명령·이벤트 노출·화면 표시·인증 판단 금지 — §3-7 조항 7). 즉 데몬이 「어느 클라이언트인가」를 **말할 수 있게 되지는 않는다.**
...
★단서(3판)★ 위 §4-②·④ 항목의 **손실 폭이 줄었다** — 3판이 이미 `OWNER_UNAVAILABLE` 판정을 포기했으므로(ADR-0144), 가정 A를 뒤집어 잃는 것은 「미적용 확정」이 아니라 **명부 자체와 그 위의 `UNKNOWN_COMMAND` 판정**이다.
...
- **결정:** **ADR-0144(3판의 정본 — 명부 주인 = 클라이언트 자작 식별자 · 끊기면 자취 없이 제거 · 두 오류 구분 포기) · ADR-0140(S20 구조 — 주인 토큰 산출만 0144가 개정) · ADR-0141(발견 경로 3건 — 등록 패킷 동봉 · 값 읽기 v1 포함 · ~~tombstone 만료 없음~~ **결정 3은 0144가 개정**)** · ADR-0146(되돌려 보내는 호출자 문자열엔 상한 없음 — 0144의 입구 상한과 **방향이 반대인 다른 축**) · ADR-0142(명령 인자의 모르는 칸 — 입구는 거절, 배선은 관용) · ADR-0022(방향 — 상태 **제안**) · ADR-0055(프론트 레지스트리 골격) · ADR-0064(메뉴 = command id 참조) · ADR-0081(릴레이 형태) · ADR-0132 결정 7(후속 순서) · ADR-0129(net 3층 분리) · ADR-0035/0057(레이아웃 권위) · ADR-0012(테스트 격리) · ADR-0003(코어 격리) · ADR-0110(메시징 커널 무의존)
...
- ~~**tombstone 보존 기간**~~ — ★2판에서 닫혔다(ADR-0141): 만료 없음★ → ★**3판에서 질문 자체가 소멸했다(ADR-0144)**★ **자취를 남기지 않는다** — 연결이 끊기면 제거한다(§3-7 조항 4). 그래서 보존 기간도, TTL도, 그 위에 세운 두 오류 구분도 없다(§4-②). 구판 2판 문장(축약 보존): "만료 없음 — 데몬 수명 동안 유지 · 재등록 시 last-wins."
- ★3판 신설★ **첫 인사 식별자의 입구 검증 값** — ADR-0144는 「길이 상한·문자셋을 **둔다**」까지만 정했고 **구체 값을 정하지 않았다.** 구현 시 확정한다(내부 상수 — §3-7 조항 6). ★ADR-0146의 「상한 없음」과 섞지 말 것★ — 그건 나가는 값이다.
- ★3판 신설★ **클라이언트가 그 식별자를 어디에 보관하나** — ADR-0144는 「스스로 만들어 **보관**한다」까지만 정했다. 프로세스 수명뿐인지 앱 재시작을 가로지르는 영속인지, 웹(`src/api/wsTransport.ts`)과 셸(`src-tauri/src/daemon_client/connection.rs`)이 같은 결을 고르는지가 미정이다. **자취 없는 제거가 쌓임을 이미 막으므로 명부 정확성에 미치는 영향은 좁다** — 갈리면 **두 클라의 행동이 달라지는 것**이 문제다.
- ★3판 신설★ ~~**왕복 중 주인 연결이 끊긴 경합이 지금 어떤 코드를 내는가**~~ — ★**결정으로 닫힘(2026-08-18, ADR-0148 「찢어진 창」 조항)**★ 답은 `OUTCOME_UNKNOWN`이다(조회가 `Available`을 답한 뒤 닿는 길이 사라진 경우). ★**재서 닫은 것이 아니다 — 데몬 전달 구간은 아직 0줄이라 그 구간을 잰 것이 아니다.**★ 고른 근거는 **재시도 축**이다 — `OUTCOME_UNKNOWN`은 같은 `request_id` 재시도를 허가하고(dedup가 흡수한다) 확실한 실패 계열은 그것을 금지하는데, 이 경합에서 주인은 그냥 다시 붙을 수 있으므로 같은 id 재시도가 안전하면서 쓸모도 있다. **받치는 실측은 `route`가 *그 전달 갈래에서* 스스로 만드는 오류 어휘가 단일값(`ErrorCode::OutcomeUnknown`)이라는 것**까지다(★`route` 전체로는 아니다 — `handler_panicked`의 `Internal`과 주인 부재의 `UnknownCommand`가 따로 있다★) — 그 두 자리는 파국 그물이라 「봉투가 이미 선을 탔을 수 있는」 반대 조건이고, `OWNER_UNAVAILABLE`을 실어 보내는 링크의 답장은 `route`가 그대로 통과시키므로 「그 경로에 다른 코드가 없다」는 성립하지 않는다. `OWNER_UNAVAILABLE`은 **여전히 도달 불가**이고(§4-⑦) 되살리지 않는다. 구판: "이 경합이 `OWNER_UNAVAILABLE`의 자연스러운 미래 소유자로 보이나 현 경로가 무엇을 내는지 실측하지 않았다 — 추측을 결정으로 올리지 않는다."
