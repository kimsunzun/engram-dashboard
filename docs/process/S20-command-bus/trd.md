...
> 이력 — **개정 2판(2026-08-13).** **ADR-0149**(S20 구조 자체)와 **ADR-0150**(발견 경로 3건 — 등록 패킷에 모양 동봉 · 값 읽기 v1 포함 · tombstone 만료 없음)를 반영해 그 셋에 걸려 있던 미확정·미결 항목을 닫았다. ★그중 「tombstone 만료 없음」은 3판이 폐기했다★ — 만료를 어떻게 둘지가 아니라 **자취 자체가 없어진다**(ADR-0144가 ADR-0150 결정 3만 개정 — 나머지 두 조항은 살아 있다).
...
> 앵커: **ADR-0144(명부 주인 = 클라이언트 자작 식별자 · 자취 없는 제거 — 3판의 정본) · ADR-0149(S20 구조 — 주인 토큰 산출만 0144가 개정) · ADR-0150(발견 경로 3건 — 결정 3만 0144가 개정)** · ADR-0146(되돌려 보내는 문자열엔 상한 없음 — 0144의 입구 상한과 **방향이 반대인 다른 축**) · ADR-0151(명령 인자의 모르는 칸) · ADR-0022(방향) · ADR-0055(프론트 레지스트리 골격) · ADR-0064(메뉴 = command id 참조) · ADR-0081(릴레이 형태 — 이 문서는 **확장**이지 번복이 아니다) · ADR-0132 결정 7(후속 순서) · ADR-0129(net 격리) · ADR-0035/0057(레이아웃 권위) · ADR-0003(코어 격리) · ADR-0012(테스트 격리) · CLAUDE.md 「LLM-우선 제어」.
...
★개정(2판)★ **등록이 나르는 것 = `{이름 + 모양}`.** 위 블록은 등록 페이로드를 적지 않아 1판이 그것을 「이름만」으로 읽었고(§2-4 ③ 구판 문장), ADR-0150가 그 읽기를 뒤집었다. `모양`은 **불투명 문자열 한 칸(`help`)**이고 데몬은 저장·중계만 한다(§3-7).
...
- ★개정★ **각 프로세스가 자기 표를 채우고 자기가 구현한 것을 `{이름 + 모양}`으로 데몬에 등록한다.** 구판: "자기가 구현한 **이름만** 등록한다 · 데몬은 주인 토큰 → **이름 집합**만 쥔다". 현판에서 데몬이 쥐는 것은 주인 토큰 → `{이름, help}` 집합이고, `help`는 **불투명 문자열**이라 데몬은 여전히 "클라이언트 셸"이라는 구체 개념도 명령의 뜻도 배우지 않는다(ADR-0150 · §3-7).
...
★개정★ **구판 표에는 「값 읽기(조회)를 명령 목록에 담기 — §8 가정 B 미결」 행이 있었다.** ADR-0150로 **포함**이 확정돼 그 행을 삭제한다. 읽기는 이제 비목표가 아니라 v1 범위다(§8 가정 B · §6 Step 3·5).
...
> ★개정★ **crate 이름 = 확정. `engram-dashboard-command`.** (ADR-0149.) 구판: "이름은 **잠정** — 네이밍은 구현 갈림길이라 **사용자 결정 사항**(CLAUDE.md 「개발 스텝」 2)". 확정됐으므로 §5 게이트 정규식도 이 이름으로 박는다(더는 「이름이 바뀌면」 조건절이 붙지 않는다).
...
3. **JSON Schema 문자열** — 매크로가 필드 이름·타입을 구문에서 읽어 그대로 찍는다. ★2판 추가★ **이 문자열이 곧 등록 패킷의 `help`가 된다**(§3-7 · ADR-0150) — Rust 선언은 매크로가 자동으로 채우므로 손으로 적을 것이 없고, TypeScript 선언만 한 줄을 손으로 적는다.
...
★2판 단서★ 위 좁히기는 유지된다 — **디스크의 파생 파일이 발견 표면인 것은 아니다.** 다만 매크로가 찍는 스키마 항목(위 배열의 **원소 하나**)이 직렬화돼 **등록 패킷의 `help`로 실려 나간다**(§2-2 3 · ADR-0150). 즉 같은 내용이 두 경로로 나가되, LLM이 읽는 것은 파일이 아니라 **명부에 등록된 사본**이다.

★**개정 — 닫혔다(ADR-0150)**★: LLM이 **셸·웹뷰 소유 명령의 인자 모양**을 어디서 얻는가 = **등록 패킷에 동봉한다.**
...
// 현판: Read 항목도 v1 목록에 실린다(ADR-0150 — §8 가정 B 채택). Read 는 §4-⑥ dedup 면제.
...
**현판:** 아래. 바뀐 것은 ㉠ `OwnerClass` 삭제 ㉡ **델타**와 **연결 해제 무효화**를 계약에 명시 ㉢ **셸이 웹뷰 몫을 대신 등록**한다는 조항 ㉣ 데몬이 빌드 목록을 **조회하지 않는다**는 금지 조항 ★2판 추가★ ㉤ 등록 단위가 **이름에서 `{name, help}`로** 바뀐다(ADR-0150). ★3판 추가(ADR-0144)★ ㉥ 연결 해제 처리가 **자취 남기기에서 제거로** 바뀐다(조항 4) ㉦ 명부 주인 키가 **첫 인사에 실려 온 클라이언트 자작 식별자**가 된다 — 연결 id 파생을 대체한다(조항 5) ㉧ 그래서 **첫 인사 프레임에 칸 하나가 additive로 생기고 입구 검증이 붙는다**(조항 6).
...
//   1판: owner: OwnerClass 삭제 · 2판: help 추가(ADR-0150)
...
4. **연결이 끊기면 그 주인의 등록을 제거한다 — 자취를 남기지 않는다.** ★개정(3판, ADR-0144)★ ADR-0081 「연결 cleanup sweep」과 같은 트리거를 쓰는 것은 그대로다.
   - **구판(축약 보존):** "★지우지 않고 **비가용 표시(tombstone)로 남긴다**★ — §4-②가 요구하는 두 오류 구분이 여기 달려 있다. ★2판 확정 — 만료를 두지 않는다(ADR-0150)★: tombstone은 데몬 수명 동안 유지되고 TTL·LRU로 지워지지 않는다 · `help`도 함께 남으므로 주인이 꺼져 있어도 모양은 조회된다."
   ...
   - **TTL은 여기서도 되살리지 않는다.** ADR-0150 대안 C가 이미 기각했고, 우리가 채택한 계기는 **시간이 아니라 연결 종료라는 사건**이다. 그래서 §4-②가 TTL을 거부하며 든 문제(같은 질문에 시점에 따라 다른 답이 나간다)가 생기지 않는다.
...
★**하드 제약 — 데몬은 help를 열어보지 않는다**★ (ADR-0150)
...
- **TTL은 여전히 거부다 — 근거가 살아 있다.** 시간 만료를 두면 **같은 질문에 시점에 따라 다른 답이 나간다**(호출자 입장에서 재시도가 의미 있는지가 시계에 달린다). 3판이 채택한 계기는 시간이 아니라 **연결 종료라는 사건**이므로 이 문제가 생기지 않는다. 구판(축약 보존): "보존 기간 미확정(데몬 수명 전체 / TTL) → ★ADR-0150로 닫힘: 만료 없음★ · dedup store와 같은 축으로 보던 1판의 읽기가 틀렸다 — dedup는 *진행 중 왕복*이라 유한하지만 명부는 *어휘*라 유한하지 않다."
...
- **호출자가 보는 것:** 진행 보고가 **없다는 것이 계약이다.** 오래 걸릴 수 있는 호출은 반드시 마감시각(⑥)에 기대야 하고, 중간 상태를 물어보려면 **별도 조회 명령을 부른다.** ★개정★ 구판은 여기에 "그리고 조회는 §8 가정 B로 v1에 없다 — 이 조합의 대가를 여기 적어둔다"를 달았다. 가정 B가 채택되면서(ADR-0150) **그 대가가 사라졌다** — 진행 보고는 여전히 없지만 상태를 물어볼 경로는 v1 안에 있다.
...
- ★2판 추가(ADR-0150 — 가정 B 채택의 귀결)★ **읽기 명령(`Effect::Read`)은 dedup 대상에서 면제한다.** 멱등이라 두 번 실행돼도 상태가 안 바뀌므로 중복 방지가 지킬 것이 없고, 면제하면 조회가 5분 재시도 창의 표를 채우지 않는다. dedup 커밋 규칙(위 항목)은 **쓰기에만** 적용된다. 구판에는 이 규칙이 없었다 — 전 명령이 dedup 대상이었고, 그건 읽기가 v1에 없다는 전제 위였다(§8 가정 B 「뒤집으면 바뀌는 절」이 예고한 항목).
...
- **★이 줄은 이력이다 — 현황이 아니다★** 에이전트 문(HTTP `/control`)의 5동사 match가 **이 Step 착수 시점의 벽이었다**(구판이 적어 둔 `agent.rs:222-236`은 낡았다 — 그 코드가 없으므로 줄 번호로 찾지 말 것). 이 Step이 그 match를 표 조회로 바꿨고, **지금은 이미 표 조회다**(아래 착지 줄).
  - ★착지(2026-08-16, 슬라이스 3)★ **그 match는 삭제됐다 — 위 줄번호로 찾지 말 것.** 현행 입구는 `handle_agent`(`crates/engram-dashboard-daemon/src/control/agent.rs`) 하나이고 `{verb}` → `agent.<verb>` 이름 조립 → `contains` → `check_args`(ADR-0151) → `call` 순으로 간다.
...
1. ★개정★ **`layout.rs`의 `#[tauri::command]` 16개 전부**를 전송 중립 적용 서비스 뒤로 옮기고, 그 옆에서 `window`/`tab`/`slot`을 선언한다. 구판: "**쓰기 12개**(16개 중 **읽기 4개 제외** — §8 가정 B)". 가정 B가 채택되면서(ADR-0150) 읽기 4종(`get_view` `:497` · `list_tabs` `:508` · `list_windows` `:520` · `resolve_spatial` `:530`)이 합류해 **16 = 쓰기 12 + 읽기 4** 전량이 이 Step의 범위다(재실측 2026-08-13).
...
| `window.__engramChat` | `eventBus.ts:118-124`, 5항목 | ★개정★ **5항목 전부 command로 승격 → 핸들 삭제.** 구판: "`set`·`patch`·`reset`만 승격 · `get`(`:119`)·`defaults`(`:123`)는 **읽기**라 §8 가정 B에 걸려 v1 범위 밖 → 핸들을 통째로 못 지운다". 조회가 v1에 들어와(ADR-0150) 그 둘도 command가 된다 |
| `window.__engramCmd` | `eventBus.ts:134-137` | **남긴다.** 이게 화면 표의 로컬 입구다 |

★개정★ **곁문 둘을 v1에서 전부 철거한다.** 구판: "가정 B의 실제 대가가 여기 드러난다 — 「곁문 둘을 없앤다」가 v1에선 「하나 반」이 된다." 가정 B가 채택되면서(ADR-0150) 그 대가가 사라졌고, 이 Step의 이름(「곁문 둘 철거」)이 문자 그대로 성립한다.
...
**남는 17개의 내역**(구판이 안 적었다): `commands/layout.rs` 16개 + `commands/popout.rs` 1개. ★개정★ 구판: "Step 3이 다루는 것은 layout **12개(쓰기)**이고, layout **읽기 4개는 §8 가정 B에**, popout 1개는 미분류로 남는다." 현판: **Step 3이 layout 16개 전부**를 다루고(ADR-0150), 남는 미분류는 **popout 1개뿐**이다 — 이관 시점에 주인을 정한다.
...
★개정(2판)★ **데몬이 배우는 것은 이름 · 주인 토큰 · 불투명 `help` 문자열 한 칸뿐이다.** 구판(축약 보존): "여전히 **이름과 주인 토큰뿐**이다." ADR-0150로 모양이 등록 패킷에 동봉되면서(§1 · §3-7 · §5 daemon 행) 칸이 하나 늘었으므로 구판 문장은 그대로 두면 거짓이다.
...
### 가정 B — 값 읽기(조회)를 v1 명령 목록에 넣는다 ★개정 — 미결에서 채택으로(ADR-0150)★
...
- **이 설계 트랙의 타임라인:** `docs/process/step-log.md:1649`(S20.1 — 선례 조사 → TRD 초안 → 개정 1판의 반전 3건) · `:1657`(S20.2 — 발견 경로 결정 · ADR-0149/0150 · 개정 2판과 적대 리뷰 FIX). **왜**는 ADR, **언제·무엇**은 이 두 절이다.
- **결정:** **ADR-0144(3판의 정본 — 명부 주인 = 클라이언트 자작 식별자 · 끊기면 자취 없이 제거 · 두 오류 구분 포기) · ADR-0149(S20 구조 — 주인 토큰 산출만 0144가 개정) · ADR-0150(발견 경로 3건 — 등록 패킷 동봉 · 값 읽기 v1 포함 · ~~tombstone 만료 없음~~ **결정 3은 0144가 개정**)** · ADR-0146(되돌려 보내는 호출자 문자열엔 상한 없음 — 0144의 입구 상한과 **방향이 반대인 다른 축**) · ADR-0151(명령 인자의 모르는 칸 — 입구는 거절, 배선은 관용) · ADR-0022(방향 — 상태 **제안**) · ADR-0055(프론트 레지스트리 골격) · ADR-0064(메뉴 = command id 참조) · ADR-0081(릴레이 형태) · ADR-0132 결정 7(후속 순서) · ADR-0129(net 3층 분리) · ADR-0035/0057(레이아웃 권위) · ADR-0012(테스트 격리) · ADR-0003(코어 격리) · ADR-0110(메시징 커널 무의존)
...
- ~~**LLM이 셸·웹뷰 소유 명령의 인자 모양을 어디서 얻는가**~~ — ★개정 — 닫혔다(ADR-0150)★ **등록 패킷에 동봉**한다(§2-4 ③ · §3-7). 구판: "등록 패킷은 이름만 나르고 웹뷰 선언은 Rust 파생물에 없다. 후보 둘(스키마 동봉 / `command.describe` 왕복)은 굵은 형태 결정이라 **사용자 결정**." 이 자리에 남는 미확인은 없다.
...
- ~~**tombstone 보존 기간**~~ — ★2판에서 닫혔다(ADR-0150): 만료 없음★ → ★**3판에서 질문 자체가 소멸했다(ADR-0144)**★ **자취를 남기지 않는다** — 연결이 끊기면 제거한다(§3-7 조항 4). 그래서 보존 기간도, TTL도, 그 위에 세운 두 오류 구분도 없다(§4-②). 구판 2판 문장(축약 보존): "만료 없음 — 데몬 수명 동안 유지 · 재등록 시 last-wins."
