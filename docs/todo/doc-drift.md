...
★**`v0.3.0/chore/misc` 를 재사용하지 말 것 — 2026-08-26 실측**★. 그 브랜치는 crate 경계 재편(ADR-0175) **이전**에 갈라져 나와 머지 안 된 커밋 셋(`.claude/skills/` 피드백 적립)을 물고 있다. master 를 끌어오면 대규모 diff 에 더해 **modify/delete 충돌 셋**이 난다 — master 가 `.claude/skills/` 를 통째로 지웠기 때문이다(스킬 정본이 이 저장소 밖으로 옮겨 갔다). ★그 세 커밋을 master 로 어떻게 들일지가 별도 미결이다★. 새 작업은 master 에서 새 브랜치를 판다.
