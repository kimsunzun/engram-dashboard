...
### T-27. `src-tauri` 단위 스위트가 CI에 **통째로는** 안 실린다 (★2026-08-24 정정 — 옛 제목 「로컬·CI 어디서도 실행되지 않는다」에서 로컬 절반은 해소됐고, CI 절반도 내보내기 테스트 8건만큼 열렸다★)
- **상태:** 보류(선재 구조 문제 — S22가 발견한 것이지 만든 것이 아니다). 착수 시 ADR-0038 절차(추측·매직넘버 금지).
...
- **증상(★2026-08-24 정정 — 원문은 아래 인용, 그 아래가 현재 사실★):** 원문 = "① 워크스페이스 회귀 명령이 `--exclude engram-dashboard`로 이 패키지를 뺀다(그 lib 테스트 타깃이 `0xc0000139 STATUS_ENTRYPOINT_NOT_FOUND`로 죽어서 — 실측 2026-08-05). ② CI 백엔드 잡도 **같은 제외**를 쓴다. ③ `cargo build`는 `#[cfg(test)]` 본문을 컴파일하지 않으므로 **컴파일 검사조차 안 된다.** 결과: `src-tauri/src/layout/{types,tree,spatial,manager}.rs`·`output_router.rs`의 테스트는 **작성돼 있으나 아무도 돌리지 않는다.**"
  - **현재 사실:** ①의 괄호 안 사유가 죽었다 — `[lib] test = false` + `[[test]] name = "lib_unit"`(`path = "src/lib.rs"`) + `build.rs`의 `cargo:rustc-link-arg-tests=` 로 Windows manifest가 테스트 exe에도 링크되면서 즉사가 사라졌다(**결정과 거부한 대안 = ADR-0174**). 그 스위트는 `cargo test -p engram-dashboard --test lib_unit` 으로 **로컬에서 돈다**. 제외는 유지되지만 이유가 바뀌었다: 이제는 즉사가 아니라 **그 남은 실패**다. ②는 **부분적으로만 그대로다** — CI는 이 스위트에서 **내보내기 테스트만 이름으로 골라** 돌고(그 덕에 `src-tauri/bindings/` sync 게이트가 섰다), **스위트 전체는 아직 등재 전**이다. ③은 로컬에서 해소(스위트가 실행되므로 컴파일도 함께 검사된다) — 단 CI에서는 그 8건 범위뿐이다. 실패의 원인은 제품이 아니라 하네스다: `daemon_client::start_connection` 이 `app: None` 이면 연결 태스크를 안 띄우고 `Ok(())` 로 단락하는데 테스트 생성자가 전부 `app: None` 이다(커밋 `ffcd766`, 2026-07-01부터 · 운영 경로는 `app: Some` 이라 무영향 · **원인 서술의 정본 = 그 파일 주석**). ★**수치·명령·CI 등재 상태를 여기 베끼지 않는다 — 정본 = CLAUDE.md 「빌드·검증 명령」의 `lib_unit` 줄**★(베낀 수치가 실제로 하루 만에 낡았다).
- **왜 위험한가:** 레이아웃 권위가 `src-tauri`에 있다(ADR-0035). 즉 **뷰·슬롯·분할의 백엔드 로직 전체가 자동 회귀망 밖**이고, 관측 지점이 GUI 실측 하나뿐이다 — S22의 방향 어휘 개명에서 실제로 그랬다. 로컬에서 `cargo check -p engram-dashboard --all-targets`로 컴파일만은 확인되지만 이건 게이트가 아니라 사람이 기억해야 도는 명령이다.
- **필요한 것:** ~~① `0xc0000139`의 원인 규명~~ → **규명·해소 완료(2026-08-24)**: 원인은 `tauri-build`/`embed-resource`가 Windows `RT_MANIFEST` 리소스를 `cargo:rustc-link-arg-bins=` 로만 링크하고 cargo가 그것을 `[[bin]]` 타깃에만 적용한 것이었다 — manifest 없는 테스트 exe가 comctl32 v5.82를 물고 `TaskDialogIndirect` 부재로 로더가 `main` 전에 죽였다. 해법 = `[[test]]` 선언으로 kind=test 타깃을 만들고 `cargo:rustc-link-arg-tests=` 로 그쪽에도 링크(**결정·거부한 대안 = ADR-0174**). ★`--lib`·`--all-targets`는 여전히 즉사한다★(`[lib] test = false`는 *기본 선택*에서만 뺀다 — 설명 정본 = `src-tauri/Cargo.toml` 주석). ② 대안 경로 결정 — 레이아웃 로직을 테스트 가능한 crate로 분리(ADR-0012의 seam 분리 판단은 사용자 결정). ★후단이었던 `cargo check --all-targets` CI 게이트 승격은 아래 사용자 결정으로 이미 근거를 잃었고, 지금은 그와 별개로 **실행되는 스위트가 생겨** 대체 수단으로서도 의미가 옅다.★
- **★새로 필요한 것 — 이 항목이 남은 갭과 결정권의 정본이다(2026-08-24)★:** 실패를 만드는 `app: None` 단락을 어떤 모양으로 고칠지가 **사용자 결정**이다(후보 = `run_connection` 이 `Option<AppHandle>` 을 받아 emit no-op / 테스트용 `AppHandle` 주입). **그 결정에 걸려 있는 것은 이제 하나뿐이다 — `--test lib_unit` 스위트 *전체*의 CI 등재.** ~~㉡ `src-tauri/bindings/` 동기 게이트 확장~~ 은 **먼저 풀렸다**: CI가 그 스위트에서 내보내기 테스트만 이름으로 골라 도는 스텝을 세워 빨간 건을 건드리지 않고 그 8개를 다시 굽고, 기존 sync 게이트가 세 디렉터리를 전부 대조한다. **즉 순서 의존이라던 두 항목 중 하나는 순서를 타지 않았다.** ★이 결정의 ADR은 아직 미채번이다★ — **ADR-0174는 테스트 타깃을 세운 방법의 결정이지 이 수정 모양의 결정이 아니다.** 둘을 같은 것으로 읽지 말 것.
...
- **★동기가 바뀌었다(사용자 결정 2026-08-23)★:** 기존 테스트 파일에 미련을 갖지 않고 분리 가능한 단위에 초점을 맞춘다. 그래서 위 ② 후단(`cargo check --all-targets` 게이트 승격)과 이 항목의 「작성돼 있으나 아무도 돌리지 않는다」 프레이밍은 **근거를 잃었다** — 살릴 대상(미실행 209개)을 버리기로 했으므로 복구할 그물이 없다. 남는 분리 동기는 ㉠ 앞으로 쓸 테스트가 워크스페이스 회귀에 실린다 ㉡ 의존 방향이 컴파일러로 강제된다, 둘뿐이다.
  - ★**그 결정의 전제가 2026-08-24에 무너졌다 — 결정 자체는 그대로 선다(사용자 것이다)**★. "복구할 그물이 없다"는 이제 사실이 아니다: 그 스위트는 로컬에서 돌고 **대부분이 초록이며 빨간 것은 한 축에 몰려 있다**(수치는 위 「증상」이 가리키는 정본). 즉 「미실행 209개를 버린다」는 이제 **안 돌아가는 것을 버리는 것**이 아니라 **도는 것을 버리는 것**이다. ★이 문서는 그 긴장을 드러내기만 하고 판정하지 않는다 — 재론 여부는 사용자 몫★.
