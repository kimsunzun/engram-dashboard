...
### T-27. 갓 스폰된 터미널 수신자가 배달된 편지를 제출하지 않는다
...
### T-28. 우편함 키를 이름에서 고유 id로 재키
- **상태:** 보류(사용자 명시 보류 결정 2026-08-17/18 — 기각 아님. ADR-0140이 거부한 대안 절에 그 취지로 기록).
- **출처:** ADR-0140.
...
### T-29. `in_flight_targets`에 fail-open 백스톱 없음
...
- **지금 안전한 이유:** 등록(`insert`)과, 해제를 쥔 `FlightSettle` 구성 사이에 실패 가능한 연산(조기 return · `?` · 패닉 가능 호출)이 하나도 없다는 불변식이 유일한 보호장치다.
- **재론 트리거(기록해 둘 것):** 그 구간에 조기 return이 하나라도 들어오는 변경. 대상 id가 그 집합에 갇히면 그 세션은 다시는 우편을 못 받는다 — 이 서브시스템에서 가장 나쁜 실패 모드고, 조용히 일어난다.
