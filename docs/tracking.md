...
### T-27. `src-tauri` 테스트가 로컬·CI 어디서도 실행되지 않는다
...
- **증상:** ① 워크스페이스 회귀 명령이 `--exclude engram-dashboard`로 이 패키지를 뺀다(그 lib 테스트 타깃이 `0xc0000139 STATUS_ENTRYPOINT_NOT_FOUND`로 죽어서 — 실측 2026-08-05). ② CI 백엔드 잡도 **같은 제외**를 쓴다. ③ `cargo build`는 `#[cfg(test)]` 본문을 컴파일하지 않으므로 **컴파일 검사조차 안 된다.** 결과: `src-tauri/src/layout/{types,tree,spatial,manager}.rs`·`output_router.rs`의 테스트는 **작성돼 있으나 아무도 돌리지 않는다.**
...
- **필요한 것:** ① `0xc0000139`의 원인 규명(DLL 진입점 실패로 보이나 미확정 — `ldd`는 누락 DLL 0을 보고했다) ② 못 고치면 대안 경로 결정 — 레이아웃 로직을 테스트 가능한 crate로 분리(ADR-0012의 seam 분리 판단은 사용자 결정)하거나, 최소한 `cargo check --all-targets`를 CI 게이트로 승격.
