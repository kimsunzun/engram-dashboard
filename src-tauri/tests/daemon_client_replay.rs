...
//! (`0xc0000139 STATUS_ENTRYPOINT_NOT_FOUND`, 실측 2026-08-20 재확인) — `src/daemon_client/tests.rs` 의
//! mock WS 하네스를 포함해 `#[cfg(test)]` 안의 단언은 한 번도 돌지 않는다. 실행:
...
//! 소켓에 미는 줄. 그 자리는 실 소켓·실 `AppHandle` 없이 세울 수 없다
