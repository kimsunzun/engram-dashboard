...
//! (데몬→셸)과는 별개의 다리다.
//!
//! ★검증이 서는 자리★: 실 소켓·실 `AppHandle` 이 필요한 조각(연결 select 루프)은 **어떤 하네스로도 세울 수
//! 없어** 그 갈래 선택만 무커버로 남고, 그 아래 전부(슬롯 조회 · 결말 조립 · 채널 배달 · 등록 패킷 내용)는
//! 소켓 없는 하네스가 실코드로 덮는다 — 잔여 목록의 정본은 `tests/layout_commands.rs` 헤더다.
//! ★무커버로 남는 사유는 그 요구 하나뿐이다★ — 같은 패키지의 단위 스위트가 도는지 여부와는 무관하다.
//!
