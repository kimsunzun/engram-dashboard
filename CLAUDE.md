...
- **단위 테스트는 기능 단위로 그때그때 작성**한다. 나중에 `cargo test`(workspace 루트) 한 번으로 전 모듈을 한꺼번에 검증할 수 있게 누적한다. 데몬 모듈은 phase 6 격리 하네스(integration test + harness bin)로 한 번에 돌도록 모은다.
