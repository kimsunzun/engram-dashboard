...
- **설명 추상화 레벨 — 미시 코드명은 숨기고, 아키텍처명은 흘린다.** 사용자 설명·브리핑에서 **변수명·함수명 같은 미시 코드 고유명사를 나열하지 않는다** — 코드를 안 본 사용자는 그게 잔뜩 깔리면 이해 자체가 막힌다. 그 코드가 *무엇을 하는지* 역할·동작으로 풀어 말한다. 반대로 **클래스·모듈 단위의 큰 아키텍처 이름(`AgentManager`·`ProtocolClient`·`TauriTransport` 등)은 빼지 말고 문맥에 의도적으로 계속 흘린다** — 사용자가 반복 노출로 코드베이스 지도를 점진 학습하는 장치다(미시=숨김, 거시=노출, **정반대로 다룬다**). **일반 기술 용어**(직렬화·락·경쟁 조건·타임아웃·상태 머신·발행구독·멱등성·액터·백프레셔 등)는 그대로 쓰되 맥락에서 짧게 짚는다(생소할 법한 것만 단서 한 조각). 비유는 보조 수단(필요할 때만), 주력은 한 단계 추상화한 직접 설명이다.
