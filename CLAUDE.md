...
모든 백엔드가 같은 인터페이스(start/write_input/resize/kill/output)를 구현한다. 차이는 구조가 아니라 **capability 유무**. 출력은 종류를 가정하지 않는다(터미널 강제 금지) — `OutputEvent`+`capabilities.output`로 종류를 구분하고 슬롯이 렌더러를 고른다(터미널=xterm / API=구조화·마크다운). (ADR-0002)
...
프론트는 이 매트릭스로 미지원 옵션을 회색 처리. 목록은 resume/resize만 확실, "API 옵션"은 codex_api 실측 때 확장.
...
- **숨은 의도·불변식은 그 코드에 자세히 박는다 — 시그니처·타입만 봐선 안 보이는 load-bearing 의미.** "이 변수가 사실 테스트 격리 탈출구다", "이 분기가 어떤 race를 막는다", "이건 detached여야 한다" 같이 *코드 형태로는 설명 안 되는 의미*는 빠뜨리면 다음 세션이 모르고 "불필요"로 지우거나 잘못 바꾼다. (실제 사례: `ENGRAM_DATA_DIR`이 테스트 격리용인데 주석이 없어 "배포 노브"로 오해→제거→통합 테스트 격리·운영 폴더 오염. 주석 한 줄이 막았을 사고.) 일일이 다는 건 노이즈 — *의도가 섞인 지점*만 깊게.
- **load-bearing 파일은 `//!` overview 헤더로 역할·불변식·진입점 요약 — 상세 규약·예시는 `docs/reference/commenting-conventions.md`(ADR-0032).** 점진 권고(soft) — 만지는 파일부터 boy-scout로.
