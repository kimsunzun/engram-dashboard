...
> 새 기능·단계는 위에서 아래로 좁힌다: **PRD → TRD → 모듈 경계(DDD) → 구현+TDD.** ADR(결정)과 step-log(흐름)는 이 과정 내내 옆에서 계속 기록된다.

1. **PRD — 무엇/왜.** 요구사항을 먼저 고정한다. (`docs/process/.../spec/`)
2. **TRD — 어떻게.** 세부 구현·인터페이스를 확정한다. 굵은 설계 결정은 이 단계부터 ADR로 박는다(끝나고가 아니라 결정 나는 즉시).
3. **모듈 경계 긋기 (DDD).** seam으로 영역을 분할한다 — 각 모듈이 외부 의존(Tauri/네트워크/실제 프로세스)을 끊고 **단독 검증 가능**하게. (ADR-0012)
4. **구현 + TDD.** 기능 단위로 테스트를 먼저(또는 함께) 쓴다. 테스트는 *명세한 동작*을 지키는 회귀 안전망·환각 거름망이지 "완전성 보장"이 아니다(경계를 잘 그어야 강해진다). 실제 코드 변경은 아래 **구현 실행 규약**(서브에이전트)으로 수행.
