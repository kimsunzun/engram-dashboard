...
> "결정권은 사용자" 기조를 실천하는 법. 기조 자체는 완화 아님.

- **사용자가 체감하는 것 = 사용자 결정.** 동작·정책·데이터 위치가 여기 든다.
- **안 보이는 내부 구현 = 메인이 정하고 반드시 보고.** 라이브러리·상수·코드 배치. 떠넘기기도 무보고도 위반이다.
- **결정을 올릴 땐 코드가 아니라 동작으로 번역한다.** 각 선택엔 거부한 대안 한 줄 → ADR.
- **보고는 두 층. 위층엔 코드 이름·명령·경로·수치를 쓰지 않는다.** 결론과 이유만 쓰고 근거·재현·세부는 구분선 아래로 내린다. 사용자는 위층만 읽고 판단할 수 있어야 한다.
- **구조 이름은 위층에서도 쓴다.** `AgentManager`·`ProtocolClient`·command 버스 같은 거시 이름은 반복 노출이 목적이다. 한글 역할명 + 괄호 영문명으로 적는다.
...
<examples>
<bad>
"터미널 화면부품이 등록을 취소한다" → TerminalSlot인지 웹뷰인지 슬롯인지 불명
