...
- `cargo test -p engram-dashboard --test layout_apply` — 셸 패키지(`src-tauri`)의 **통합** 테스트 타깃(IPC 핸들러 16종을 전송 중립 서비스 뒤로 옮긴 리팩터의 회귀망). ★**이 줄이 그 스위트를 도는 유일한 경로다**★ — 맨 위 워크스페이스 회귀가 이 패키지를 통째로 빼기 때문. 빠진 근거인 `0xc0000139`로 죽는 것은 **lib 테스트 타깃뿐**이고, **통합 타깃은 `engram_dashboard_lib`를 링크해 정상 기립한다**(실측 2026-08-17). 그래서 `--test`로 타깃을 집는다 — `-p`만 쓰거나 `--tests`로 넓히면 죽는 lib 타깃을 도로 끌어온다. **`-- --test-threads=4`는 붙이지 않는다** — 인메모리 fake 포트뿐이라 자식 프로세스를 하나도 안 띄운다(위 병렬 항목의 판정 규칙이 그대로 적용된 결과).
- `cargo build` — 전체 workspace 빌드. ★**테스트 타깃은 컴파일하지 않는다**★ — 이 명령만으로는 위 셸 통합 스위트가 깨져도 안 보인다.
