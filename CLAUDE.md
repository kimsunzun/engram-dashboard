...
- **브랜치 이름 = `v<버전>/<타입>/<슬러그>`.** 버전은 현재 제품 버전(`package.json`·`src-tauri/Cargo.toml`), 타입 어휘는 커밋과 같다(`feat`·`fix`·`refactor`·`docs`·`chore`·`ci`). 버전을 앞에 두는 이유 = git GUI가 `/`를 폴더로 접어 **릴리스별로 묶여 보인다**(사용자 결정 2026-08-17). **워크트리 이름(`wt1`·`wt2`) 금지** — 그래프에서 무슨 작업인지 안 보인다. ★브랜치 이름의 `v0.1.0/`은 릴리스를 발행하지 않는다★ — 워크플로 트리거가 브랜치와 태그를 따로 받는다(실측 2026-08-17). 발행되는 건 **태그** `v*`뿐이다(아래).
