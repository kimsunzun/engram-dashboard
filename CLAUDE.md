...
- **커밋은 게이트 통과 후에만.**
- **브랜치 = 작업 덩어리 이름.** master에서 주제 브랜치를 파고(`<타입>/<슬러그>` — 타입 어휘는 커밋과 같다: `feat`·`fix`·`docs`·`chore`·`ci`), 덩어리가 끝나면 master로 통합한다. **워크트리 이름(`wt1`·`wt2`)을 브랜치 이름으로 쓰지 않는다** — 그래프에서 무슨 작업인지 안 보인다(사용자 결정 2026-08-16, 옛 규약 폐기 — `docs/reference/branch-strategy.md`의 `wip/aN` 안도 같이 폐기). 세션이 두 주제에 걸치면 브랜치를 새로 판다.
- **★master에 직접 커밋하지 않는다★** — 통합 직후 워크트리는 master에 앉아 있으니 **다음 덩어리는 반드시 새 주제 브랜치를 파고 시작한다.** 빠뜨리면 머지 커밋이 안 생겨 이 규약이 스스로 무효화된다(옛 "워크트리 이름 브랜치" 규약이 보장하던 것이 여기로 옮겨왔다). 세션 격리는 "워크트리 하나에 브랜치 하나"로 유지 — **남의 워크트리가 든 브랜치는 커밋도 정리(삭제·개명)도 하지 않는다.** 옛 이름을 쓰는 브랜치가 남아 있어도 그 세션이 끝낼 때까지 둔다.
- **통합 순서 = 주제 브랜치 push → CI 초록 확인 → `--no-ff` 머지 → 브랜치 삭제(로컬·원격 둘 다).** push를 건너뛰고 머지하면 로컬 게이트를 CI에 위임한 구조상(`/qa` 바인딩) **그 코드의 첫 CI가 master에서 처음 돌아**, 빨간 경우 master가 이미 오염된다. `--no-ff`가 핵심인 이유 = fast-forward면 머지 커밋이 없어 **브랜치를 지우는 순간 이름이 그래프에서 증발**한다(덩어리 경계를 남기는 게 이 규약의 목적). 머지 커밋 본문에 무엇이 들어갔는지 항목으로 적는다.
- **거부한 대안 = PR 플로우.** CI에 `pull_request` 트리거가 살아 있어도 통합을 PR로 하지 않는다 — 1인 저장소라 승인자가 자기 자신이고, 필요한 것(덩어리 경계·작업 이름)은 머지 커밋으로 이미 얻는다. (사용자 결정 2026-08-16 — 다시 꺼내지 말 것)
