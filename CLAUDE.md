...
- **세션 핸드오프(/handoff 인계):** 정본 = **handoff 스킬** — 저장 위치·파일명·형식 전부 스킬이 정의(변경 가능 영역이라 경로 하드코딩 금지).
...
- **에이전트 4상태 → 메시지 3결말 — 맨 "부재"·"비활성" 금지**(실발생 2026-07-30: 사용자는 "부재"로 *턴 중*을, spec은 "부재"로 *없음*을 뜻해 결정이 꼬였다). 두 축을 섞지 말고 이렇게 부른다. **에이전트 상태(3):** ① **살아 있음**(`Running`·`Exiting` — 턴 중이든 아니든) ② **잠듦**(프로세스 닫힘·트리 항목(프로필) 남음·복원 가능) ③ **없음**(프로필도 없는 이름 = 오타·미스폰·삭제됨). **메시지 결말(3):** **배달**(①) · **파킹**(① 중 턴 신호 있는 백엔드가 턴 중일 때 + ②) · **반려**(③). **구조화 출력(턴 신호)은 상태 축이 아니라 "언제 넣을지" 축이다** — 신호가 없으면 게이트 없이 즉시 주입한다(ADR-0116 결정 7). *결정 근거* = ADR-0116, *구현 계약* = spec §5(두 정본을 섞지 말 것).
- **`AgentCommand` — 같은 이름이 세 곳, 뜻은 둘. "러스트 `AgentCommand`" 라고 부르면 뜻이 안 정해진다 — crate 를 붙여 부를 것.** ① `protocol` 의 `AgentCommand` = **데몬에 보내는 wire 명령**(`Spawn`/`Kill`/`Interrupt` …) ② core `agent::profile` 의 `AgentCommand` = **프로필이 띄울 프로그램**(`{kind:'Claude'|'Shell'}`) ③ 프론트 `src/api/types.ts` 의 `AgentCommand` = ②의 미러이고, ②의 **wire 미러 이름은 `AgentSpawnCommand`** 다(경계를 넘으며 이름이 바뀐다). **배선 함정(방향 주의):** 생성 바인딩 `AgentSpawnCommand.ts` 는 프론트 `AgentCommand` 와 **구조가 같아 바꿔 껴도 tsc 가 아무 말 안 한다** — 다만 그건 *올바른* 짝이라 안전하다. 위험한 건 이름이 같은 wire `AgentCommand.ts` 쪽인데, 이건 externally-tagged(`{"Spawn":…}`)라 잘못 끼우면 tsc 가 잡는다 — **단 그 방어는 타입 모양이 아니라 그 값을 짓는 테스트 픽스처 4곳에서 나온다**(프론트 프로덕션 코드는 이 값을 만들지도 읽지도 않는다). 픽스처를 걷어내면 방어도 조용히 사라진다. 이름 정책은 wire 타입 배선 시점의 미결(구현 갈림길 = 사용자 결정).
...
**기계적 작업은 `/adr` 스킬(전역 배포 — 스크립트는 스킬 내장 `scripts/adr.mjs`)이 자동화** — 채번·스캐폴드·인덱스 재생성·supersede 양방향(전체/부분)·drift lint. 폐기 판단과 본문 prose는 호출자(메인/사용자). 결정 날조 금지(거부한 대안·근거는 사용자 제공·fact-check). 이 프로젝트의 스킬 바인딩(실명령·경로)은 `.claude/skill-bindings/<skill>.md`가 정본이다(2026-07-07 — 스킬 골격은 전역 SSOT로 이사, 프로젝트 사본 없음).
