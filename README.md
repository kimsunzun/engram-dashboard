...
에이전트 명부와 프리셋은 실행파일 옆에 만들어지는 `engram-data\` 폴더에 저장됩니다. **이 폴더를 지우거나 옮기기 전에 앱을 완전히 종료하세요** — 앱이 켜져 있는 동안에는 데몬이 `engram-data\daemon.json`(접속 정보 겸 중복 실행 방지 장치)을 붙들고 있어서 그 폴더의 삭제도 이동·이름 변경도 거부됩니다. 앱을 끈 뒤에는 앱 폴더째 지우면 데이터도 함께 사라지고, 폴더째 옮기면 따라옵니다. 쓰기 권한이 없는 곳(`C:\Program Files` 등)에 압축을 풀면 이 폴더를 만들 수 없어 데몬이 기동하지 않으니, 사용자 폴더처럼 쓸 수 있는 위치에 두세요.
