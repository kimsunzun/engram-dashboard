...
function extractStatusVocab(statusLineBody, vocab = DEFAULT_STATUS_VOCAB) {
  ...
  //   긴 어휘 우선(정렬)으로 접두 충돌 방지. 선두 앵커라 뒤에 붙는 절(부분폐기 도장 등)은 무시된다.
  const m = stripped.match(new RegExp(`^(${vocabAlt(vocab)})(?![가-힣])`));
  return m ? m[1] : null; // 어휘 없음 = 형식 위반 후보
}

// 어휘 목록 → 정규식 대안 문자열. 긴 어휘 우선(정렬)으로 접두 충돌 방지 + 메타문자 이스케이프.
function vocabAlt(vocab) {
  return [...vocab].sort((a, b) => b.length - a.length).map((v) => v.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|');
}

// ── 부분 폐기 도장(상태줄) ────────────────────────────────────────────────────
// partial supersede 는 관련줄에 Amends/Amended by 양방향 링크를 박는다. 그것만으론
//   *헤더(상태)만 읽는 독자*가 부분 폐기를 놓친다 → 상태줄에도 도장을 찍는다(대시보드 ADR-0066 수작업 관습 자동화).
//   형태: "- 상태: 확정 (2026-07-10, 근거: ...) · 부분 폐기 by ADR-0067 (조항)"
//
// ★위치 = head 끝(끼워넣기 금지). 어휘·"폐기 (Superseded by ADR-N)" 같은 기존 토큰을 *쪼개지 않는다*:
//   - 어휘 바로 뒤에 끼우면 비-굵은 전체폐기 줄("- 상태: 폐기 (Superseded by ADR-N) — 사유")에서
//     "폐기"와 "(Superseded by"가 갈라져 full 재폐기 가드·인덱스 파생이 뚫린다.
//   - head 끝에 붙이면 어휘·전체폐기 링크가 붙어 있는 채로 남고, 기존 도장 뒤에 쌓인다
//     (번호가 커지는 순서로 찍히므로 결과가 오름차순처럼 보일 뿐 — 정렬을 보장하진 않는다. 손으로 끼운 도장은 그 자리에 남는다.)
//   head 경계 = *괄호 밖* 첫 em-dash(없으면 줄 끝). 괄호 안 em-dash("근거: 사용자 결정 — consult ...")는
//   실데이터에 흔해서, 거기 끼우면 근거 문장 한복판을 가른다(파서엔 무해하나 문서가 망가진다).
function partialStampRe(newNum) {
  // 번호 뒤 경계: 숫자뿐 아니라 영문도 막는다 — "ADR-0003X" 같은 비정상 토큰을 ADR-0003 도장으로 오인하면
  //   진짜 도장이 누락된다(도장 없음 = 헤더 독자가 부분폐기를 놓침).
  return new RegExp(`·\\s*부분 폐기 by ADR-${pad4(newNum)}(?![0-9A-Za-z])`);
}

// 괄호 깊이 카운터용 — 전각 괄호（）도 반각과 같이 센다(한글 문서에 섞여 들어온다).
const PAREN_OPEN = new Set(['(', '（']);
const PAREN_CLOSE = new Set([')', '）']);
const PAREN_PAIR = new Map([['(', ')'], ['（', '）']]); // 종류 대응(입력 검증용 스택)

// 각 문자 *앞*에서의 괄호 깊이 배열. 원문을 안 건드린다(치환 사본은 금지 —
//   공백 치환은 지워진 구간을 가로질러 정규식이 이어 붙는 가짜 매치를 만든다).
function parenDepths(s) {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    else if (PAREN_CLOSE.has(ch)) depth = Math.max(0, depth - 1);
  }
  return d;
}

// 괄호 균형(타입 무관 카운터 — 레거시 상태줄 데이터는 거부할 수 없으므로 관대하게).
function isBalancedParens(s) {
  let depth = 0;
  for (const ch of s) {
    if (PAREN_OPEN.has(ch)) depth++;
    else if (PAREN_CLOSE.has(ch)) { depth--; if (depth < 0) return false; }
  }
  return depth === 0;
}

// ★불변식(깨면 조용히 손상된다): 삽입 지점은 괄호-인식 headEndIndex 를 쓰는데,
//   읽는 쪽(extractStatusVocab · deriveIndexStatus · cmdLint)은 여전히 naive 한 split('—')[0] 로 head 를 자른다.
//   안전의 근거는 "삽입 지점 ≥ naive 컷" — 깊이 0 em-dash 는 전체 em-dash의 부분집합이라 첫 깊이-0 위치는
//   항상 첫 em-dash 위치 이상이다(구조적으로 성립). 따라서 도장은 naive head 안의 내용을 절대 밀어내지 않는다.
//   실측(대시보드 상태줄 106개): 두 경계가 갈리는 줄 27개, 위반 0.
//   → 삽입 지점을 naive 컷보다 *앞*으로 옮기는 변경은 이 불변식을 깬다(읽는 쪽도 같이 고쳐야 한다).
// 괄호 밖 첫 em-dash 위치(단서절 시작). 없으면 -1.
function headEndIndex(line) {
  let depth = 0;
  for (let i = 0; i < line.length; i++) {
    const c = line[i];
    if (PAREN_OPEN.has(c)) depth++;
    else if (PAREN_CLOSE.has(c)) depth = Math.max(0, depth - 1);
    else if (c === '—' && depth === 0) return i;
  }
  return -1;
}

// 상태줄 한 줄을 받아 도장을 박은 줄을 돌려준다(멱등).
//   같은 새 ADR 번호 도장이 이미 있으면 재삽입 금지, 다른 번호는 절을 하나 더 붙인다.
//   clause 는 진입부 assertClauseSafe 로 걸러진 값이어야 한다(· — 도장/링크 키워드 금지).
//   ★멱등 검사 = *원문 head* 에 도장 정규식을 돌리고, 매치 시작 위치의 괄호 깊이가 0인 것만 도장으로 센다.
//     - 단서절(tail)의 자유서술("— 참고: · 부분 폐기 by ADR-0003 (가짜)")은 head 밖이라 애초에 안 본다.
//     - head 안이라도 괄호에 싸인 인용("(참고: · 부분 폐기 by ADR-0003 (가짜))")은 깊이>0 → 도장 아님.
//     진짜 도장은 스크립트가 찍든 손으로 찍든 항상 깊이 0에 붙는다 — 괄호 안은 인용문이다.
//     원문을 그대로 보는 게 핵심: 괄호 구간을 공백으로 지우고 스캔하면
//       ① 열림이 안 닫힌 head 는 뒤가 통째로 지워져 *기존 도장을 못 보고 매번 덧찍고*(중복 = 비가역 손상),
//       ② "· (도장 아님) 부분 폐기 by ADR-N" 처럼 지워진 구간을 가로질러 가짜 매치가 합성된다.
//   ★불균형 head 폴백: 깊이를 믿을 수 없으므로 깊이 필터 없이 원문 전체를 훑는다(보수적).
//     방향은 스킵 쪽 — 거짓 "이미 도장됨"(스킵·보존)은 감수하고, 중복 도장은 절대 안 만든다.
//     실데이터(대시보드 상태줄 106개)에 불균형 head 는 0건. 이 경로로 스킵하면 사유를 따로 보고한다.
function stampPartialStatusLine(line, newNum, clause) {
  const i = headEndIndex(line);
  const head = i < 0 ? line : line.slice(0, i);
  const re = new RegExp(partialStampRe(newNum).source, 'g');
  if (!isBalancedParens(head)) {
    if (re.test(head)) return { line, stamped: false, reason: 'unbalanced-head-conservative' };
  } else {
    const depths = parenDepths(head);
    let m;
    while ((m = re.exec(head))) {
      if (depths[m.index] === 0) return { line, stamped: false, reason: 'already-stamped' };
    }
  }
  const stamp = ` · 부분 폐기 by ADR-${pad4(newNum)} (${clause})`;
  if (i < 0) return { line: `${line}${stamp}`, stamped: true, position: 'line-end' };
  return { line: `${head.replace(/\s+$/, '')}${stamp} ${line.slice(i)}`, stamped: true, position: 'before-em-dash' };
}

// --clause 는 옛/새 ADR의 상태줄·관련줄에 그대로 박히는 자유 텍스트다. 파서가 쓰는 구분자·키워드가
//   섞이면 도장·양방향 링크를 위조하거나 head 창을 어긋나게 만든다
//   (멱등 검사 오탐 → 진짜 도장 스킵 / full 가드 거짓거부 / lint 거짓 양방향 불일치 / 인덱스 파생 손실).
//   이스케이프·자동 치환은 뜻을 조용히 바꾸므로 금지 — 거부하고 다시 쓰게 한다.
const CLAUSE_FORBIDDEN = [
  ['·', "가운뎃점('·') = 도장·관련줄 구분자"],
  ['—', "em-dash('—') = 상태줄 단서절 경계"],
  ['부분 폐기 by', '"부분 폐기 by" = 부분폐기 도장 키워드'],
  ['Superseded by', '"Superseded by" = 전체폐기 가드 키워드'],
  ['Supersedes', '"Supersedes" = 전체폐기 링크 키워드'],
  ['Amended by', '"Amended by" = 부분폐기 링크 키워드'],
  ['Amends', '"Amends" = 부분폐기 링크 키워드'],
];
function assertClauseSafe(label, value) {
  if (value === undefined || value === null) return;
  const lower = value.toLowerCase();
  for (const [tok, why] of CLAUSE_FORBIDDEN) {
    if (lower.includes(tok.toLowerCase())) fail(`${label} 에 ${why} 포함 불가 — 파서 토큰과 충돌한다. 조항 문구를 바꿔 재시도: "${value}"`);
  }
  // 괄호 균형: 도장은 "(...)"로 감싸 붙고 삽입 위치 계산이 괄호 깊이를 스캔한다 → 불균형이면 이후 도장 위치가 어긋난다.
  //   *입력*은 종류까지 따지는 스택으로 본다 — 개수만 세면 "（mismatch)" 가 통과해
  //   인덱스 셀은 반각 ')' 에서 잘리는데 잘림 경고(반각 카운트 기준)는 안 뜨는 엇갈림이 생긴다.
  //   (headEndIndex 는 종류 무관 카운터 유지 — 레거시 상태줄 데이터는 거부할 수 없다. 실데이터 엇갈림 0건.)
  const stack = [];
  for (const ch of value) {
    if (PAREN_OPEN.has(ch)) stack.push(PAREN_PAIR.get(ch));
    else if (PAREN_CLOSE.has(ch)) {
      const want = stack.pop();
      if (want !== ch) fail(`${label} 의 괄호가 안 맞음 — 여는 괄호와 닫는 괄호의 종류가 다르거나 짝이 없다(반각 ()·전각（）를 섞지 말 것). 괄호를 맞춰 재시도: "${value}"`);
    }
  }
  const depth = stack.length;
  if (depth !== 0) fail(`${label} 의 괄호가 안 맞음 — 도장 삽입 위치 계산이 어긋난다. 괄호를 맞춰 재시도: "${value}"`);
}
...
function scaffold({ num, title, status, related }, cfg = {}) {
  // 로컬 날짜 — toISOString()은 UTC라 KST 새벽에 하루 어긋남(실측 2026-07-08 ADR-0007)
  const d = new Date();
...
const countChar = (s, ch) => [...s].filter((c) => c === ch).length;
...
function buildIndex(cfg) {
  ...
  for (const adr of adrs) {
    ...
    // 조항 잘림 경고: 셀 문구가 불완전할 수 있음을 드러낸다(파생 출력은 그대로 — 기존 인덱스 바이트 보존).
    if (der.clauseTruncated) {
      warnings.push({ num: adr.num, kind: 'index-clause-truncated', field: 'status',
        msg: `ADR-${pad4(adr.num)}: 인덱스 status 셀의 조항이 괄호에서 잘림(관련줄 "Amended by ADR-N (...)" 안의 괄호 때문에 첫 ')' 까지만 캡처) — 셀 문구가 불완전하다. 정본은 본문 관련줄.`,
        clause: der.clause });
    }
...
// ── 진입 ──────────────────────────────────────────────────────────────────────
// ADR_LIB_ONLY=1|true 로 import 하면 CLI를 안 돌리고 순수 함수만 노출한다(테스트 하네스용).
//   ★그 두 값만 인정한다 — "0"·"false"·"" 같은 값이 환경에 새어 있어도 CLI는 정상 실행된다
//     (truthy 검사로 두면 ADR_LIB_ONLY=0 환경에서 모든 명령이 조용히 no-op + exit 0 이 된다).
//   lib 모드일 땐 stderr 로 한 줄 알린다 — 누수가 조용히 넘어가지 않게.
//   "직접 실행 판별"(argv[1] 비교) 대신 opt-out 인 이유: 심링크로 배포되는 스킬 폴더에서
//   argv[1] ≠ import.meta.url 이 되어 CLI가 조용히 no-op 할 수 있다.
